  ...
  if command -v node >/dev/null 2>&1; then
    CLEANED=$(node -e "
      ...
      function isStalePluginHook(cmd) {
        ...
        // Skip entries with env prefix (intentional user override)
        if (cmd.startsWith('env ')) return false;
        return pluginHookPatterns.some(p => p.test(cmd));
      }
...
if [ -n "$FIX_SCRIPT" ]; then
  if [ "$SKIP_FIX" = true ]; then
    node "$FIX_SCRIPT" --dry-run $VERBOSE_FLAG
  else
    node "$FIX_SCRIPT" --fix $VERBOSE_FLAG
  fi
  if [ $? -ne 0 ]; then
    EXIT_CODE=1
    echo ""
...
if [ -n "$FIX_SCRIPT" ]; then
  if [ "$VERBOSE_FLAG" = "--verbose" ]; then
    node "$FIX_SCRIPT" --verify-runtime $VERBOSE_FLAG
  else
    node "$FIX_SCRIPT" --verify-runtime >/dev/null 2>&1
  fi
  if [ $? -ne 0 ]; then
    EXIT_CODE=1
