...
if [ -f "cross-platform-plugin/scripts/lib/agent-deliverable-validator.js" ]; then
    echo "   ✅ Validator script found"
else
    echo "   ❌ Validator script missing"
    ERRORS=$((ERRORS + 1))
...
if [ -f "cross-platform-plugin/scripts/lib/agent-completion-validator.js" ]; then
...
if [ -f "cross-platform-plugin/scripts/lib/user-expectation-tracker.js" ]; then
...
if node -e "const T = require('./cross-platform-plugin/scripts/lib/user-expectation-tracker'); const t = new T(); t.initialize().then(() => t.close()).catch(() => process.exit(1));" 2>/dev/null; then
...
if [ -f "cross-platform-plugin/scripts/lib/business-process-coverage-tracker.js" ]; then
    echo "   ✅ Tracker script found"
else
    echo "   ❌ Tracker script missing"
    ERRORS=$((ERRORS + 1))
...
# Check if database can be initialized
if node -e "const T = require('./cross-platform-plugin/scripts/lib/business-process-coverage-tracker'); const t = new T(); t.initialize().then(() => t.close()).catch(() => process.exit(1));" 2>/dev/null; then
    echo "   ✅ Database initialization works"
else
    echo "   ⚠️  Database initialization failed (may need sqlite3 npm package)"
fi
