...
LOG_FILE="/tmp/agent-hook-mandatory.log"
BYPASS_FILE="/tmp/agent-bypass-reasons.log"
...
suggest_for_medium_risk() {
    ...
    declare -A AGENT_PATTERNS=(
        ...
        ["sfdc-field-analyzer"]="field.*analysis|formula|validation.*rule"
...
main() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "${CYAN}  Task tool with subagent_type='${REQUIRED_AGENTS[$RISK_CATEGORY]%% or*}'${NC}"
        echo ""
        echo -e "${YELLOW}Press Ctrl+C to cancel, or ENTER to see agent details${NC}"
        read -r
        ...
        echo "  Use this agent to safely perform your operation."
        echo "  The agent will handle all complexity and safety checks."
        echo ""
        ...
        # For medium-risk, allow proceeding but track decision
        if [ -n "$TASK_INPUT" ]; then
            echo ""
            echo -e "${BOLD}Continue without agent? [y/N]:${NC} "
            read -r continue_choice

            if [ "${continue_choice,,}" != "y" ]; then
                echo -e "${GREEN}Good choice! Use 'agent-suggest' to find the right agent.${NC}"
                exit 0
            else
                echo -e "${YELLOW}Why are you proceeding without an agent? (for analytics):${NC}"
                read -r skip_reason
                track_decision "$TASK_INPUT" "SKIPPED" "$skip_reason"
                echo -e "${YELLOW}Proceeding without agent. Good luck!${NC}"
            fi
        fi
    fi
}
