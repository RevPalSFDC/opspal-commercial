...
describe('PermissionSetOrchestrator', () => {
  ...
  describe('No-Downgrade Enforcement', () => {
    it('should block field permission downgrades', () => {
      const existing = {
        fieldPermissions: [
          { object: 'Account', field: 'Name', readable: true, editable: true }
        ]
      ...
      assert.throws(() => {
        orchestrator.merger.merge(existing, additions);
      }, /Permission downgrades detected/);
    });

    it('should block object permission downgrades', () => {
      const existing = {
        objectPermissions: [
          {
            object: 'Account',
    ...
    it('should allow downgrades when explicitly enabled', () => {
      ...
      assert.strictEqual(merged.fieldPermissions[0].editable, false); // Downgraded
    });
  });
