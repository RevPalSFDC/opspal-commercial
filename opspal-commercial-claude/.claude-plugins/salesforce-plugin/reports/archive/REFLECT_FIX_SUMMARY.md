...
📈 Query your reflections:
   node .claude-plugins/salesforce-plugin/scripts/lib/query-reflections.js recent
```
...
1. Check hook registration:
   ```bash
   cat .claude-plugins/salesforce-plugin/.claude-plugin/plugin.json | grep -A 3 hooks
   ```

2. Run with debug mode:
   ```bash
   REFLECT_DEBUG=1 node .claude-plugins/salesforce-plugin/scripts/lib/submit-reflection.js \
