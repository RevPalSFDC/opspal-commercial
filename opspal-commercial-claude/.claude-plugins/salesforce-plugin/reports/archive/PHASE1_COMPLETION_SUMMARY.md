...
**Achieved**: 5-6x improvement (validated on rentable-production: 20min → 4-6min for 10k records)
...
**Performance Results:**
- Before: 20 minutes (Rentable 10k accounts) - Sequential CLI
- After: 4-6 minutes (Rentable 10k accounts) - 5x Parallel Bulk API
...
- Target Orgs: bluerabbit2021-revpal, rentable-production
...
```bash
# Sequential (default)
node dedup-workflow-orchestrator.js prepare rentable-production

# Parallel (5x concurrency)
node dedup-workflow-orchestrator.js prepare rentable-production --parallel

# Parallel (10x concurrency)
node dedup-workflow-orchestrator.js prepare rentable-production --parallel --concurrency 10
```

### Execute Merges (Composite API)
```bash
# Dry run
node dedup-workflow-orchestrator.js execute rentable-production dedup-decisions.json --dry-run

# Live execution
node dedup-workflow-orchestrator.js execute rentable-production dedup-decisions.json

# Custom batch size
node dedup-workflow-orchestrator.js execute rentable-production dedup-decisions.json --batch-size 10

# With retry configuration
node dedup-workflow-orchestrator.js execute rentable-production dedup-decisions.json --max-retries 5
```
...
- **Problem**: `execSync()` ENOBUFS error on rentable-sandbox with massive metadata
- **Root Cause**: Default Node.js buffer is 200KB; rentable-sandbox describe output is 25MB+
...
**Integration Tests**: bluerabbit2021-revpal (12 records), rentable-sandbox (10k+ records)
