...
validate_field_before_deployment() {
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-validator.js validate-field-design \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"; then
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-conflict-detector.js check-field-conflicts \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"; then
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-dependency-validator.js validate-field-dependencies \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"; then
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-impact-analyzer.js analyze-field-impact \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"; then
...
deploy_field_with_comprehensive_validation() {
    ...
    node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/dependency-analyzer.js check-field \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"
    ...
    if [ $? -ne 0 ]; then
        ...
        node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/conflict-detector.js resolve \
            --object "${object_name}" \
            --field "${field_name}" \
            --auto-resolve
    fi
    ...
    local monitor_id
    monitor_id=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/monitoring/metadata-monitor.js start-field-deployment \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}")

    # Execute validated deployment
    if node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validated-metadata-deployer.js deploy-field \
        --manifest "${manifest}" \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}" \
        ...
        if validate_field_after_deployment "${object_name}" "${field_name}" "${org_alias}"; then
            node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/monitoring/metadata-monitor.js complete-deployment \
...
validate_field_after_deployment() {
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/permission-validator.js validate-field-permissions \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"; then
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-integrity-validator.js validate-field-integrity \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}"; then
...
trigger_metadata_recovery() {
    ...
    failure_analysis=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-failure-analyzer.js analyze \
        --object "${object_name}" \
        --field "${field_name}" \
        --org "${org_alias}")
    ...
    case "${failure_analysis}" in
        ...
        "validation_rule_conflict")
            ...
            node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validation-rule-conflict-resolver.js resolve \
                --object "${object_name}" \
                --field "${field_name}" \
                --org "${org_alias}"
            ;;
