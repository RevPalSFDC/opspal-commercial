...
- **Org Alias**: bluerabbit2021-revpal
- **Instance URL**: https://bluerabbit2021--revpal.sandbox.my.salesforce.com
- **API Version**: v65.0
- **Org Type**: Sandbox
- **Username**: cacevedo@wedgewood.com.revpal
...
sf data query --query "SELECT Id, Name, Active, SobjectType FROM AssignmentRule WHERE SobjectType = 'Lead'" --use-tooling-api --target-org bluerabbit2021-revpal

# Query Case Assignment Rules
sf data query --query "SELECT Id, Name, Active, SobjectType FROM AssignmentRule WHERE SobjectType = 'Case'" --use-tooling-api --target-org bluerabbit2021-revpal
```
...
sf project retrieve start --metadata AssignmentRules:Lead --target-org bluerabbit2021-revpal
...
```bash
# Query real Users
sf data query --query "SELECT Id, Name, IsActive FROM User WHERE IsActive = true LIMIT 5" --target-org bluerabbit2021-revpal

# Query real Queues
sf data query --query "SELECT Id, DeveloperName, Type FROM Group WHERE Type = 'Queue' LIMIT 5" --target-org bluerabbit2021-revpal

# Validate a real User ID
node scripts/lib/assignee-validator.js validateUser <user-id> bluerabbit2021-revpal

# Validate a real Queue ID
node scripts/lib/assignee-validator.js validateQueue <queue-id> bluerabbit2021-revpal
```
...
```bash
# Query sample Leads
sf data query --query "SELECT Industry, State FROM Lead LIMIT 10" --target-org bluerabbit2021-revpal
...
node scripts/lib/validators/assignment-rule-validator.js preDeployment <test-rule.xml> bluerabbit2021-revpal
...
node scripts/lib/validators/assignee-access-validator.js checkUserAccess <user-id> Lead bluerabbit2021-revpal

# Check Queue object access
node scripts/lib/validators/assignee-access-validator.js checkQueueAccess <queue-id> Lead bluerabbit2021-revpal

# Audit all assignees
node scripts/lib/validators/assignee-access-validator.js auditAccess <rule> bluerabbit2021-revpal
```
...
sf data create record --sobject Group --values "Name='Test Assignment Queue' Type='Queue'" --target-org bluerabbit2021-revpal
...
node scripts/lib/assignment-rule-deployer.js deploy <test-rule.xml> Lead bluerabbit2021-revpal

# Step 5: Verify deployment
sf data query --query "SELECT Id, Name, Active FROM AssignmentRule WHERE SobjectType = 'Lead' AND Name = 'Healthcare_CA_Test'" --use-tooling-api --target-org bluerabbit2021-revpal

# Step 6: Test with sample Lead (DO NOT ACTIVATE RULE)
sf data create record --sobject Lead --values "FirstName='Test' LastName='User' Company='TestCo' Industry='Healthcare' State='CA'" --target-org bluerabbit2021-revpal

# Step 7: Clean up - Delete test rule
sf data delete record --sobject AssignmentRule --record-id <rule-id> --use-tooling-api --target-org bluerabbit2021-revpal
```
...
node scripts/lib/assignment-rule-deployer.js backup Lead bluerabbit2021-revpal ./backups

# Verify backup file created
ls -la ./backups/

# Test restore (dry-run)
node scripts/lib/assignment-rule-deployer.js restore ./backups/Lead_<timestamp>.xml bluerabbit2021-revpal --dry-run
```
...
curl -X POST "https://bluerabbit2021--revpal.sandbox.my.salesforce.com/services/data/v65.0/sobjects/Lead/" \
...
### If Deployment Fails
1. Run: `node scripts/lib/assignment-rule-deployer.js restore <backup-path> bluerabbit2021-revpal`
...
3. Contact Wedgewood admin if needed

### Emergency Contacts
- **Wedgewood Admin**: cacevedo@wedgewood.com
...
**Org Environment**: Wedgewood RevPal Sandbox (bluerabbit2021-revpal)
