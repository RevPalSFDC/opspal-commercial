...
**Key Findings**:
- ✅ Successfully connected to Wedgewood RevPal Sandbox
...
sf data query --query "SELECT Id, Name, Active FROM AssignmentRule" \
  --use-tooling-api --target-org bluerabbit2021-revpal --json
```
...
sf project retrieve start --metadata AssignmentRules:Lead \
  --target-org bluerabbit2021-revpal
```
...
sf data query --query "SELECT Id, Name, IsActive FROM User \
  WHERE IsActive = true LIMIT 5" --target-org bluerabbit2021-revpal

# Query Queues
sf data query --query "SELECT Id, DeveloperName, Type FROM Group \
  WHERE Type = 'Queue' LIMIT 5" --target-org bluerabbit2021-revpal
```
...
### Key Achievements
1. ✅ Successfully connected to Wedgewood RevPal Sandbox
...
**Sandbox Environment**: Wedgewood RevPal Sandbox (bluerabbit2021-revpal)
