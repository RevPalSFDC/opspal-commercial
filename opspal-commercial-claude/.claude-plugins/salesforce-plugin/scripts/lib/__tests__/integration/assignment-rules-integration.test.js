...
<AssignmentRules xmlns="http://soap.sforce.com/2006/04/metadata">
  ...
  describe('Workflow 1: Complete Rule Creation & Deployment', () => {
    it('should successfully create and deploy a new assignment rule', async () => {
      ...
      expect(validation.criticalIssues).toBe(0);
      expect(validation.checksCompleted).toBe(20);

      // Step 6: Build XML for deployment
      const deployXml = deployer.buildAssignmentRuleXML(rule);
  ...
  describe('Workflow 2: Rule Modification & Conflict Resolution', () => {
    it('should detect conflicts when modifying existing rule', async () => {
      ...
      expect(riskScore).toBeGreaterThan(30); // Medium/High risk

      // Step 3: Generate recommendations
      const reordering = overlapDetector.suggestReordering(rule.entries);
      expect(reordering.recommended).toBeDefined();
      expect(reordering.rationale).toBeDefined();

      // Step 4: Apply reordering
      const reorderedEntries = reordering.recommended;
      expect(reorderedEntries[0].order).toBeLessThan(reorderedEntries[1].order);
    ...
    it('should handle circular routing detection', async () => {
      ...
      const hasCircular = overlapDetector.detectCircularRouting(circularChain);
      expect(hasCircular.circular).toBe(true);
      expect(hasCircular.path).toBeDefined();
      expect(hasCircular.path).toContain('User A');
    });
  });
  ...
  describe('Workflow 4: Access Validation & Permission Audit', () => {
    ...
    it('should detect assignees without proper access', async () => {
      ...
      expect(audit.inaccessibleAssignees).toBeGreaterThan(0);
    });
  });
  ...
  describe('Workflow 5: Backup & Rollback', () => {
    it('should backup existing rule before deployment', async () => {
      const existingRuleXml = testXmlContent;
  ...
  describe('Workflow 6: Error Recovery', () => {
    it('should handle deployment failure gracefully', async () => {
      ...
      await expect(
        deployer.deployRule(testXmlContent, 'Lead', testOrgAlias)
    ...
    it('should retry deployment on transient errors', async () => {
      ...
      const successResponse = mockOrgResponses.deploySuccess;
      ...
      execSync
        ...
        .mockReturnValueOnce(JSON.stringify(successResponse));
