...
**Scenario**: You're managing a Salesforce instance (`rentable-sandbox`) and want to:
...
**Observation Created**:
```bash
instances/rentable-sandbox/observations/deployment-2025-10-20-143000.json
```
...
await Task({
    ...
    prompt: `Validate deployment package for rentable-sandbox.
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-versioner.js --org rentable-sandbox --action list
```

**Output**:
```
📚 Version History: rentable-sandbox
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-differ.js --org rentable-sandbox --from v1.0.0 --to v1.1.0
```
