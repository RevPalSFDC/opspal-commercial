...
```bash
# Full context (JSON)
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org <org-alias>
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org <org-alias> --operation-type deployment

# Filter by objects
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org <org-alias> --objects "Account,Contact,Opportunity"

# Write to file
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org <org-alias> --output /tmp/context.json
```
...
if echo "$CONTEXT" | jq -e '.criticalExceptions[] | select(contains("field history"))'; then
    ...
    node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/field-history-validator.js --org $ORG_ALIAS
fi
```
```
...
```bash
# Delete runbook
rm instances/rentable-sandbox/RUNBOOK.md
```
