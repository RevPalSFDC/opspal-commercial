...
#
# Usage: bash .claude-plugins/cross-platform-plugin/scripts/setup-prevention-system.sh
#
...
jq --arg root "\${CLAUDE_PLUGIN_ROOT}" '. + {
  "hooks": {
    "UserPromptSubmit": {
      "command": "bash \($root)/.claude-plugins/cross-platform-plugin/hooks/master-prompt-handler.sh",
      "timeout": 10000,
    ...
    "SessionStart": {
      "command": "bash -c '"'"'\($root)/.claude-plugins/salesforce-plugin/hooks/session-start-agent-reminder.sh && \($root)/.claude-plugins/cross-platform-plugin/hooks/session-context-loader.sh'"'"'",
      "timeout": 5000,
...
if [ -f "$PROJECT_ROOT/.claude-plugins/cross-platform-plugin/hooks/master-prompt-handler.sh" ]; then
...
if [ -f "$PROJECT_ROOT/.claude-plugins/cross-platform-plugin/hooks/prevention-system-orchestrator.sh" ]; then
...
if [ -f "$PROJECT_ROOT/.claude-plugins/cross-platform-plugin/hooks/session-context-loader.sh" ]; then
