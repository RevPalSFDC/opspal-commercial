...
class ACEHealthCheck {
    ...
    async checkHooks() {
        ...
        // Check if hooks contain ACE integration
        const boosterHook = path.join(hookDir, 'subagent-utilization-booster.sh');
        if (fs.existsSync(boosterHook)) {
            const content = fs.readFileSync(boosterHook, 'utf-8');
            component.details.ace_integrated = content.includes('ACE FRAMEWORK') || content.includes('skill-routing-boost');
    ...
    async checkCacheDirectory() {
        ...
        if (fs.existsSync(CONFIG.CACHE_DIR)) {
            ...
            component.details.agent_stats_files = statsFiles.length;
            ...
            if (statsFiles.length > 0) {
                ...
                component.details.oldest_cache_age_hours = ((Date.now() - oldest) / 3600000).toFixed(1);
                component.details.newest_cache_age_hours = ((Date.now() - newest) / 3600000).toFixed(1);
            }
        } else {
