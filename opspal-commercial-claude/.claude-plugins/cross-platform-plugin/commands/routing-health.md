...
### 2. File System Check
- Hook file exists: `.claude-plugins/cross-platform-plugin/hooks/user-prompt-router.sh`
- Hook is executable
...
```bash
# Run setup script
.claude-plugins/cross-platform-plugin/scripts/setup-auto-routing.sh
```
...
**Solution**:
```bash
node .claude-plugins/cross-platform-plugin/scripts/lib/routing-index-builder.js
```
...
```bash
.claude-plugins/cross-platform-plugin/scripts/routing-health-check.sh
```
