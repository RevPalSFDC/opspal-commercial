...
/schedule-add --name="ACE Weekly Maintenance" \
  --type=script \
  --schedule="0 3 * * 0" \
  --command="node .claude-plugins/cross-platform-plugin/scripts/lib/ace-maintenance-runner.js"
```
...
Steps:
1. Run ACE health check using `node .claude-plugins/cross-platform-plugin/scripts/lib/ace-health-check.js`
2. If Supabase is configured, run confidence decay: `node .claude-plugins/cross-platform-plugin/scripts/lib/strategy-confidence-decay.js`
3. Clean expired cache files in `~/.claude/cache/ace-routing/` older than 24 hours
4. Generate performance summary using `node .claude-plugins/cross-platform-plugin/scripts/lib/ace-execution-recorder.js health`
