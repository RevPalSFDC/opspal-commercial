...
allowed-tools: Read, Write, Bash, mcp__asana__asana_get_task, mcp__asana__asana_list_tasks
...
```bash
./.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scripts/lib/asana-connection-manager.sh fix
```
...
```javascript
const { AsanaTaskReader } = require('./.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scripts/lib/asana-task-reader');
...
1. [TEST] Salesforce Field Audit - Wedgewood Sandbox
   Priority: Medium
   Due: 2025-10-28
   Status: In Progress
...
=== Task Details ===
Name: [TEST] Salesforce Field Audit - Wedgewood Sandbox
Priority: Medium
Status: In Progress
Due: 2025-10-28
...
Requirements:
✓ Audit custom fields on Account and Contact objects in wedgewood sandbox
✓ Identify unused fields (0% population)
✓ Generate recommendations report
...
Instructions:
1. Connect to wedgewood RevPal sandbox
...
```bash
# Parse specific task
node ./.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scripts/lib/asana-task-reader.js 1211748609238981
