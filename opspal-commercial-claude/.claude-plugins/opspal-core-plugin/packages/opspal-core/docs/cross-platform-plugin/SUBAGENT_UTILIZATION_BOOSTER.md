...
```json
{
  "hooks": {
    "UserPromptSubmit": {
      "command": "bash ${CLAUDE_PLUGIN_ROOT}/.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/subagent-utilization-booster.sh",
      "timeout": 5000,
...
grep -r "Sub-agent Utilization" .claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/reports/
...
**Check**:
```bash
echo '{"message":"test"}' | bash .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/subagent-utilization-booster.sh | jq .
```

**Expected**: `{"systemMessage": "Using the appropriate sub-agents, test"}`

**Fix**:
- Verify hook is executable: `chmod +x .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/subagent-utilization-booster.sh`
...
CLAUDE_PLUGIN_ROOT=/path/to/plugin echo '{"message":"deploy to production"}' | \
  bash .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/subagent-utilization-booster.sh | jq .
```
...
**Documentation**: `.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/docs/`
**Hook Location**: `.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/subagent-utilization-booster.sh`
