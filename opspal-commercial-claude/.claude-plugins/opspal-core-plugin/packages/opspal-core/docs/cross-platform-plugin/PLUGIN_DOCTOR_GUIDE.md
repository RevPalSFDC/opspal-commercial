...
**Fix:**
```bash
cd .claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin
...
Edit the agent file `.claude-plugins/opspal-core-plugin/packages/domains/hubspot/agents/hubspot-workflow-orchestrator.md`:
...
⚠️ Agent 'sfdc-custom-validator' defined in multiple locations
  Location 1: .claude-plugins/opspal-core-plugin/packages/domains/salesforce/agents/
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/query-reflections.js \
...
**Cause**: cross-platform-plugin not installed or not loaded
...
# Check script exists
ls -la .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scripts/lib/plugin-health-checker.js
...
# Check network
curl -I https://REDACTED_SUPABASE_PROJECT.supabase.co
```
