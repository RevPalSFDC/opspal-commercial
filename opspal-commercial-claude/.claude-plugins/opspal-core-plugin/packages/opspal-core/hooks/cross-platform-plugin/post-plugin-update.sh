...
# Location: .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/post-plugin-update.sh
#
# Features:
...
HEALTH_CHECKER="$PLUGIN_ROOT/cross-platform-plugin/scripts/lib/plugin-health-checker.js"
...
if [ -n "$PLUGIN_NAME" ]; then
  ...
  if node "$HEALTH_CHECKER" --plugin "$PLUGIN_NAME" --json > /tmp/plugin-health-check.json 2>&1; then
    # Health check passed
    ISSUES=$(node -e "const data = require('/tmp/plugin-health-check.json'); console.log(data.summary.totalErrors + data.summary.totalWarnings);")
  ...
  # Clean up temp file
  rm -f /tmp/plugin-health-check.json
  ...
  if node "$HEALTH_CHECKER" --json > /tmp/plugin-health-check.json 2>&1; then
    # Parse results
    HEALTHY=$(node -e "const data = require('/tmp/plugin-health-check.json'); console.log(data.healthy.join(', '));")
    ERRORS=$(node -e "const data = require('/tmp/plugin-health-check.json'); console.log(data.summary.totalErrors);")
    WARNINGS=$(node -e "const data = require('/tmp/plugin-health-check.json'); console.log(data.summary.totalWarnings);")
