...
function resolveSalesforceProfilerModule() {
  const candidates = [
    path.resolve(__dirname, '../../../../..', 'domains', 'salesforce', 'scripts', 'lib', 'agent-profiler'),
    path.resolve(__dirname, '../../../../../..', 'salesforce-plugin', 'scripts', 'lib', 'agent-profiler'),
    path.resolve(process.cwd(), '.claude-plugins', 'opspal-core-plugin', 'packages', 'domains', 'salesforce', 'scripts', 'lib', 'agent-profiler'),
    path.resolve(process.cwd(), '.claude-plugins', 'salesforce-plugin', 'scripts', 'lib', 'agent-profiler')
  ];
...
function resolveSalesforceProfilerDir(rootDir) {
  const candidates = [
    path.join(rootDir, '.claude-plugins', 'opspal-core-plugin', 'packages', 'domains', 'salesforce', '.profiler'),
...
function loadProfilerData(agentName, profilerDir, timeRange = '24h') {
  try {
    const AgentProfiler = require(resolveSalesforceProfilerModule());
...
function buildInventory(options = {}) {
  ...
  const profilerDir = options.profilerDir || resolveSalesforceProfilerDir(rootDir);
