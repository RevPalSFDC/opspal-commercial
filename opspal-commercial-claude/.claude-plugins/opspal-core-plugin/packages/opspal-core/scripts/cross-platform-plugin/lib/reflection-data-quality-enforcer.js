...
function enforceDataQuality(reflection, options = {}) {
  ...
  if (!enhanced.outcome) {
    const issues = enhanced.issues || enhanced.issues_identified || [];
    const hasP0Issues = issues.some(i => i.priority === 'P0');
    const hasP1Issues = issues.some(i => i.priority === 'P1');
  ...
  // Calculate ROI if issues present and no ROI provided
  if (!enhanced.roi_analysis && (enhanced.issues || enhanced.issues_identified)) {
    const issues = enhanced.issues || enhanced.issues_identified || [];
    const roiAnalysis = calculateROIFromIssues(issues);
    if (roiAnalysis.total_annual_roi !== '$0') {
