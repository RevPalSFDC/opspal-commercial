...
SELECT
  c.Name AS Campaign,
  c.Type AS Channel,
  COUNT(DISTINCT o.Id) AS Opps_Sourced,
...
SELECT
  c.Name AS Campaign,
  c.Type AS Channel,
  COUNT(DISTINCT o.Id) AS Opps_Influenced,
  SUM(o.Amount) AS Revenue_Attributed
FROM Opportunity o
JOIN CampaignMember cm ON cm.ContactId = o.Contact__c
JOIN Campaign c ON c.Id = cm.CampaignId
...
SELECT
  lookback_days,
  COUNT(DISTINCT o.Id) AS opps_with_attribution,
  COUNT(DISTINCT o.Id) / :total_opps AS coverage_pct
FROM Opportunity o
JOIN CampaignMember cm ON cm.ContactId = o.Contact__c
WHERE o.StageName = 'Closed Won'
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/gtm-planning/scripts/lib/attribution-calculator.js extract \
  --org <org-alias> \
  --lookback 90 \
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/gtm-planning/scripts/lib/attribution-calculator.js backtest \
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/gtm-planning/scripts/lib/attribution-calculator.js optimize-lookback \
  --org <org-alias> \
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/gtm-planning/scripts/lib/attribution-calculator.js recommend \
