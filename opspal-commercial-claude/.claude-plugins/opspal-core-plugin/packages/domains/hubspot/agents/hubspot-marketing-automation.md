...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/hubspot-marketing-automation-optimizer.js <options>
```
...
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/hubspot-marketing-automation-optimizer.js --portal my-portal
```
...
async function createTopicClusterCampaign(topic, clusterCount = 5) {
  ...
  const cluster = await Task.invoke('hubspot-seo-optimizer', JSON.stringify({
...
async function createOrganicLeadNurtureCampaign() {
  ...
  const contentAudit = await Task.invoke('hubspot-seo-optimizer', JSON.stringify({
    action: 'audit',
    scope: 'on-page',
    minScore: 70
  }));
...
async function createContentRefreshCampaign() {
  ...
  const freshnessAudit = await Task.invoke('hubspot-seo-optimizer', JSON.stringify({
    action: 'audit',
  ...
  const workflows = await Promise.all(outdatedPosts.map(async (post) => {
    ...
    const reOptimization = await Task.invoke('hubspot-seo-optimizer', JSON.stringify({
      action: 'optimize-content',
      postId: post.id,
...
Task.invoke('hubspot-seo-optimizer', JSON.stringify({
  action: 'optimize-content',
  postId: '123456789',
  targetKeyword: 'marketing automation',
