...
async function autonomousUpdate(records) {
  if (records.length < 10) {
    ...
    const ImportsAPIWrapper = require('..claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/imports-api-wrapper');
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/hubspot-autonomous-operations-optimizer.js <options>
```
...
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/hubspot-autonomous-operations-optimizer.js --portal my-portal
```
