    ...
    return await governance.executeWithGovernance(
        {
            ...
            rollbackCommand: `node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/field-rollback.js ${org} Account CustomField__c`,
    ...
    return await governance.executeWithGovernance(
        {
            ...
            rollbackCommand: `node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/permission-set-rollback.js ${org} ${permissionSetName} ${newPermissions.join(',')}`,
...
rollbackPlan: 'Remove Account.Industry_Segment__c from AgentAccess permission set field permissions. Command: node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/permission-set-rollback.js production AgentAccess Account.Industry_Segment__c'
```
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/human-in-the-loop-controller.js approve <request-id>
```
...
- **Hook entrypoint**: `.claude-plugins/opspal-core-plugin/packages/domains/salesforce/hooks/universal-agent-governance.sh`
- **Registration**: `.claude-plugins/opspal-core-plugin/packages/domains/salesforce/.claude-plugin/hooks.json`
