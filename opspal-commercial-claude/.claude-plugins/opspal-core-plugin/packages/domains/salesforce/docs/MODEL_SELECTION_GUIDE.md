...
**Model Selection** (introduced in Claude Code v2.0.30) allows agents to specify their preferred model via the `preferredModel` field in their YAML frontmatter. This optimization matches agent complexity to model capability, resulting in 40-60% cost savings while maintaining quality for appropriate tasks.

## What is preferredModel?

The `preferredModel` field is an optional hint in agent definitions that suggests which Claude model should be used:
...
preferredModel: haiku  # Optimal model for this agent's task
---
```

**Key Characteristics:**
- **Hint, not mandate**: Claude Code may override based on context
- **Cost optimization**: Haiku costs ~5x less than Sonnet for equivalent tasks
- **Quality maintenance**: Haiku excels at focused, well-defined tasks
- **Performance**: Haiku is often faster for simple operations

## Model Comparison

| Feature | Haiku | Sonnet |
|---------|-------|--------|
| **Best For** | Read-only, CRUD, validation, analysis | Planning, orchestration, complex logic, security |
| **Cost** | ~$0.25 per 1M input tokens | ~$3.00 per 1M input tokens |
| **Speed** | Faster for simple tasks | Optimal for complex reasoning |
| **Context Window** | 200K tokens | 200K tokens |
| **Quality** | Excellent for focused tasks | Superior for open-ended problems |
...
### Adding preferredModel to Agents
...
preferredModel: haiku  # Add this line
---
```
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/add-model-hints-to-agents.js --dry-run

# Verbose output
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/add-model-hints-to-agents.js --verbose

# Filter by model type
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/add-model-hints-to-agents.js --model haiku
```
...
**Symptom:** Agent with `preferredModel: haiku` produces lower quality results than expected
...
**Solutions:**
1. **Switch to Sonnet**: Change `preferredModel: sonnet`
...
**Symptom:** Agent uses different model than `preferredModel` specifies
...
**Expected Behavior:**
- `preferredModel` is a hint, not a mandate
...
grep "preferredModel" .claude-plugins/opspal-core-plugin/packages/domains/salesforce/agents/sfdc-state-discovery.md
