...
check_file_format() {
    ...
    if file "$file" | grep -q "CRLF"; then
        record_check "Line Endings" "PASS" "Windows line endings (CRLF)"
    elif file "$file" | grep -q "LF"; then
        if [[ "$AUTO_FIX" == true ]]; then
            unix2dos "$file" 2>/dev/null || true
            record_check "Line Endings" "PASS" "Converted to Windows line endings"
        else
            record_check "Line Endings" "WARN" "Unix line endings detected (Salesforce prefers CRLF)"
        fi
    fi
