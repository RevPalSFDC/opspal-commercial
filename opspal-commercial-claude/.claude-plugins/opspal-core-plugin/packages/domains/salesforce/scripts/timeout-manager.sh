...
calculate_timeout_for_records() {
    ...
    # Calculate timeout: base + (records / 100) * 10 seconds
    # This gives approximately 10 seconds per 100 records
    local calculated=$((base_timeout + (records / 100) * 10))
    
    # Cap at 2 hours
    if [ $calculated -gt 7200 ]; then
        calculated=7200
    fi
