...
class ReportPreProcessor {
  constructor(options = {}) {
    ...
    this.tempDir = options.tempDir || '/tmp/audit-pdf-processing';
  }
...
if (require.main === module) {
  ...
  console.log('  --verbose        Show detailed processing information');
    console.log('\nExample:');
    console.log('  node report-pre-processor.js ./audit-reports audit-complete.pdf --org "NeonOne" --version "3.29.0"');
    process.exit(1);
  }
  ...
  processor.process(inputDir, outputPdf, { orgName, version, coverTemplate })
    .then(() => {
