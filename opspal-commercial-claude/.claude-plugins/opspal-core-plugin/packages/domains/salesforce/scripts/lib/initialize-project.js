...
class ProjectInitializer {
  ...
  detectPlugins() {
    ...
    const markDetected = (key, label) => {
      if (!this.installedPlugins[key]) {
        this.installedPlugins[key] = true;
        console.log(`  ${colors.green}✓${colors.reset} ${label} detected`);
      }
    };

    for (const dir of pluginDirs) {
      // Check for OpsPal Core Salesforce domain
      if (fs.existsSync(path.join(dir, 'opspal-core-plugin', 'packages', 'domains', 'salesforce'))) {
        markDetected('salesforce', 'opspal-core Salesforce domain');
      }

      // Check for legacy Salesforce plugin
      if (fs.existsSync(path.join(dir, 'salesforce-plugin', '.claude-plugin', 'plugin.json'))) {
        markDetected('salesforce', 'salesforce-plugin (legacy)');
      }

      // Check for OpsPal Core HubSpot domain
      if (fs.existsSync(path.join(dir, 'opspal-core-plugin', 'packages', 'domains', 'hubspot'))) {
        markDetected('hubspot', 'opspal-core HubSpot domain');
      }

      // Check for legacy HubSpot plugin
      if (fs.existsSync(path.join(dir, 'hubspot-plugin', '.claude-plugin', 'plugin.json'))) {
        markDetected('hubspot', 'hubspot-plugin (legacy)');
      }
    }

    if (!this.installedPlugins.salesforce && !this.installedPlugins.hubspot) {
      throw new Error('No OpsPal Core domains or legacy plugins detected. Install OpsPal Core (salesforce/hubspot) or legacy plugins first.');
    }
  ...
  generateClaudeMd() {
    ...
    if (this.installedPlugins.salesforce) {
      ...
      if (templatePath) {
        ...
        content += template;
        pluginList.push('- ✅ **OpsPal Core Salesforce Domain** (v3.5.0) - 49 agents, 97 scripts, 14 commands');
    ...
    if (this.installedPlugins.hubspot) {
      ...
      if (templatePath) {
        ...
        if (content) {
          ...
          pluginList.push('- ✅ **OpsPal Core HubSpot Domain** (v1.1.0) - 35 agents, 31 scripts, 9 commands');
  ...
  mergeClaudeMdTemplates(sfdcTemplate, hsTemplate) {
    ...
    merged += `**Auto-generated by OpsPal Core Domains (Salesforce v3.5.0 + HubSpot v1.1.0)**\n\n`;
    ...
    merged += `- ✅ **OpsPal Core Salesforce Domain** (v3.5.0) - 49 agents, 97 scripts, 14 commands\n`;
    merged += `- ✅ **OpsPal Core HubSpot Domain** (v1.1.0) - 35 agents, 31 scripts, 9 commands\n\n`;
    ...
    merged += `@import .claude-plugins/opspal-core-plugin/packages/domains/salesforce/USAGE.md\n\n`;
    ...
    merged += `@import .claude-plugins/opspal-core-plugin/packages/domains/hubspot/USAGE.md\n\n`;
    ...
    merged += `---\n\n**Last Updated**: {DATE}\n`;
    merged += `**Generated by**: OpsPal Core Domains: Salesforce v3.5.0 + HubSpot v1.1.0\n`;
  ...
  generateGitignore() {
    ...
    if (this.installedPlugins.salesforce) {
      ['.sf/'].forEach(rule => rules.add(rule));
    }
