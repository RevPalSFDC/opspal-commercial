...
# Exhaustive Automation Catalog Generator for neonone - V2
# This script queries ALL automation components with correct field names

ORG_ALIAS="${1:-neonone}"
OUTPUT_DIR="./neonone-exhaustive-catalog-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
sf data query --query "SELECT Id, DeveloperName, MasterLabel, ProcessType, Status, VersionNumber, Description, LastModifiedDate, LastModifiedBy, IsActive, IsDeleted, IsOverridable, IsTemplate, TriggerObjectOrEvent.QualifiedApiName, TriggerOrder, TriggerType FROM FlowDefinitionView ORDER BY ProcessType, MasterLabel" --target-org "$ORG_ALIAS" --use-tooling-api --result-format json > "$OUTPUT_DIR/flows.json"
...
sf data query --query "SELECT Id, DeveloperName, MasterLabel, Status, VersionNumber, Description FROM FlowDefinitionView WHERE ProcessType = 'Workflow' ORDER BY MasterLabel" --target-org "$ORG_ALIAS" --use-tooling-api --result-format json > "$OUTPUT_DIR/process_builders.json"
