...
-- Properly formatted for copy-paste

-- Add columns
ALTER TABLE reflections
  ADD COLUMN IF NOT EXISTS reflection_status TEXT DEFAULT 'new';
...
-- Add constraint
ALTER TABLE reflections
  DROP CONSTRAINT IF EXISTS valid_reflection_status;

ALTER TABLE reflections
  ADD CONSTRAINT valid_reflection_status
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_reflections_status
  ON reflections(reflection_status);

CREATE INDEX IF NOT EXISTS idx_reflections_asana_task
  ON reflections(asana_task_id)
  WHERE asana_task_id IS NOT NULL;

-- Update RLS
DROP POLICY IF EXISTS "auth_update_workflow" ON reflections;

CREATE POLICY "auth_update_workflow"
  ON reflections
  FOR UPDATE
  TO authenticated
  USING (true)
