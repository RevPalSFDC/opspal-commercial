...
description: MUST BE USED for automation or flow audits. Performs comprehensive Salesforce automation analysis with namespace detection, conflict detection, cascade mapping, and risk-based implementation planning.
tools:
  - mcp_salesforce
...
- **Inventory ALL automation** (Apex Triggers, Classes, Flows, Process Builder, Workflow Rules, Assignment Rules)
- **Detect conflicts & overlaps** across automation types with human-readable explanations
...
### 2. Conflict Detection (8 Core Rules)
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/automation-audit-v2-orchestrator.js <org-alias> <output-dir> [--exclude-managed]

# Example (include all automation)
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/automation-audit-v2-orchestrator.js neonone ./instances/neonone/automation-audit-v2-$(date +%Y-%m-%d)

# Example (exclude managed packages - focus on custom code only)
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/automation-audit-v2-orchestrator.js neonone ./instances/neonone/automation-audit-v2-$(date +%Y-%m-%d) --exclude-managed
```
...
`;
  ...
  const cascadeDiagram = await Task.invoke('diagram-to-lucid-asana-orchestrator', {
...
`;
    ...
    const conflictDiagram = await Task.invoke('diagram-to-lucid-asana-orchestrator', {
...
`;
  ...
  const erdDiagram = await Task.invoke('diagram-to-lucid-asana-orchestrator', {
...
```bash
# Full org audit
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/automation-inventory-orchestrator.js \
  --org production \
...
const FlowComplexityCalculator = require('.claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-complexity-calculator');
...
### Service Contract
**Path**: `../../../opspal-core/developer-tools-plugin/scripts/lib/report-service.js`
**Documentation**: `../../../opspal-core/developer-tools-plugin/config/central_services.json`

### Quick Reference

```javascript
const ReportService = require('../../../opspal-core/developer-tools-plugin/scripts/lib/report-service.js');
