...
**Load context:** `CONTEXT=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org [org-alias] --operation-type merge_operations --format json)`
...
```javascript
const GenericRecordMerger = require('.claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/generic-record-merger');
...
async loadMergeProfile(objectType) {
  const profilePath = `.claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/merge-profiles/${objectType.toLowerCase()}-merge-profile.json`;
...
# 4. Test in sandbox
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/generic-record-merger.js sandbox \
...
async function loadObjectValidator(objectType) {
  const validatorMap = {
    'Contact': '.claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validators/contact-merge-validator',
    'Lead': '.claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validators/lead-merge-validator',
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/procedure-a-field-restoration.js {org} {survivor-id} --dry-run
```

**Procedure B: Entity Separation** (Type 1 - Different Entities)
```bash
# Different entities merged, needs separation + contact migration
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/procedure-b-entity-separation.js {org} {survivor-id}
```

**Procedure C: Quick Undelete** (Type 1 - Within 15 Days)
```bash
# Different entities merged, quick undo
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/procedure-c-quick-undelete.js {org} {survivor-id}
```
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/sfdc-full-backup-generator.js {org} Account
```

2. **Importance Fields Detected**:
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/importance-field-detector.js {org} Account
```

3. **Pre-Merge Validation Passed**:
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/sfdc-pre-merge-validator.js {org} Account
```
...
Use templates from `../../../opspal-core/cross-platform-plugin/templates/asana-updates/`:
...
```javascript
const { AsanaUpdateFormatter } = require('../../../opspal-core/cross-platform-plugin/scripts/lib/asana-update-formatter');
