...
run_node_validator() {
    ...
    if [ ! -f "${LIB_DIR}/picklist-dependency-validator.js" ]; then
        print_info "Node.js validator not found, skipping advanced validation"
        return 0
    fi

    print_info "Running comprehensive validation via picklist-dependency-validator.js..."

    # Note: This would require a CLI wrapper for the validator
    # For now, we'll just inform the user
    print_info "To run comprehensive validation manually:"
    print_info "  node ${LIB_DIR}/picklist-dependency-validator.js \\"
    print_info "    --org $org \\"
    print_info "    --object $object_name \\"
    print_info "    --controlling $controlling_field \\"
    print_info "    --dependent $dependent_field"
