...
PLUGIN_DIR="$(resolve_domain_root "$SCRIPT_DIR")"
PROJECT_DIR="$(resolve_project_root "$PLUGIN_DIR")"
if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ]; then
  PROJECT_DIR="$(resolve_project_root "$CLAUDE_PLUGIN_ROOT")"
fi
    ...
    if [ -n "$ORG_CONTEXT_JSON" ] && [ "$ORG_CONTEXT_JSON" != '{}' ]; then
        ...
        if command -v jq &>/dev/null; then
            ...
            if [ -n "$DETECTED_ORG" ] && [ "$DETECTED_ORG" != "null" ]; then
                ...
                echo "{\"alias\":\"$DETECTED_ORG\",\"source\":\"$ORG_SOURCE\",\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\"}" > /tmp/sf-org-context.json
