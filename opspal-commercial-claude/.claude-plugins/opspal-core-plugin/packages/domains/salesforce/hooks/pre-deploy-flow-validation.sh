...
if [ -z "$LEGACY_ROOT" ]; then
    LEGACY_ROOT="$(cd "$SCRIPT_DIR/../../../../../.." && pwd)/salesforce-plugin"
...
VALIDATOR_SCRIPT="$DOMAIN_ROOT/scripts/lib/flow-decision-logic-analyzer.js"
if [ ! -f "$VALIDATOR_SCRIPT" ] && [ -n "$LEGACY_ROOT" ]; then
    LEGACY_VALIDATOR="$LEGACY_ROOT/scripts/lib/flow-decision-logic-analyzer.js"
    if [ -f "$LEGACY_VALIDATOR" ]; then
        VALIDATOR_SCRIPT="$LEGACY_VALIDATOR"
    fi
fi
...
FIELD_REF_VALIDATOR="$DOMAIN_ROOT/scripts/lib/flow-field-reference-validator.js"
if [ ! -f "$FIELD_REF_VALIDATOR" ] && [ -n "$LEGACY_ROOT" ]; then
    LEGACY_FIELD_VALIDATOR="$LEGACY_ROOT/scripts/lib/flow-field-reference-validator.js"
    if [ -f "$LEGACY_FIELD_VALIDATOR" ]; then
        FIELD_REF_VALIDATOR="$LEGACY_FIELD_VALIDATOR"
    fi
fi
...
while IFS= read -r flow_file; do
    if [ -n "$flow_file" ]; then
        ...
        if node "$VALIDATOR_SCRIPT" "$flow_file" > /tmp/flow-validation-$$.json 2>&1; then
            ...
            # Show errors
            if [ -f /tmp/flow-validation-$$.json ]; then
                cat /tmp/flow-validation-$$.json | grep -E "ERROR|WARNING" | head -5
            fi
        fi
