...
if [ -f scripts/scrape-sf-connected-apps.js ]; then
  ...
  echo "Test with: ORG=production node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/scrape-sf-connected-apps.js"
fi
```
...
3. Run the scraper with appropriate flags:
   - `HEAD=1 ORG=[org-name] node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/scrape-sf-connected-apps.js` (headed, for initial auth)
   - `ORG=[org-name] node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/scrape-sf-connected-apps.js` (headless, uses saved session)
