...
create_validated_field() {
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-validator.js validate-new-field \
        --object "${object_name}" \
        --field "${field_name}" \
    ...
    node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validated-metadata-generator.js generate-field \
        --object "${object_name}" \
        --field "${field_name}" \
        --spec "${field_spec}" \
...
create_validation_rule_with_flow_analysis() {
    ...
    if echo "${formula}" | grep -q "PRIORVALUE"; then
        ...
        local flow_impact
        flow_impact=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-impact-analyzer.js analyze-validation-rule \
            --object "${object_name}" \
            --formula "${formula}" \
        ...
        if [[ "${flow_impact}" == "blocking" ]]; then
            ...
            node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-safe-validator-generator.js suggest-alternatives \
                --object "${object_name}" \
    ...
    node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validated-validation-rule-creator.js create \
        --object "${object_name}" \
        --rule-name "${rule_name}" \
        --formula "${formula}" \
        --validate-syntax \
...
deploy_metadata_batch_with_validation() {
    ...
    if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-package-validator.js validate-batch \
    ...
    deployment_plan=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-deployment-planner.js plan \
        --manifest "${package_manifest}" \
        --org "${org_alias}" \
        --optimize-sequence)
    ...
    while IFS= read -r deployment_phase; do
        ...
        if ! node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validated-metadata-deployer.js deploy-phase \
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-best-practices-validator.js ./flows/MyFlow.flow-meta.xml
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/fls-aware-field-deployer.js Account '{"fullName":"Field__c","type":"Text","length":255}' --org myorg
```
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/metadata-deployment-planner.js plan --manifest package.xml --optimize-sequence
```
