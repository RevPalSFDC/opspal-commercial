...
2. **If configured, invoke the gemini-consult agent**:
   ...
   ```
   Task(subagent_type='gemini-consult', prompt='<user's question or topic>')
   ```
...
2. Invoke: `Task(subagent_type='gemini-consult', prompt='How should I structure my authentication middleware?')`
3. The agent will:
