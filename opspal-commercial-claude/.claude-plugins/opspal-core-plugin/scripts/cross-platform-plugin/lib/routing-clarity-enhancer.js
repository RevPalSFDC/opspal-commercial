 ...
 *   const result = await enhancer.route("Run CPQ assessment for hivemq");
 *
...
class RoutingClarityEnhancer {
    ...
    async route(taskDescription, options = {}) {
        ...
        const semanticResult = this.semanticRouter.route(taskDescription);
    ...
    generateExplanation(semanticResult, taskDescription) {
        ...
        const agentMetadata = this.routingIndex.agents?.[agent];
        if (agentMetadata) {
    ...
    generateAlternatives(semanticResult) {
        ...
        const alternatives = semanticResult.matches.slice(1, 4).map(match => {
            const agentMetadata = this.routingIndex.agents?.[match.agent];
...
Examples:
  node routing-clarity-enhancer.js route "Run CPQ assessment for hivemq"
  node routing-clarity-enhancer.js stats
