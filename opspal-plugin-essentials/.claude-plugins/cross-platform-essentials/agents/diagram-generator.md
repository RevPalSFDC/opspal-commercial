...
**Need a diagram?** Start with these examples:

### Example 1: Process Flowchart (Beginner)
```
Use diagram-generator to create a flowchart showing my lead-to-opportunity conversion process
from first contact through closed-won
```
**Takes**: 1-2 minutes | **Output**: Professional flowchart in Mermaid format

### Example 2: Entity Relationship Diagram (Intermediate)
```
Use diagram-generator to create an ERD showing the relationships between
Account, Contact, Opportunity, and OpportunityContactRole objects in Salesforce
```
**Takes**: 2-3 minutes | **Output**: ERD with all relationships mapped

### Example 3: Integration Sequence Diagram (Advanced)
```
Use diagram-generator to create a sequence diagram showing the API flow when
a new contact is created in HubSpot and synced to Salesforce, including
webhook triggers and field mappings
```
**Takes**: 3-4 minutes | **Output**: Detailed sequence diagram

### Example 4: State Diagram
```
Use diagram-generator to show the lifecycle states for an Opportunity:
Prospecting → Qualification → Proposal → Negotiation → Closed Won/Lost
```
**Takes**: 1-2 minutes | **Output**: State transition diagram

**💡 TIP**: All diagrams are generated in Mermaid format and can be:
- Embedded in Markdown documentation
- Rendered in GitHub, GitLab, Notion
- Exported to PNG/SVG via online tools
...
- **Salesforce diagrams**: `.claude-plugins/salesforce-plugin/instances/<org>/diagrams/`
- **HubSpot diagrams**: `.claude-plugins/hubspot-plugin/instances/<portal>/diagrams/`
- **Cross-platform**: `.claude-plugins/cross-platform-plugin/diagrams/`
...
await Task.invoke('diagram-generator', JSON.stringify(diagramRequest));
```
