...
**Need to clean up data?** Start with these examples:

### Example 1: Find Duplicate Contacts (Beginner)
```
Use hubspot-data-hygiene-specialist to scan for duplicate contacts
based on email address and show me the top 20 duplicate groups
```
**Takes**: 1-2 minutes | **Output**: List of duplicate contact groups with merge recommendations

### Example 2: Standardize Phone Numbers (Intermediate)
```
Use hubspot-data-hygiene-specialist to standardize all phone numbers:
- Convert to E.164 format (+1XXXXXXXXXX for US)
- Remove invalid numbers
- Flag numbers missing country codes
- Preview changes before applying
```
**Takes**: 2-3 minutes | **Output**: Phone standardization report with preview

### Example 3: Comprehensive Data Quality Audit (Advanced)
```
Use hubspot-data-hygiene-specialist to perform a complete data quality audit:
- Find and merge duplicates (Contacts and Companies)
- Standardize formatting (phones, emails, addresses)
- Identify incomplete records (missing required fields)
- Detect data anomalies (invalid dates, out-of-range values)
- Generate cleanup plan prioritized by impact
- Provide rollback capability for all changes
```
**Takes**: 5-10 minutes | **Output**: Complete data quality report with prioritized action plan

### Example 4: Salesforce Sync Preparation
```
Use hubspot-data-hygiene-specialist to prepare data for Salesforce sync:
- Identify records that will cause sync errors
- Standardize field formats to match Salesforce requirements
- Merge duplicates that would create sync conflicts
- Validate required fields are populated
```
**Takes**: 3-5 minutes | **Output**: Sync readiness report with pre-sync cleanup tasks

**💡 TIP**: Run data hygiene monthly. Organizations with monthly cleanups report 50% fewer sync errors and 35% better campaign performance.
...
node scripts/lib/hubspot-merge-strategy-selector.js <master-id> <duplicate-id>
...
```javascript
const MergeStrategySelector = require('./scripts/lib/hubspot-merge-strategy-selector');
...
async function resolveDuplicate(masterId, duplicateId) {
  ...
  if (strategy.strategy === 'LIFT_AND_SHIFT') {
    ...
    const AssociationMigrator = require('./scripts/lib/association-migrator');
...
```bash
# Scan for duplicates
node scripts/lift-and-shift-company-duplicates.js --scan

# Preview resolution
node scripts/lift-and-shift-company-duplicates.js --dry-run --max=3

# Execute resolution
node scripts/lift-and-shift-company-duplicates.js --confirm --max=N
...
## Asana Integration for Data Hygiene Operations

### Overview

For large-scale data cleanup operations tracked in Asana, provide stakeholders with progress updates on duplicate detection, merging, and data quality improvements.

**Reference**: `../../cross-platform-plugin/docs/ASANA_AGENT_PLAYBOOK.md`

### When to Use Asana Updates

Post updates for cleanup operations that:
- Process > 1,000 records
- Require Salesforce sync validation
- Involve duplicate merging
...
```markdown
**Progress Update** - Contact Data Cleanup

**Completed:**
- ✅ Scanned 15,000 contacts for duplicates
- ✅ Identified 450 duplicate groups (3% of database)
- ✅ Validated SF sync status (12 sync-protected records)

**In Progress:**
- Merging 200 of 450 duplicate groups (44% complete)

**Next:**
- Complete remaining merges
- Re-scan for quality validation
...
```markdown
**✅ COMPLETED** - Contact Deduplication

**Deliverables:**
- 450 duplicate groups resolved (1,200 records merged)
- Data quality report: [link]
- SF sync integrity: 100% maintained

**Results:**
- Success rate: 100% (0 data loss)
- Database reduction: 8% (1,200 duplicates removed)
- SF sync protected: 12 records (lift-and-shift used)
- Processing time: 2.8 hours

**Quality Improvements:**
- Email validity: 100%
- Complete profiles: +15%
- Association integrity: Verified ✅

**Handoff:** @sales-ops for data validation

**Notes:** SF sync-protected records documented in report
```

### Hygiene-Specific Metrics

Include in updates:
- **Duplicates found/merged**: Count (e.g., "450 groups, 1,200 records")
- **Success rate**: Percentage (e.g., "100% success, 0 data loss")
- **SF sync status**: Protected count (e.g., "12 sync-protected records")
- **Quality improvement**: Metrics (e.g., "+15% complete profiles")
