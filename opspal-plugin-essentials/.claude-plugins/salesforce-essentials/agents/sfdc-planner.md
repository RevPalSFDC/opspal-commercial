...
tools: mcp_salesforce, Read, Grep, TodoWrite, ExitPlanMode
disallowedTools:
  - Write
  - Edit
  - NotebookEdit
...
**Need to plan a Salesforce project?** Start with these examples:

### Example 1: Plan Field Cleanup (Beginner)
```
Use sfdc-planner to create an implementation plan for removing
50 unused fields from the Account object that have <5% population
```
**Takes**: 2-3 minutes | **Output**: Step-by-step plan with timeline and rollback strategy

### Example 2: Plan Process Automation (Intermediate)
```
Use sfdc-planner to create a plan for automating lead assignment:
- New leads auto-assigned by territory (5 territories)
- Notification emails to assigned reps
- Escalation if not touched within 24 hours
Include time estimates, dependencies, and testing approach
```
**Takes**: 3-5 minutes | **Output**: Detailed implementation roadmap with time estimates

### Example 3: Plan Data Migration (Advanced)
```
Use sfdc-planner to create a comprehensive plan for migrating 100,000 Contacts
from legacy CRM to Salesforce:
- Analyze current state and dependencies
- Map fields between systems
- Identify data quality issues
- Plan migration phases
- Estimate effort and timeline
- Create rollback and verification procedures
```
**Takes**: 5-10 minutes | **Output**: Complete project plan with risk assessment

### Example 4: Plan Security Model Redesign
```
Use sfdc-planner to create a plan for consolidating 15 permission sets
into 5 role-based sets with least-privilege access
```
**Takes**: 4-6 minutes | **Output**: Security redesign plan with user impact analysis

**💡 TIP**: Always get stakeholder approval on plans before execution. Include time buffers (20-30%) for unexpected issues. Test in sandbox before production.
