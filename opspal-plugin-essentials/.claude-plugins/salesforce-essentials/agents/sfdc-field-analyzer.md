...
**Need to analyze fields?** Start with these examples:

### Example 1: Find Unused Fields (Beginner)
```
Use sfdc-field-analyzer to analyze the Account object and show me all fields
with less than 10% population that are safe to remove
```
**Takes**: 1-2 minutes | **Output**: List of unused fields with usage percentages

### Example 2: Field Naming Audit (Intermediate)
```
Use sfdc-field-analyzer to check my custom fields on Contact, Lead, and Account
for naming inconsistencies and recommend standardization
```
**Takes**: 2-3 minutes | **Output**: Naming issues report with standardization recommendations

### Example 3: Comprehensive Field Health Check (Advanced)
```
Use sfdc-field-analyzer to perform a complete field audit on Opportunity showing:
- Field usage statistics (population rates)
- Data type mismatches (text fields that should be picklists)
- Validation rule impacts
- Field dependencies and relationships
- Prioritized cleanup plan with rollback steps
```
**Takes**: 3-5 minutes | **Output**: Full field health report with action plan

### Example 4: Pre-Migration Field Validation
```
Use sfdc-field-analyzer to validate all required fields on Case object
before I start a data import, including picklist valid values
```
**Takes**: 1-2 minutes | **Output**: Required fields checklist with valid picklist values

**💡 TIP**: Run field analysis quarterly to maintain a clean data model. Removing unused fields improves page load times by 15-25% and reduces user confusion.
