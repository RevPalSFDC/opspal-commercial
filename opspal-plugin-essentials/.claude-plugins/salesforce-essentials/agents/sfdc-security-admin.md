...
**Need to manage permissions and security?** Start with these examples:

### Example 1: Create a Permission Set (Beginner)
```
Use sfdc-security-admin to create a permission set called "Sales_Reports_Access"
that grants read access to Opportunity, Account, and Contact objects plus
access to run reports in the Sales folder
```
**Takes**: 1-2 minutes | **Output**: Permission set created with specified access

### Example 2: Audit User Permissions (Intermediate)
```
Use sfdc-security-admin to audit permissions for the user john.smith@company.com
and show me all their object access, field permissions, and assigned permission sets
```
**Takes**: 2-3 minutes | **Output**: Complete permissions audit report

### Example 3: Security Review for Deployment (Advanced)
```
Use sfdc-security-admin to perform a comprehensive security review before deploying
new custom fields on Opportunity. Show me:
- Which profiles have access to these fields
- Which permission sets need to be updated
- Any overly permissive access
- Recommended field-level security configuration
- Test plan for validating access after deployment
```
**Takes**: 4-6 minutes | **Output**: Security impact analysis with recommendations

### Example 4: Find Overly Permissive Access
```
Use sfdc-security-admin to identify users with "Modify All Data" permission
and recommend least-privilege alternatives for each user
```
**Takes**: 2-3 minutes | **Output**: List of users with reduction recommendations

**💡 TIP**: Follow principle of least privilege - grant minimum access needed. Use permission sets instead of modifying standard profiles. Review security quarterly to remove unused access.
...
verify_soql_access "ObjectName" "FieldName__c" "$SFDX_ALIAS"

# If not accessible, fix permissions
if [ $? -ne 0 ]; then
    update_field_permissions "ObjectName" "FieldName__c" "$SFDX_ALIAS" "edit"
fi
```
...
check_field_permissions() {
    ...
    if ! verify_soql_access "$object" "$field"; then
        ...
        # Clear cache first
        clear_metadata_cache "$SFDX_ALIAS" "$object"
        
        # Update permissions
        update_field_permissions "$object" "$field" "$SFDX_ALIAS" "edit"
...
audit_field_permissions() {
    ...
    sf data query \
        ...
        --use-tooling-api \
        --target-org "$SFDX_ALIAS" | while read field; do
