...
**Need to query Salesforce data?** Start with these examples:

### Example 1: Simple Query (Beginner)
```
Use sfdc-query-specialist to show me all Accounts created this month
with their names, industries, and annual revenue
```
**Takes**: 10-30 seconds | **Output**: Clean CSV-ready data

### Example 2: Complex Query with Relationships (Intermediate)
```
Use sfdc-query-specialist to find all Opportunities over $50,000
in "Closed Won" stage with their Account names, owner email addresses,
and close dates from the last 90 days
```
**Takes**: 30-60 seconds | **Output**: Multi-object joined data

### Example 3: Query Optimization (Advanced)
```
I have this slow query that's timing out:
SELECT Id, Name, (SELECT Id FROM Contacts), (SELECT Id FROM Opportunities) FROM Account

Can you optimize it and explain why it's slow?
```
**Takes**: 1-2 minutes | **Output**: Optimized query + explanation

### Example 4: Bulk Export
```
Use sfdc-query-specialist to export all Contacts with their Account information
to a CSV file. Include: First Name, Last Name, Email, Account Name, Account Industry
```
**Takes**: 1-3 minutes | **Output**: CSV file ready for import
...
node scripts/lib/org-metadata-cache.js find-field production-org Opportunity Close
...
node scripts/lib/org-metadata-cache.js query <org> <object> | jq '.fields[] | select(.type == "reference")'
```
...
4. Validate: node scripts/lib/smart-query-validator.js <org> "<soql>"
