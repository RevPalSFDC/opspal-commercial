...
**Need to audit an object?** Start with these examples:

### Example 1: Basic Object Analysis (Beginner)
```
Use sfdc-object-auditor to analyze the Account object and show me:
- Total number of fields and record types
- Automation complexity (triggers, flows, process builders)
- Sharing model and permissions
```
**Takes**: 1-2 minutes | **Output**: Object overview with key metrics

### Example 2: Relationship Analysis (Intermediate)
```
Use sfdc-object-auditor to map all relationships for the Opportunity object,
including lookups, master-details, and junction objects
```
**Takes**: 2-3 minutes | **Output**: Visual relationship diagram with field details

### Example 3: Comprehensive Object Health Audit (Advanced)
```
Use sfdc-object-auditor to perform a complete audit of the Contact object including:
- Field usage statistics and unused fields
- Performance bottlenecks (complex triggers, validation rules)
- Data quality issues and duplicate detection
- Security review (field-level security, sharing rules)
- Optimization recommendations prioritized by ROI
```
**Takes**: 4-6 minutes | **Output**: Detailed audit report with action plan

### Example 4: Multi-Object Dependency Analysis
```
Use sfdc-object-auditor to analyze dependencies between Account, Contact,
Opportunity, and Case objects before I make schema changes
```
**Takes**: 3-4 minutes | **Output**: Dependency map showing impact radius

**💡 TIP**: Run object audits before major projects to identify technical debt. Organizations with regular audits report 40% fewer production issues during deployments.
