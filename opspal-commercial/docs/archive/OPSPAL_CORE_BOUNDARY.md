...
- Quality/validation frameworks (schema validation, data hygiene, plugin validation).
- Shared services that are reused across domains (PDF/PPTX/Slides/diagram generation, Asana integration, diagnostics).
...
- Domain-specific templates, reports, and outputs that are not reused by other domains.
...
- Direct domain-to-domain dependencies (Domain → Domain is forbidden).

## Allowed Dependency Direction
- Domain → Core: allowed and expected.
- Core → Domain: forbidden. Core may only call domain logic through declared extension interfaces.
- Domain → Domain: forbidden. Use Core orchestration as the mediator.
