...
### Added
- **Shadow Duplicate Domain Resolution Capability** for account/company hygiene
  - New script: `domain-shadow-duplicate-scanner.js`
  - New command: `/gemini-domain-resolve`
  - New agent: `gemini-domain-shadow-analyzer`
- Multi-source input support for domain canonicalization:
  - `csv` and `json` file ingestion
  - `salesforce` account query support via `sf` CLI
  - `hubspot` companies API ingestion
  - `marketo` named-account API ingestion
  - `domains` inline/list mode for quick checks
- Gemini-batch domain resolution with local HTTP redirect fallback
- Structured report output with:
  - Domain-level resolution evidence
  - Shadow duplicate clusters (`distinct input domains -> same resolved domain`)
  - Record-level references for merge planning
