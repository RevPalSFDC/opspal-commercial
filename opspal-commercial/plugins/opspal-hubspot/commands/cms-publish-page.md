...
if (options.validateSeo) {
  const seoScore = await Task.invoke('opspal-hubspot:hubspot-seo-optimizer', ...);
...
const snapshot = await Task.invoke('opspal-hubspot:hubspot-cms-page-publisher', {
...
// Step 4: Publish
if (options.schedule) {
  await Task.invoke('opspal-hubspot:hubspot-cms-page-publisher', {
    action: 'publish_page',
    pageId,
