...
test_schema_generator_basic() {
  if [[ $QUICK_MODE == true ]]; then
    SKIP_REASON="--quick mode enabled"
    return 2  # Skip in quick mode (requires network)
  fi
...
test_hubspot_deployer_dry_run() {
  ...
  if [[ $status -ne 0 ]]; then
    if [[ -z "${HUBSPOT_ACCESS_TOKEN:-}" && -z "${HUBSPOT_PRIVATE_APP_TOKEN:-}" && -z "${HUBSPOT_API_KEY:-}" ]]; then
      SKIP_REASON="missing HubSpot credentials"
      return 2
    fi
    return 1
  fi
