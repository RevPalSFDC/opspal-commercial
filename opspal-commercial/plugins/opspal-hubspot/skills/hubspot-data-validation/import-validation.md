...
For **international phone validation**, use the cross-platform `PhoneCountryDetector`:

```javascript
// RECOMMENDED: International phone normalization (245 countries)
const { PhoneCountryDetector } = require('@opspal-core/scripts/lib/phone-country-detector');
const detector = new PhoneCountryDetector();

function normalizePhone(phone, options = {}) {
  const result = detector.normalize(phone, {
    defaultCountry: options.country || 'US',
    format: 'E164'  // E164, INTERNATIONAL, or NATIONAL
  });
...
// Example usage
normalizePhone('+44 20 7946 0958');
// { valid: true, normalized: '+442079460958', country: 'GB', confidence: 0.95 }

normalizePhone('(212) 456-7890', { country: 'US' });
// { valid: true, normalized: '+12124567890', country: 'US', confidence: 0.85 }
```

**Legacy fallback** (US-only, not recommended):
```javascript
function normalizePhoneLegacy(phone) {
  const cleaned = phone.replace(/[^\d+]/g, '');
  if (!cleaned.startsWith('+')) {
    return '+1' + cleaned; // Assumes US
  }
  return cleaned;
}
```
