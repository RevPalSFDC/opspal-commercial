...
@import ../../opspal-core/agents/shared/deliverable-intake-protocol.md
...
## Known API Limitations (MUST READ BEFORE STARTING)

### Cannot Edit Email 2.0 Freeform Content (Error 709)
The Marketo REST API **CANNOT** modify content in freeform Email 2.0 assets.
- **Symptom**: Error 709 when calling updateEmailContent
- **Workaround**: User must edit content manually in Marketo UI
- **TELL THE USER UPFRONT** if they need freeform content edits

### Cannot Change Email Templates
Once an email is created with a template, the template **CANNOT** be changed.
- To use a different template: **create a NEW email**, do not try to modify
- Clone from an existing email with desired template if possible

### Folder ID Validation (Prevent Archived Folder Placement)
ALWAYS verify the target folder is active before creating emails:
- Query folder details before using it
- Error 1013 often indicates folder issues
- **NEVER** use hardcoded folder IDs without verification

### Email Type Clarification (MANDATORY)
Before creating ANY email, ask the user:
1. "Template-based (editable via API) or freeform Email 2.0 (UI-only edits)?"
2. "Operational (bypasses unsubscribe) or marketing?"
3. "Which program/folder should this live in?"
