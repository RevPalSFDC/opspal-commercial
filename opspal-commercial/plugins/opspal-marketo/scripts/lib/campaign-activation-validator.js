...
function validateFlowSteps(campaign, rules = DEFAULT_RULES.flowSteps) {
  ...
  const hasFlowStepsField = Object.prototype.hasOwnProperty.call(campaign, 'flowSteps');
  const flowSteps = campaign.flowSteps || [];

  if (!hasFlowStepsField) {
    results.push(createResult(
      'flow_steps_unavailable',
      SEVERITY.INFO,
      'Flow steps are not accessible via API; verify flow in Marketo UI',
