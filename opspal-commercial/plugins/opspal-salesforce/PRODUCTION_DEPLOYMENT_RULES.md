...
- ✅ `ACME_SANDBOX` - acme-corp staging sandbox
- ✅ `delta-sandbox` - delta-corp sandbox
- ✅ `beta-corp-uat` - beta-corp UAT sandbox
- ✅ `epsilon-corp2021-revpal` - Blue Rabbit sandbox

**NEVER deploy test code to these orgs**:
- ❌ `acme-production` - acme-corp PRODUCTION
- ❌ `delta-production` - delta-corp PRODUCTION
- ❌ `beta-production` - beta-corp PRODUCTION
...
- **What**: Deployed test Contact layouts to `acme-production` (production)
- **Impact**: None - layouts not activated, not visible to users
- **Root Cause**: Confused `acme-production` with `ACME_SANDBOX`
