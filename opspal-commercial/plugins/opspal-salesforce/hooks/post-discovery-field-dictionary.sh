...
populate_cache() {
    ...
    if node "$METADATA_CACHE_SCRIPT" info "$org_alias" &>/dev/null; then
        node "$METADATA_CACHE_SCRIPT" refresh "$org_alias" 2>&1 || {
            log_verbose "Cache refresh failed, attempting init"
            node "$METADATA_CACHE_SCRIPT" init "$org_alias" 2>&1
        }
    else
        node "$METADATA_CACHE_SCRIPT" init "$org_alias" 2>&1
    fi
