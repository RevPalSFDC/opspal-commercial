...
# Run automated setup
./.claude-plugins/opspal-salesforce/scripts/setup-validation-hooks.sh
...
Modified files requiring validation:
  - .claude-plugins/opspal-salesforce/scripts/analyze-renewals.js
...
Documentation:
  • .claude-plugins/opspal-salesforce/docs/SOQL_BEST_PRACTICES.md
```
...
node scripts/lib/deployment-source-validator.js validate-manifest ./package.xml

# Validate metadata exists in org
node scripts/lib/deployment-source-validator.js validate-metadata ApexClass:MyClass my-org
```
...
node scripts/lib/deployment-wrapper-example.js deploy \
  --source-dir ./force-app \
  --org my-org

# Validate without deploying
node scripts/lib/deployment-wrapper-example.js validate-only \
  --source-dir ./force-app \
  --org my-org
...
**Integration Example**:
```javascript
const DeploymentWrapper = require('./scripts/lib/deployment-wrapper-example');
...
node scripts/lib/deployment-source-validator.js --help
```
  ...
  deploy:
    steps:
      ...
      - name: Validate Deployment Source
        run: |
          node .claude-plugins/opspal-salesforce/scripts/lib/deployment-source-validator.js \
...
./.claude-plugins/opspal-salesforce/scripts/setup-validation-hooks.sh --force
```
...
```bash
# Check file exists
ls -la .claude-plugins/opspal-salesforce/scripts/qa/query-lint.js

# If missing, restore from git
git checkout .claude-plugins/opspal-salesforce/scripts/qa/query-lint.js
```
...
# Setup script help
./scripts/setup-validation-hooks.sh --help
```
