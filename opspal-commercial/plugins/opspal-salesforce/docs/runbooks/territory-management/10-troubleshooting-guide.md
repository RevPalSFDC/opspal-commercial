...
async function assignmentDiagnostic(modelId) {
  ...
  // User assignment stats - using valid SOQL (no COUNT(DISTINCT) or JOIN)
  ...
  // Account assignment stats - using valid SOQL
  ...
  `);
  // Assignment cause breakdown (valid SOQL with subquery)
  const causeCounts = await query(`
    SELECT AssociationCause, COUNT(Id) CountByCause
    FROM ObjectTerritory2Association
    WHERE Territory2Id IN (SELECT Id FROM Territory2 WHERE Territory2ModelId = '${modelId}')
    GROUP BY AssociationCause
  `);
  results.accountAssignments = causeCounts.map(c => ({
    ...c,
