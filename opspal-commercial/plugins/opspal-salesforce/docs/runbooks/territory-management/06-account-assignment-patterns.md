...
> Use separate queries with subqueries instead.

```bash
# Assigned accounts (unique via GROUP BY, then count)
sf data query --query "SELECT ObjectId FROM ObjectTerritory2Association WHERE Territory2Id IN (SELECT Id FROM Territory2 WHERE Territory2ModelId = '<model_id>') GROUP BY ObjectId"

# Total assignments
sf data query --query "SELECT COUNT(Id) FROM ObjectTerritory2Association WHERE Territory2Id IN (SELECT Id FROM Territory2 WHERE Territory2ModelId = '<model_id>')"

# Territories with accounts
sf data query --query "SELECT COUNT(Id) FROM Territory2 WHERE Territory2ModelId = '<model_id>' AND Id IN (SELECT Territory2Id FROM ObjectTerritory2Association)"
```
