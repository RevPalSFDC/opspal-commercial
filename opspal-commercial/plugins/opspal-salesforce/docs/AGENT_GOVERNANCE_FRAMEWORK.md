...
node scripts/lib/compliance-report-generator.js gdpr \
  --start-date 2025-01-01 \
  --end-date 2025-10-25 \
  --output ./reports/gdpr-2025-Q3.pdf

# Generate SOX compliance report
node scripts/lib/compliance-report-generator.js sox \
  --quarter Q3-2025 \
...
```bash
# Check current usage
node scripts/lib/api-usage-monitor.js status production

# Generate weekly report
node scripts/lib/api-usage-monitor.js report production --save

# Pre-operation quota check (before bulk operations)
node scripts/lib/api-usage-monitor.js check production 500
```
...
```bash
# Test API monitor
node scripts/lib/api-usage-monitor.js track production metadata deploy

# Test change ticket manager (requires JIRA_* env vars)
node scripts/lib/change-ticket-manager.js create-test-ticket

# Test enhanced PII detection
node scripts/lib/data-classification-framework.js test-value-sampling production Contact
```
