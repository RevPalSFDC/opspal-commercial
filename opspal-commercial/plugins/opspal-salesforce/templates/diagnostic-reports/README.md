...
- name: Run Flow Diagnostics
  run: |
    node scripts/run-diagnostics.js MyFlow production --format json
...
stage('Flow Diagnostic') {
    steps {
        sh 'node scripts/run-diagnostics.js MyFlow production --format json'
...
- **Diagnostic Modules**: `../../scripts/lib/flow-*.js`
