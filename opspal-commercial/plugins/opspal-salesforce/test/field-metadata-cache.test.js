...
const performanceTests = [
  test('Cache achieves >80% hit rate with realistic workload', async () => {
    ...
    // Prime cache
    await batchMeta.getMetadata(fields);
    batchMeta.resetStats();

    // Simulate 10 operations accessing random subsets of fields
    for (let i = 0; i < 10; i++) {
      const cachedSubset = fields.slice(0, 10);
      const newFields = [`Account.NewField${i}__c`, `Account.NewField${i}_b__c`];
      await batchMeta.getMetadata([...cachedSubset, ...newFields]);
    }
