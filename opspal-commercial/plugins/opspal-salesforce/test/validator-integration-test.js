 ...
 * and contact-merge-validator.js against delta-corp Sandbox.
 *
 * Usage: node test/validator-integration-test.js [delta-sandbox]
 */
...
const ORG_ALIAS = process.argv[2] || 'delta-sandbox';

// Valid org alias pattern - alphanumeric, underscores, and hyphens only
const VALID_ORG_ALIAS = /^[a-zA-Z0-9_-]+$/;

// Validate org alias at startup
if (!VALID_ORG_ALIAS.test(ORG_ALIAS)) {
  console.error(`Invalid org alias format: ${ORG_ALIAS}. Must be alphanumeric with underscores/hyphens only.`);
  process.exit(1);
}
