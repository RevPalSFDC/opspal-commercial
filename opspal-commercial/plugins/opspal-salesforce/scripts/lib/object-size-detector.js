 ...
 *
 * @example
 * const detector = new ObjectSizeDetector({ org: 'delta-production' });
...
class ObjectSizeDetector {
  ...
  _recommendStrategy(fieldCount, recordCount, estimatedMemoryMB) {
    ...
    // Proven: 85% reduction (554 → 81 fields) in delta-corp reflection
  ...
  _generateReasoning(fieldCount, recordCount, estimatedMemoryMB, recommendation) {
    ...
    switch (recommendation) {
      case BackupStrategy.INTELLIGENT:
        ...
        reasons.push(`Proven success: delta-corp Account backup reduced from 554 → 81 fields (85% reduction)`);
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    ...
    console.log('  node object-size-detector.js delta-production Account');
    console.log('  node object-size-detector.js delta-production Account Contact Opportunity\n');
    process.exit(1);
  }
