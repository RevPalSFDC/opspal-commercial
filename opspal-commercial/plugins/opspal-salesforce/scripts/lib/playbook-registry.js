...
class PlaybookRegistry {
    ...
    /**
     * Resolve the default documentation directory.
     * Supports both legacy docs/playbooks and current docs/runbooks layouts.
     */
    resolveDocsDirectory() {
        ...
        // Keep deterministic fallback when neither directory exists.
        return candidates[0];
    }

    /**
     * Recursively collect markdown documentation files.
     * Excludes README and inventory files that are not executable runbooks.
     */
    collectDocumentationFiles(rootDir) {
        const files = [];
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                files.push(...this.collectDocumentationFiles(fullPath));
                continue;
            }
            ...
            if (lowerName.includes('readme') || lowerName.includes('inventory')) {
                continue;
            }
    ...
    async buildRegistry() {
        ...
        const files = this.collectDocumentationFiles(this.playbooksDir);
        if (files.length === 0) {
            throw new Error(`No runbook/playbook markdown files found in ${this.playbooksDir}`);
        }

        for (const file of files) {
            const playbook = await this.parsePlaybook(file);
            if (playbook) {
                ...
                if (playbook.triggers) {
                    playbook.triggers.forEach(trigger => {
                        ...
                        if (!registry.triggers[triggerText]) {
                            registry.triggers[triggerText] = [];
                        }
                        registry.triggers[triggerText].push(playbook.name);
                    });
                }
...
`;
    ...
    async getPlaybookContent(name) {
        ...
        const targetPath = playbook.absolute_path || playbook.path;
