 ...
 * Salesforce API names must be alphanumeric with underscores
 */
const SAFE_PATTERNS = {
    apiName: /^[a-zA-Z][a-zA-Z0-9_]{0,39}$/,  // SF API name: 1-40 chars, start with letter
    orgAlias: /^[a-zA-Z0-9_-]{1,64}$/,        // Org alias: alphanumeric, dash, underscore
    eventList: /^[a-zA-Z_,]+$/                 // Comma-separated event names
};
...
`;
    ...
    async _generateHandlerClass(triggerName, object, events, handlerConfig) {
        ...
        validateInput(triggerName, 'apiName', 'triggerName');
        validateInput(object, 'apiName', 'object');
        const eventList = events.join(',');
        validateInput(eventList, 'eventList', 'events');

        try {
            // Use execFileSync with array arguments to prevent injection
            execFileSync('node', [
                generatorScript,
                'generate',
                '--name', triggerName,
                '--object', object,
                '--events', eventList,
                '--output', path.join(this.outputDir, 'classes')
            ], { stdio: 'inherit' });
        } catch (error) {
    ...
    async _deployTrigger(triggerName, triggersDir) {
        ...
        // Deploy using sf CLI - validate org alias first
        if (!this.orgAlias) {
            throw new Error('orgAlias is required for deployment');
        }
        validateInput(this.orgAlias, 'orgAlias', 'orgAlias');

        try {
            // Use execFileSync to prevent command injection
            execFileSync('sf', [
                'project', 'deploy', 'start',
                '--source-dir', path.dirname(triggerPath),
    ...
    async _runTests(testClasses) {
        ...
        // Validate inputs to prevent command injection
        if (!this.orgAlias) {
            throw new Error('orgAlias is required for test execution');
        }
        validateInput(this.orgAlias, 'orgAlias', 'orgAlias');

        // Validate each test class name
        for (const testClass of testClasses) {
            validateInput(testClass, 'apiName', 'testClassName');
        }

        try {
            // Run tests using sf CLI with execFileSync for safety
            const output = execFileSync('sf', [
                'apex', 'run', 'test',
                '--class-names', testClasses.join(','),
                '--result-format', 'json',
                '--target-org', this.orgAlias
            ], { encoding: 'utf-8' });
