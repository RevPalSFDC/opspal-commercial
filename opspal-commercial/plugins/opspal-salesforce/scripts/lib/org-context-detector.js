 ...
 * defaulted to eta-corp instead of inferring acme-production from session context.
 *
 * Detection Sources (in priority order):
 * 1. Explicit --target-org parameter (highest priority)
 * 2. Working directory path pattern (e.g., /instances/eta-corp/ → eta-corp)
 ...
 */
const ORG_ALIAS_PATTERN = /^[a-zA-Z0-9_-]{1,64}$/;
function validateOrgAlias(alias) {
    if (!alias || typeof alias !== 'string') return false;
    return ORG_ALIAS_PATTERN.test(alias);
}
...
async function validateOrg(orgAlias, verbose) {
  // Validate org alias to prevent command injection
  if (!validateOrgAlias(orgAlias)) {
    if (verbose) {
      console.error(`Invalid org alias format: ${orgAlias}`);
    }
    return { valid: false, error: 'Invalid org alias format' };
  }
