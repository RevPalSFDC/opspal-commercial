...
class DashboardMetadataDeployer {
    constructor(org, options = {}) {
        ...
        // Variation support
        this.variationResolver = new VariationResolver(this.org, {
            ...options,
            pluginRoot: this.pluginRoot
        });
        this.cpqDetector = new CPQDetector(this.org, options);
        this.cpqEnabled = false;
        this.variationInitialized = false;
    }

    /**
     * Initialize variation resolver (lazy initialization)
     */
    async initializeVariations() {
        if (this.variationInitialized) return;
        ...
        try {
            ...
            this.variationInitialized = true;
        } catch (error) {
            console.warn(`   ⚠️  Variation initialization skipped: ${error.message}`);
            this.variationInitialized = true; // Mark as initialized even on failure to avoid retries
        }
    }
    ...
    async deploy(template, options = {}) {
        ...
        try {
            ...
            // Apply variation if template supports variations
            let variationResult = null;
            if (template.variations || options.variation) {
                await this.initializeVariations();
                variationResult = await this.variationResolver.resolveVariation(
                    template.templateMetadata?.templateId || template.developerName,
            ...
            let personaValidation = null;
            try {
                personaValidation = validateDashboardPersonaContract(template, {
                    org: this.org,
                    source: 'dashboard-metadata-deployer'
                });
                if (personaValidation.status === 'warn') {
                    console.warn('  ⚠️  Persona KPI warnings detected for this dashboard');
                    personaValidation.issues.forEach(issue => {
                        console.warn(`     - ${issue.message}`);
                    });
                }
            } catch (personaError) {
                console.warn(`  ⚠️  Persona KPI validation skipped: ${personaError.message}`);
            }
