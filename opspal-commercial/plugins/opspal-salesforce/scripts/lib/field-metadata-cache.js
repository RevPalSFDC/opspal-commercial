 ...
 * Set FIELD_METADATA_LIVE_FIRST=false to use cache-first behavior.
 *
 * @version 1.1.0
...
// Set FIELD_METADATA_LIVE_FIRST=false to use cache-first behavior (not recommended)
const LIVE_FIRST = process.env.GLOBAL_LIVE_FIRST !== 'false' &&
                   process.env.FIELD_METADATA_LIVE_FIRST !== 'false';
...
class FieldMetadataCache {
   ...
   * In live-first mode, always returns false unless useCacheFirst option is set
   * @param {string} key - Cache key
   * @param {Object} options - Check options
   * @param {boolean} options.useCacheFirst - Force cache-first for this call
   * @returns {boolean}
   */
  has(key, options = {}) {
    // In live-first mode, pretend cache doesn't have it unless fallback mode
    if (this.liveFirst && !options.useCacheFirst) {
      return false;
    }
  ...
  /**
   * Check if key exists for fallback purposes (bypasses live-first check)
   * @param {string} key - Cache key
   * @returns {boolean}
   */
  hasFallback(key) {
    return this.has(key, { useCacheFirst: true });
  }
