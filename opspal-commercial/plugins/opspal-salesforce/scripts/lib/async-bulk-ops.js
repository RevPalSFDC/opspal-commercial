 ...
 * const jobId = await submitBulkUpdate('Contact', csvPath, 'delta-production');
 * console.log(`Job submitted: ${jobId}`);
 *
 * // Monitor job progress
 * const status = await monitorJob(jobId, 'delta-production', { poll: true });
 *
 * // Check job status without polling
 * const quickStatus = await getBulkJobStatus(jobId, 'delta-production');
 *
 * // Resume monitoring after interruption
 * await resumeJob(jobId, 'delta-production');
 */
...
Examples:
  node async-bulk-ops.js submit update Contact updates.csv delta-production
  node async-bulk-ops.js monitor 7501234567890ABCD delta-production
  node async-bulk-ops.js list delta-production active
        `);
        process.exit(0);
    }
