 ...
 * - Domain-aware matching (property-management, government, technology, financial)
 *
 * Usage:
 ...
 *
 *   // With domain-aware matching
 *   const matcher = new FuzzyMatcher({ domain: 'property-management' });
 *   const matches = matcher.match('ABC HOA', targets);
 *
 *   // With auto-detection
 *   const matcher = new FuzzyMatcher({ autoDetectDomain: true });
 *   const matches = matcher.match('FDIC insured bank', targets);
 */

// Try to load domain-aware matcher from opspal-core
let DomainAwareMatcher = null;
let DomainDictionaryLoader = null;
try {
    const path = require('path');
    const opspalCorePath = path.join(__dirname, '..', '..', '..', 'opspal-core', 'scripts', 'lib');
    DomainAwareMatcher = require(path.join(opspalCorePath, 'domain-aware-matcher')).DomainAwareMatcher;
    DomainDictionaryLoader = require(path.join(opspalCorePath, 'domain-dictionary-loader')).DomainDictionaryLoader;
} catch (error) {
    // Domain-aware matching not available - will use base functionality
}
...
class FuzzyMatcher {
    constructor(options = {}) {
        ...
        this._domainMatcher = null;
        this._domainAbbreviations = {};

        // Initialize domain-aware matching if configured
        if ((this.domain || this.autoDetectDomain) && DomainAwareMatcher) {
            this._initializeDomainMatcher();
        }
        ...
        // Merge domain-specific abbreviations if loaded
        if (Object.keys(this._domainAbbreviations).length > 0) {
            this.ABBREVIATIONS = { ...this.ABBREVIATIONS, ...this._domainAbbreviations };
        }
    }

    /**
     * Initialize domain-aware matcher
     * @private
     */
    _initializeDomainMatcher() {
        try {
            this._domainMatcher = new DomainAwareMatcher({
                domain: this.domain,
                autoDetect: this.autoDetectDomain,
                orgOverride: this.orgOverride
            });

            // Load domain abbreviations into this matcher as well
            if (this.domain && DomainDictionaryLoader) {
                const loader = new DomainDictionaryLoader();
                this._domainAbbreviations = loader.getAbbreviations(this.domain, {
                    orgOverride: this.orgOverride
                });
            }
        } catch (error) {
            console.warn(`Failed to initialize domain matcher: ${error.message}`);
            this._domainMatcher = null;
        }
    }
    ...
    setDomain(domain, orgOverride = null) {
        ...
        if (DomainAwareMatcher) {
            this._initializeDomainMatcher();
            // Re-merge abbreviations
            if (Object.keys(this._domainAbbreviations).length > 0) {
                this.ABBREVIATIONS = { ...this.ABBREVIATIONS, ...this._domainAbbreviations };
            }
        }
     ...
     * @returns {Object|null} Domain information or null if not using domain matching
     */
    getDomainInfo() {
        if (this._domainMatcher) {
            return this._domainMatcher.getDomainInfo();
        }
        return null;
    }
    ...
    listDomains() {
        if (this._domainMatcher) {
            return this._domainMatcher.listDomains();
        }
     ...
     * @param {string} options.domain - Override domain for this match (optional)
     * @param {boolean} options.useDomainMatcher - Force use of domain matcher (default: true when available)
     * @returns {Array} - Sorted array of matches with confidence scores
     */
    match(source, targets, options = {}) {
        // Use domain-aware matcher if available and not explicitly disabled
        const useDomainMatcher = options.useDomainMatcher !== false;
        if (useDomainMatcher && this._domainMatcher) {
            // If a domain override is provided, update the matcher
            if (options.domain && options.domain !== this.domain) {
                this._domainMatcher.setDomain(options.domain, options.orgOverride || this.orgOverride);
            }

            // Delegate to domain-aware matcher
            return this._domainMatcher.match(source, targets, {
                region: options.region,
                expectedStates: options.expectedStates,
