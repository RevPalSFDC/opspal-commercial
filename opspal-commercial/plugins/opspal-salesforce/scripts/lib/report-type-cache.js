 ...
 * In live-first mode, the cache is used only as fallback on API failure.
 * Set REPORT_TYPE_LIVE_FIRST=false to use cache-first behavior.
 */
...
// Set REPORT_TYPE_LIVE_FIRST=false to use cache-first behavior (not recommended)
const LIVE_FIRST = process.env.GLOBAL_LIVE_FIRST !== 'false' &&
                   process.env.REPORT_TYPE_LIVE_FIRST !== 'false';
...
class ReportTypeCache {
    ...
    /**
     * Get cached data as fallback (bypasses live-first check)
     ...
     * @param {boolean} options.useCacheFirst - Force cache-first for this call (for fallback)
     */
    async getReportTypes(options = {}) {
        return await this.get('report_types', options);
    }

    /**
     * Get cached report types as fallback (bypasses live-first check)
     */
    async getReportTypesFallback() {
        return await this.getReportTypes({ useCacheFirst: true });
    }
     ...
     * @param {boolean} options.useCacheFirst - Force cache-first for this call (for fallback)
     */
    async getFieldDescription(reportType, options = {}) {
        return await this.get(`fields_${reportType}`, options);
    }

    /**
     * Get cached field description as fallback (bypasses live-first check)
     */
    async getFieldDescriptionFallback(reportType) {
        return await this.getFieldDescription(reportType, { useCacheFirst: true });
    }
