...
check_prerequisites() {
    ...
    if [ ! -f "$REPO_ROOT/.claude-plugins/opspal-salesforce/scripts/qa/query-lint.js" ]; then
...
MODIFIED_SF_FILES=$(git diff --cached --name-only --diff-filter=ACM | grep -E '\.claude-plugins/opspal-salesforce/(scripts|agents|commands).*\.(js|sh|md)$' || true)
...
# Run query linter
QUERY_LINTER=".claude-plugins/opspal-salesforce/scripts/qa/query-lint.js"
...
if echo "$LINT_OUTPUT" | grep -q "Found [1-9][0-9]* potential violations"; then
  ...
  if [ -n "$RELEVANT_VIOLATIONS" ]; then
    ...
    echo ""
    echo "Documentation:"
    echo "  • .claude-plugins/opspal-salesforce/docs/SOQL_BEST_PRACTICES.md"
    echo "  • .claude-plugins/opspal-salesforce/docs/SALESFORCE_TOOLING_API_FLOW_OBJECTS.md"
    echo ""
...
print_summary() {
    ...
    echo "   node .claude-plugins/opspal-salesforce/scripts/qa/query-lint.js"
    echo ""
    echo "📚 Documentation:"
    echo "   .claude-plugins/opspal-salesforce/docs/SOQL_BEST_PRACTICES.md"
    echo "   .claude-plugins/opspal-salesforce/docs/SALESFORCE_TOOLING_API_FLOW_OBJECTS.md"
