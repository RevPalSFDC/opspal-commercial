...
QA-001: Supports DATA_INTEGRITY_STRICT=1 environment variable for strict mode.
When enabled, assessment fails explicitly on critical query failures.
"""
...
class DataIntegrityError(Exception):
    """Raised when strict mode is enabled and a critical query fails."""
...
class EvidenceBasedRevOpsAuditor:
    ...
    # Critical queries that MUST succeed for assessment validity
    ...
    def __init__(self, org_alias: str):
        ...
        # QA-001: Strict mode enforcement
        self.strict_mode = os.environ.get('DATA_INTEGRITY_STRICT', '0') == '1'
        self.critical_query_failures: List[Dict[str, Any]] = []
        self.assessment_status = 'pending'  # pending, complete, incomplete, failed

        if self.strict_mode:
            logger.info("🔒 DATA_INTEGRITY_STRICT mode enabled - assessment will fail on critical query errors")

    def _is_critical_query(self, query: str) -> bool:
        """Check if query is critical for assessment validity"""
        query_upper = query.upper()
        return any(pattern.upper() in query_upper for pattern in self.CRITICAL_QUERY_PATTERNS)
    ...
    def _execute_soql(self, query: str, description: str = "", paginate: bool = False) -> List[Dict]:
        ...
        QA-001: In strict mode, critical query failures raise DataIntegrityError.
        """
        self.query_count += 1
        ...
        try:
            ...
            # QA-001: Track critical failures and raise in strict mode
            is_critical = self._is_critical_query(query)
            failure_record = {
            ...
            self.critical_query_failures.append(failure_record)

            if self.strict_mode and is_critical:
                self.assessment_status = 'failed'
                raise DataIntegrityError(
                    f"Strict mode: Critical query failed - {description}",
                    query=query,
                    original_error=e
                )
    ...
    def generate_report(self, all_findings):
        ...
        QA-001: Includes assessment_status and data_integrity details.
        """
        # Determine final assessment status
        if self.assessment_status == 'pending':
            critical_failures = [f for f in self.critical_query_failures if f.get('is_critical')]
            if critical_failures:
                self.assessment_status = 'incomplete'
            elif self.failed_queries > 0:
                self.assessment_status = 'complete_with_warnings'
            else:
        ...
        report = {
            'metadata': {
                ...
                'data_integrity': {
                    'status': 'valid' if not self.critical_query_failures else 'degraded',
                    'critical_failures': [f for f in self.critical_query_failures if f.get('is_critical')],
                    'non_critical_failures': [f for f in self.critical_query_failures if not f.get('is_critical')],
                    'warning': 'Assessment data may be incomplete' if self.critical_query_failures else None
                }
            },
    ...
    def assess_lifecycle_governance(self) -> Dict[str, Any]:
        """QA-004: Assess org against lifecycle stage definitions and exit criteria"""
        logger.info("\n" + "="*60)
        logger.info("ANALYZING LIFECYCLE GOVERNANCE")
        logger.info("="*60)

        findings = {}

        # Try to load lifecycle governance via Node.js helper
        try:
            import subprocess
            script_path = os.path.join(
                os.path.dirname(__file__),
                '../lib/lifecycle-governance-loader.js'
            )

            if os.path.exists(script_path):
                result = subprocess.run(
                    ['node', script_path, 'governance', '--platform', 'salesforce'],
                    capture_output=True,
                    text=True,
                    timeout=30
                )
                ...
                if result.returncode == 0:
                    ...
                    findings = governance_data.get('lifecycle_governance', {})
                    logger.info(f"✅ Loaded lifecycle governance: {findings.get('stages_defined', 0)} stages defined")
                else:
                    logger.warning(f"⚠️ Lifecycle governance loader returned error: {result.stderr}")
            else:
                logger.warning(f"⚠️ Lifecycle governance loader not found at {script_path}")

        except subprocess.TimeoutExpired:
            logger.warning("⚠️ Lifecycle governance loader timed out")
        except Exception as e:
            logger.warning(f"⚠️ Error loading lifecycle governance: {e}")

        # Add stage analysis from actual org data
        stage_compliance = self._analyze_stage_compliance()
        if stage_compliance:
            findings['stage_compliance'] = stage_compliance

        return findings

    def _analyze_stage_compliance(self) -> Optional[Dict[str, Any]]:
        """Analyze org's compliance with standard funnel stages"""
        # Check for custom stage fields/picklists
        stage_fields = self._execute_soql("""
            SELECT QualifiedApiName, DataType, DeveloperName
            FROM FieldDefinition
            WHERE EntityDefinition.QualifiedApiName IN ('Lead', 'Opportunity')
            AND (DeveloperName LIKE '%Stage%' OR DeveloperName LIKE '%Status%')
            LIMIT 50
        """, "Stage/status field discovery")

        if not stage_fields:
            return None

        return {
            'custom_stage_fields': len(stage_fields),
            'fields_found': [
                {
                    'object': f.get('EntityDefinition', {}).get('QualifiedApiName', 'Unknown') if isinstance(f.get('EntityDefinition'), dict) else 'Unknown',
                    'field': f.get('QualifiedApiName', 'Unknown'),
                    'type': f.get('DataType', 'Unknown')
                }
                for f in stage_fields[:10]
            ]
        }
    ...
    def run_assessment(self):
        ...
        # QA-004: Add lifecycle governance assessment
        all_findings['lifecycle_governance'] = self.assess_lifecycle_governance()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run evidence-based RevOps assessment for a Salesforce org alias."
    )
    parser.add_argument(
        "org_alias",
        nargs="?",
        default=os.environ.get("SF_TARGET_ORG") or "gamma-corp",
        help="Salesforce org alias (defaults to $SF_TARGET_ORG or 'gamma-corp')"
    )
    parser.add_argument(
        "--output",
        dest="output_path",
...
def main():
    ...
    QA-001: Exits with non-zero code on critical failures in strict mode.
    """
    args = parse_args()
    ...
    try:
        ...
        logger.error(f"❌ CRITICAL: Assessment failed due to data integrity error")
        logger.error(f"   {e}")
        report = {
            'metadata': {
                'assessment_date': datetime.now(timezone.utc).isoformat(),
    ...
    output_path = args.output_path or f'revops_assessment_{org_alias}_{timestamp}.json'
    ...
    # QA-001: Exit with error code if assessment failed or incomplete in strict mode
    strict_mode = os.environ.get('DATA_INTEGRITY_STRICT', '0') == '1'
    status = report.get('metadata', {}).get('assessment_status', 'unknown')

    if strict_mode and status in ('failed', 'incomplete'):
        logger.error(f"❌ Assessment status: {status} - exiting with error")
        sys.exit(1)
