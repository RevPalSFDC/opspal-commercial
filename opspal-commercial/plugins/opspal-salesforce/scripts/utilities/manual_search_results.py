...
def is_delta-corp_related(text):
    """Check if text contains delta-corp-related terms"""
    if not text:
        return False
        
    text_lower = text.lower()
    search_terms = [
        'delta-corp', 'rental', 'rent', 'revpal', 'rev pal',
...
def main():
    print("🔍 Starting manual Asana search for delta-corp-related projects...\n")
    ...
    for workspace in workspaces_data['data']:
        ...
        for project in projects_data['data']:
            ...
            # Check if project is delta-corp-related
            combined_text = f"{project_name} {project_notes}"
            if is_delta-corp_related(combined_text):
        ...
        if workspace_results:
            ...
            print(f"  📝 No delta-corp-related projects found in {workspace_name}")
    
    # Display results
    print('\n' + '='*80)
    print('🎯 DELTA_CORP-RELATED PROJECTS SEARCH RESULTS')
    print('='*80)
    
    if not all_results:
        print('\n❌ No delta-corp-related projects found in any workspace.')
    else:
    ...
    if all_results:
        output_data = {
            ...
            'search_terms': ['delta-corp', 'rental', 'rent', 'revpal', 'property management'],
            'total_workspaces_searched': len([r for r in all_results]),
            'total_projects_found': total_projects,
            'results': all_results
        }
        
        with open('delta-corp-projects-search-results.json', 'w') as f:
            json.dump(output_data, f, indent=2)
        
        print(f'\n💾 Results saved to: delta-corp-projects-search-results.json')
    
    print(f'\n📊 SUMMARY: Found {total_projects} delta-corp-related projects across {len(all_results)} workspaces')
    print('='*80)
