...
argument-hint: "{action} {org} [params]"
arguments:
  - name: action
...
2. **Based on action selected**, present follow-up menus:
   ...
   **For "Prepare org":**
   ...
   - **Run pre-flight validation** (NEW - as of 2025-10-18):
     ```bash
     node scripts/lib/pre-flight-validator.js {org} Account --mode intelligent
     ```
   - If pre-flight BLOCKED → Show recommendations, require mode change
   - Execute: `node scripts/lib/sfdc-full-backup-generator.js`, validation, importance detection
   - **Use intelligent field selection for large objects** (>200 fields):
     ```bash
     node scripts/lib/metadata-backup-planner.js {org} Account --mode intelligent
     ```
   ...
   **For "Analyze pairs":**
   ...
   - Execute: `node scripts/lib/bulk-decision-generator.js`
   ...
   **For "Execute merges":**
   ...
   - Present execution options menu:
     ...
     ```
   - Execute: `node scripts/lib/bulk-merge-executor-parallel.js` with selected options
   ...
   **For "Recover from error":**
   ...
   - Present recovery procedure menu:
     ...
     ```
   - Execute: `node scripts/lib/dedup-rollback-system.js` with selected procedure
...
```bash
node scripts/lib/dedup-rollback-system.js --execution-log execution-logs/exec_*.json
```
