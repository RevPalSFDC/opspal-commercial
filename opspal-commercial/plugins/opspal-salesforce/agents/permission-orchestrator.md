...
### Mandatory Steps:
1. **RETRIEVE** the existing permission set XML from the org:
   ```bash
   sf project retrieve start --metadata PermissionSet:<name> --target-org <org>
   ```
2. **MERGE** your new fieldPermissions into the retrieved XML
3. **VERIFY** the merged XML contains ALL existing permissions plus new ones
4. **DEPLOY** the merged XML
5. **POST-DEPLOY**: Verify FLS with:
   ```sql
...
- You deploy XML with only 2 fieldPermissions
- Salesforce **removes** the other 50 fieldPermissions that existed
- Fields become invisible to users on that permission set
- `sf sobject describe` silently hides fields (no error, just empty)
- Tooling API still shows fields, creating false "ghost field" diagnosis
- **This has caused client-facing misdiagnosis** - treat as P0
...
node scripts/lib/permission-deployer.js deploy MyPermSet
...
# Verify assignments
node scripts/lib/verify-permission-assignments.js
...
```bash
node scripts/lib/permission-batch-deployer.js \
...
node scripts/lib/test-user-access.js --user test@example.com --permission-set Sales_Manager
```
...
```bash
# Interactive wizard
node scripts/lib/permission-wizard.js
...
# Batch operations
node scripts/lib/permission-batch-creator.js \
...
# Rename or merge
node scripts/lib/permission-merger.js --source Sales_Manager --target Sales_Manager_v2
```
...
node scripts/lib/permission-cleaner.js remove-object --permission-set Sales_Manager --object InvalidObject
```
...
node scripts/lib/permission-cleaner.js remove-field --permission-set Sales_Manager --field Account.InvalidField__c
```
...
node scripts/lib/permission-updater.js update-system-perm --permission-set Sales_Manager --old InvalidPerm --new ValidPerm
```
...
# Validate
node scripts/lib/permission-validator.js validate \
