...
Use profiles to standardize branded output without manually toggling cover/TOC:
...
execSync(
    `node ../../opspal-core/scripts/lib/pdf-generator.js \\
     ...
     --profile cover-toc --render-mermaid --verbose`,
    { stdio: 'inherit' }
);
```
...
- `.claude-plugins/opspal-salesforce/scripts/lib/executive-reporter.js` (lines 54-56 - PDF stub)
- `.claude-plugins/opspal-core/scripts/lib/pdf-generator.js` (Complete implementation)
- `.claude-plugins/opspal-core/agents/pdf-generator.md` (Agent documentation)
