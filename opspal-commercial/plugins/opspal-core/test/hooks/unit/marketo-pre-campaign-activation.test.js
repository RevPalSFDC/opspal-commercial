...
/**
 * Unit Tests for Marketo pre-campaign-activation.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/pre-campaign-activation.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo pre-campaign-activation.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_CAMPAIGN_VALIDATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__campaign_activate'
      }
    });
