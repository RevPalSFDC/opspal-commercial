...
/**
 * Unit Tests for pre-tool-execution.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/pre-tool-execution.sh');
...
async function runAllTests() {
  console.log('\n[Tests] pre-tool-execution.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH, 'sf data query --query "SELECT Id FROM Account"'], {
      encoding: 'utf8',
      env: { ...process.env, SKIP_TOOL_VALIDATION: '1' }
    });

    assert.strictEqual(result.status, 0, 'Should exit with 0');
  }));

  results.push(await runTest('Skips when no tool invocation provided', async () => {
