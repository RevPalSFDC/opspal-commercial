...
/**
 * Unit Tests for pre-operation-env-validator.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/pre-operation-env-validator.sh');
...
async function runAllTests() {
  console.log('\n[Tests] pre-operation-env-validator.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH, 'sf project deploy start', 'salesforce', 'test-org'], {
      encoding: 'utf8',
      env: { ...process.env, ENV_VALIDATION_ENABLED: '0' }
    });
