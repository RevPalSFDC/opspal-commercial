...
/**
 * Unit Tests for Marketo pre-orchestration.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/pre-orchestration.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo pre-orchestration.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_ORCHESTRATION_VALIDATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__program_clone'
      }
    });
