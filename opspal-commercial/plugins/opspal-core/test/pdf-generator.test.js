...
function isEnvironmentBlocked(error) {
    ...
    return error?.code === 'PDF_ENVIRONMENT_BLOCKED'
        || /operation not permitted|crashpad|sandbox|EPERM|ECONNRESET/i.test(message);
}
...
async function testSingleDocumentPDF() {
    ...
    try {
        const generatedPath = await generator.convertMarkdown(inputPath, outputPath, {
            renderMermaid: false
        });
        ...
        if (isEnvironmentBlocked(error)) {
            console.log(`⚠️ SKIP (env): ${error.message.split('\n')[0]}`);
            if (error.fallbackHtmlPath) {
                console.log(`   HTML fallback: ${error.fallbackHtmlPath}`);
            }
            return true;
        }
