...
const { NormalizationEngine } = require('./scripts/lib/normalization-engine');
const { PhoneCountryDetector } = require('./scripts/lib/phone-country-detector');
const { RegionDetector } = require('./scripts/lib/region-detector');
```

### International Data Normalization

The normalization pipeline supports **245 countries** via `libphonenumber-js`:

**Phone Normalization:**
- E.164 format conversion (+1 212-456-7890 → +12124567890)
- Intelligent country detection from number patterns
- Region-aware validation (NA, LATAM, EU, UK, APAC)
- Extension extraction

**Address Normalization:**
- Country-specific formats (US uppercase, EU lowercase)
- State/province code resolution (51 US states, 13 CA provinces, German Länder, etc.)
- Street type abbreviations per country (Street→ST, Straße→Str., Rue→R.)
- Postal code format validation per country

**Region Detection:**
- Weighted signal analysis for country detection
- Confidence scoring (0-1 scale)
- Multiple signals: phone prefix, postal code, state name, street keywords, character patterns

```javascript
// Example: Normalize with auto-detection
const engine = new NormalizationEngine({ defaultCountry: 'US' });

// Phone: auto-detect country from +44 prefix
const phone = engine.normalizePhone('+44 20 7946 0958');
// { normalized: '+442079460958', country: 'GB', confidence: 0.95 }

// Address: country-specific formatting
const address = engine.normalizeAddress({
...
// { normalized: '123 MAIN ST\nBOSTON, MA 02101', components: { state: 'MA', ... } }
```
