...
async function main() {
  ...
  // Read transcript (accepts path as command line argument or TRANSCRIPT_PATH env var)
  console.log('📄 Reading transcript...');
  const transcriptPath = process.argv[2] || process.env.TRANSCRIPT_PATH;
  if (!transcriptPath) {
    console.error('Usage: node test-slides-no-template.js <transcript-path>');
  ...
  const sourceContent = `
Discovery Call with zeta-corp - Key Points:

CLIENT: zeta-corp (Shopify-focused company)
...
`;
  ...
  const input = {
    deck_type: 'general',
    audience: 'executive',
    topic: 'RevPal Salesforce & Marketing Operations Proposal for zeta-corp',
    source_content: {
      type: 'text',
      data: sourceContent
    },
  ...
  console.log(`✓ Content: Sales proposal for zeta-corp`);
