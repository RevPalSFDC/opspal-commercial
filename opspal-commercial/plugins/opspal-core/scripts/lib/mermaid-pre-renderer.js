 ...
 * - Post-render verification to confirm files actually exist
 * - Strict mode option to fail if any diagrams don't render
 *
...
class MermaidPreRenderer {
    constructor(options = {}) {
        ...
        // Strict mode: fail if any diagrams don't render successfully
    ...
    async detectCapabilities() {
        ...
        this._capabilities = {
            mmdc: false,
            mmdcVersion: null,
            mmdcMajorVersion: null,
            puppeteer: false,
            placeholder: true, // Always available
            detectedAt: new Date().toISOString()
        };

        // Check for mmdc (Mermaid CLI) with version detection
        try {
            const { stdout } = await execAsync('mmdc --version');
            this._capabilities.mmdc = true;

            // Parse version (format: @mermaid-js/mermaid-cli v11.4.2)
            const versionMatch = stdout.match(/v?(\d+)\.(\d+)\.(\d+)/);
            if (versionMatch) {
                this._capabilities.mmdcVersion = `${versionMatch[1]}.${versionMatch[2]}.${versionMatch[3]}`;
                this._capabilities.mmdcMajorVersion = parseInt(versionMatch[1], 10);
            }

            if (this.verbose) {
                const version = this._capabilities.mmdcVersion || 'unknown';
                console.log(`    ✓ mmdc (Mermaid CLI) v${version} available`);
                if (this._capabilities.mmdcMajorVersion >= 11) {
                    console.log('      Note: v11+ requires explicit -o flag and stricter syntax');
                }
            }
        } catch {
            if (this.verbose) {
    ...
    async render(markdown, basePath) {
        try {
            ...
            // Post-render verification: confirm files actually exist
            const verificationSummary = {
                requested: mermaidBlocks.length,
            ...
            // Verify each claimed render by checking file existence
            for (const block of mermaidBlocks) {
                const hash = this._hash(block.code);
                const expectedPath = path.join(this.cacheDir, `${hash}.${this.outputFormat}`);
                try {
                    const stats = await fs.stat(expectedPath);
                    if (stats.size > 0) {
                        verificationSummary.verified.push(expectedPath);
                    } else {
                        verificationSummary.missing.push(expectedPath);
                    }
                } catch {
                    // File doesn't exist - check if placeholder was used
                    const placeholderPath = path.join(this.cacheDir, `${hash}.placeholder.md`);
                    try {
                        await fs.access(placeholderPath);
                        // Placeholder exists - this is expected for fallback strategy
                    } catch {
                        verificationSummary.missing.push(expectedPath);
                    }
                }
            ...
            if (this.verbose) {
                console.log(`  🔍 Verification: ${verificationSummary.verified.length}/${mermaidBlocks.length} files confirmed`);
                if (verificationSummary.missing.length > 0) {
                    console.log(`  ⚠️  Missing files: ${verificationSummary.missing.map(p => path.basename(p)).join(', ')}`);
                }
            }

            // Strict mode: fail if any diagrams are missing
            if (this.strictMode && verificationSummary.missing.length > 0) {
                throw new Error(`Strict mode: ${verificationSummary.missing.length} diagram(s) failed verification. Missing: ${verificationSummary.missing.map(p => path.basename(p)).join(', ')}`);
            }
...
`;
        ...
        if (renderResult && typeof renderResult === 'object' && renderResult.path) {
            ...
            // Convert to base64 data URI for reliable embedding
            const dataUri = this._pathToDataUri(imagePath);
...
`;
        ...
        const altText = title || 'Mermaid Diagram';
        const dataUri = this._pathToDataUri(renderResult);
        return `\n![${altText}](${dataUri})\n`;
    }

    /**
     * Convert file path to base64 data URI for reliable embedding
     * @private
     */
    _pathToDataUri(imagePath) {
        ...
        try {
            ...
            const absPath = typeof imagePath === 'string' ? imagePath : imagePath.toString();
            const buffer = fsSync.readFileSync(absPath);
            const base64 = buffer.toString('base64');
            const ext = path.extname(absPath).toLowerCase().slice(1) || 'png';
            const mimeType = ext === 'svg' ? 'image/svg+xml' : `image/${ext}`;
            return `data:${mimeType};base64,${base64}`;
        } catch (error) {
            // Fallback to original path if reading fails
            if (this.verbose) {
                console.log(`      Warning: Could not read image for base64 encoding: ${error.message}`);
            }
            return imagePath;
        }
    }
