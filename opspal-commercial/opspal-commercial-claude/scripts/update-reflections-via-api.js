...
/**
 * Update Reflection Statuses via Supabase REST API
 *
 * Uses native Node.js fetch (no external dependencies) to update reflections
 * to 'under_review' status with placeholder Asana task information.
 *
...
`;
  ...
  if (results.failures.length > 0) {
    ...
    for (const failure of results.failures) {
      ...
      md += `- **Rollback Status:** ${failure.rollback_status}\n`;
...
### 1. Create Asana Tasks Manually

Use the task information from \`asana-tasks-2025-10-22T21-40-00-000Z.json\` to create tasks in Asana.

### 2. Update Reflections with Real Task URLs

After creating Asana tasks, update the reflections with real task IDs and URLs:
...
SELECT cohort_id, fix_plan_id, COUNT(*) as reflection_count
FROM reflections
...
### Updates didn't persist (HTTP 200 but no change)

**Cause:** Anonymous key was used instead of service role key

**Solution:**
1. Verify \`SUPABASE_SERVICE_ROLE_KEY\` environment variable is set
2. Re-run script: \`node scripts/update-reflections-via-api.js\`
...
3. Increase verification wait time in script (currently 500ms)

### Rollback fails

**Cause:** Database permissions or constraint violations

**Solution:** Manually revert via SQL:
...
**Generated by:** \`scripts/update-reflections-via-api.js\`
**Timestamp:** ${timestamp}
**Service:** Supabase REST API with service role key
`;
