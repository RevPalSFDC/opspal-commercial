...
**When to use it**:
- ✅ After completing a HubSpot development session (workflow deployments, portal configurations, contact management)
...
### Example 1: After HubSpot Workflow Deployment with Errors

**Scenario**: You deployed a workflow to HubSpot and encountered 2 errors (property mapping issues, enrollment criteria conflicts)
...
🔍 Error Analysis:
   Issue #1: Property mapping error in workflow (config/env)
      Root cause: Custom property not found in target portal
      Priority: P1
      Blast radius: HIGH

   Issue #2: Enrollment criteria conflict (tool-contract)
      Root cause: Incompatible filter combinations in workflow triggers
      Priority: P0
      Blast radius: MEDIUM

💡 Playbook Generated:
   - Pre-deployment property validation
   - Workflow enrollment criteria testing
   - Portal-specific property verification
...
📈 Query your reflections:
   node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/query-reflections.js recent
```
...
💡 Manual submission (auto-loads .env/.env.local):
   node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/submit-reflection.js \
...
- **auth/permissions**: HubSpot auth issues, permission errors, API scope problems
...
- **external API drift**: HubSpot API changes, breaking updates, deprecated endpoints
...
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/query-reflections.js topIssues

# Statistics by portal
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/query-reflections.js orgStats

# Search by keyword
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/query-reflections.js search "workflow"

# All reflections for your portal
node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/query-reflections.js myOrg [portal-name]
```
...
**Cause**: Missing/invalid SUPABASE_URL/SUPABASE_ANON_KEY or network connectivity issue
...
2. **Verify plugin installation**:
   ...
   ```
   Expected: Should show hubspot-plugin as installed
...
**Cause**: Supabase connection issue or missing credentials
...
2. **Verify plugin version**:
   ...
   /plugin list | grep hubspot-plugin
   ```
   Expected: v1.3.3 or higher

3. **Reinstall plugin** if outdated:
   ```bash
   /plugin uninstall hubspot-plugin@revpal-internal-plugins
...
5. **Defer submission**: Reflection is saved locally, submit when network restores (auto-loads .env/.env.local):
   ```bash
   node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/submit-reflection.js \
     .claude/SESSION_REFLECTION_20251016_170235.json
   ```
...
- Works in both development (.claude-plugins/opspal-core-plugin/packages/domains/hubspot/) and marketplace installations (~/.claude/plugins/hubspot-plugin@revpal-internal-plugins/)
