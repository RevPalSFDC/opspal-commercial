...
# Extract publish data from environment or arguments
PAGE_ID="${PAGE_ID:-$1}"
PAGE_TYPE="${PAGE_TYPE:-landing-pages}"
PUBLISH_TYPE="${PUBLISH_TYPE:-immediate}"  # immediate | scheduled
...
if [[ -f "$SCRIPT_DIR/metrics-tracker.js" ]]; then
    node -e "
    const metricsTracker = require('$SCRIPT_DIR/metrics-tracker.js');
