...
class PDFGenerationHelper {
  ...
  static async generateSingleReportPDF(options) {
    ...
    try {
      ...
      // Generate PDF
      await generator.generate(markdownPath, pdfPath, {
        renderMermaid: true,
        coverPage: coverTemplate ? { template: coverTemplate } : undefined,
  ...
  static async preRenderMermaid(content, outputDir, verbose = false) {
    ...
    if (verbose) {
      console.log(`Found ${mermaidBlocks.length} Mermaid diagram(s) to pre-render`);
    }

    // Check if mmdc is available
    let mmdcAvailable = false;
    try {
      execSync('which mmdc 2>/dev/null || where mmdc 2>nul', { encoding: 'utf8' });
      mmdcAvailable = true;
    } catch {
      // Try npx
      try {
        ...
        if (verbose) console.warn('Mermaid CLI not available - skipping pre-rendering');
        return { content, renderedCount: 0 };
      }
    }
    ...
    for (const block of mermaidBlocks) {
      const inputFile = path.join(tempDir, `mermaid-${block.index}.mmd`);
      const outputFile = path.join(outputDir, `mermaid-${block.index}.png`);

      try {
        fs.writeFileSync(inputFile, block.code);

        const cmd = mmdcAvailable
          ? `mmdc -i "${inputFile}" -o "${outputFile}" -w ${PDF_CONFIG.mermaid.width} -H ${PDF_CONFIG.mermaid.height}`
          : `npx @mermaid-js/mermaid-cli -i "${inputFile}" -o "${outputFile}"`;

        execSync(cmd, { timeout: 30000, encoding: 'utf8' });

        renderedContent = renderedContent.replace(
          block.original,
          `![Diagram ${block.index + 1}](${outputFile})`
        );
        renderedCount++;

        if (verbose) console.log(`  ✓ Rendered diagram ${block.index + 1}`);
      } catch (e) {
        if (verbose) console.log(`  ✗ Failed diagram ${block.index + 1}: ${e.message}`);
      }
    ...
    return { content: renderedContent, renderedCount };
  }
