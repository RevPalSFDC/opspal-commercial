...
function resolveSalesforceModule(relativePath) {
    const domainPath = path.resolve(__dirname, '..', '..', 'salesforce', relativePath);
    const legacyPath = path.resolve(__dirname, '../../../../../..', 'salesforce-plugin', relativePath);
    const candidates = [domainPath, legacyPath];
...
const { RobustCSVParser } = require(resolveSalesforceModule('scripts/lib/csv-schema-validator'));
