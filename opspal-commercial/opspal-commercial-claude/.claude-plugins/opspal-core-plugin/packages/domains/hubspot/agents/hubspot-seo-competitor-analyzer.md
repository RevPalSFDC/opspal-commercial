...
if (input.competitorUrl) {
  ...
  for (const keyword of keywords) {
    ...
    // Use SERP analyzer
    const serpResult = await Bash(`node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/seo-serp-analyzer.js analyze "${keyword}" --no-patterns`);
...
if (config.includeContentGaps) {
  ...
  const gapAnalysis = await Bash(`node .claude-plugins/opspal-core-plugin/packages/domains/hubspot/scripts/lib/seo-content-gap-analyzer.js \\
