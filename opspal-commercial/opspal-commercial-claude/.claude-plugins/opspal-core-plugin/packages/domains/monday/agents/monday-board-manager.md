...
Or via GraphQL:
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/monday/scripts/lib/monday-api-client.js create-board \
  --name "Project X" \
  --kind public \
  --workspace 12345
```
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/monday/scripts/lib/monday-board-manager.js apply-template \
  --board <id> \
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/monday/scripts/lib/monday-api-client.js restore-board --id <board_id>
```
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/monday/scripts/lib/monday-api-client.js remove-subscriber \
  --board <board_id> \
  --user <user_id>
```
