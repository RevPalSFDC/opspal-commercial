...
```
User: /diagnose-report-access CampaignWithCampaignMembers --org peregrine-production
...
Quick Fix:
  sf org assign permset --name Standard_Lead_Read --target-org peregrine-production
```

### Example 2: Check Permission Set Coverage

```
User: /diagnose-report-access CampaignWithCampaignMembers --permission-set Campaign_Read_Only_Access --org peregrine-production
...
```
User: /diagnose-report-access CampaignWithCampaignMembers --org peregrine-production --fix
...
```
Task(subagent_type='sfdc-security-admin', prompt='Analyze and fix report access issue for CampaignWithCampaignMembers in peregrine-production')
```
