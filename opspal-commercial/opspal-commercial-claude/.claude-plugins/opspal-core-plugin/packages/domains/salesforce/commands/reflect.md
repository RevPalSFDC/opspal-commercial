...
(
  PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "$0")/../.." && pwd)}"
  if [[ -f "$PLUGIN_ROOT/.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/post-reflect-strategy-update.sh" ]]; then
    bash "$PLUGIN_ROOT/.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/post-reflect-strategy-update.sh" 2>/dev/null &
  fi
)
```
