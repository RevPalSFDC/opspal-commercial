...
async function queryReflections(org, options = {}) {
  const supabaseUrl = options.supabaseUrl || process.env.SUPABASE_URL || 'https://REDACTED_SUPABASE_PROJECT.supabase.co';
...
function printUsage() {
  ...
  console.log('  # Query recent reflections for rentable-sandbox');
  console.log('  node runbook-reflection-bridge.js --org rentable-sandbox');
  console.log('');
  console.log('  # Query last 100 reflections and save to file');
  console.log('  node runbook-reflection-bridge.js --org peregrine-main --limit 100 --output sections.json');
  console.log('');
  console.log('  # Query reflections from specific timeframe');
  console.log('  node runbook-reflection-bridge.js --org hivemq \\');
