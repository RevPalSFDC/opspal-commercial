...
if [[ -n "${CLAUDE_PLUGIN_ROOT:-}" ]]; then
    case "$CLAUDE_PLUGIN_ROOT" in
        *"/packages/domains/salesforce"|*/salesforce-plugin) LEGACY_ROOT="$CLAUDE_PLUGIN_ROOT" ;;
    esac
fi
if [ -z "$LEGACY_ROOT" ]; then
    if [ -d "$REPO_ROOT/.claude-plugins/salesforce-plugin" ]; then
        LEGACY_ROOT="$REPO_ROOT/.claude-plugins/salesforce-plugin"
    fi
fi

QUERY_LINTER="$DOMAIN_ROOT/scripts/qa/query-lint.js"
if [ ! -f "$QUERY_LINTER" ] && [ -n "$LEGACY_ROOT" ]; then
    QUERY_LINTER="$LEGACY_ROOT/scripts/qa/query-lint.js"
fi

DOCS_ROOT="$DOMAIN_ROOT/docs"
if [ ! -d "$DOCS_ROOT" ] && [ -n "$LEGACY_ROOT" ]; then
    DOCS_ROOT="$LEGACY_ROOT/docs"
fi

QUERY_LINTER_DISPLAY="$QUERY_LINTER"
if [[ "$QUERY_LINTER_DISPLAY" == "$REPO_ROOT/"* ]]; then
    QUERY_LINTER_DISPLAY="${QUERY_LINTER_DISPLAY#$REPO_ROOT/}"
fi

DOCS_ROOT_DISPLAY="$DOCS_ROOT"
if [[ "$DOCS_ROOT_DISPLAY" == "$REPO_ROOT/"* ]]; then
    DOCS_ROOT_DISPLAY="${DOCS_ROOT_DISPLAY#$REPO_ROOT/}"
fi
...
# Resolve Salesforce domain root (packages layout with legacy fallback)
DOMAIN_ROOT=""
if [ -d ".claude-plugins/opspal-core-plugin/packages/domains/salesforce" ]; then
  DOMAIN_ROOT=".claude-plugins/opspal-core-plugin/packages/domains/salesforce"
elif [ -d ".claude-plugins/salesforce-plugin" ]; then
  DOMAIN_ROOT=".claude-plugins/salesforce-plugin"
fi

# Check if any Salesforce plugin files were modified
MODIFIED_SF_FILES=$(git diff --cached --name-only --diff-filter=ACM | grep -E '(\.claude-plugins/opspal-core-plugin/packages/domains/salesforce|\.claude-plugins/salesforce-plugin)/(scripts|agents|commands).*\.(js|sh|md)$' || true)
...
if echo "$LINT_OUTPUT" | grep -q "Found [1-9][0-9]* potential violations"; then
  ...
  if [ -n "$RELEVANT_VIOLATIONS" ]; then
    ...
    echo ""
    echo "Documentation:"
    echo "  • $DOMAIN_ROOT/docs/SOQL_BEST_PRACTICES.md"
    echo "  • $DOMAIN_ROOT/docs/SALESFORCE_TOOLING_API_FLOW_OBJECTS.md"
    echo ""
...
print_summary() {
    ...
    echo "   node $QUERY_LINTER_DISPLAY"
    echo ""
    echo "📚 Documentation:"
    echo "   $DOCS_ROOT_DISPLAY/SOQL_BEST_PRACTICES.md"
    echo "   $DOCS_ROOT_DISPLAY/SALESFORCE_TOOLING_API_FLOW_OBJECTS.md"
    echo ""
