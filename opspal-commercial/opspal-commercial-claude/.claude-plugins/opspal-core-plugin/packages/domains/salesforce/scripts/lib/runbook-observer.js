...
function printUsage() {
  ...
  console.log('  --org <alias>          Salesforce org alias (e.g., rentable-sandbox)');
  ...
  console.log('  node runbook-observer.js --org rentable-sandbox --operation deployment \\');
  console.log('    --agent sfdc-orchestrator --objects "Account,Contact" --outcome success');
  console.log('');
  console.log('  # Workflow creation');
  console.log('  node runbook-observer.js --org peregrine-main --operation workflow-create \\');
  ...
  console.log('  node runbook-observer.js --org hivemq --operation field-audit \\');
...
function detectPluginRoot() {
  // This script is at: .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-observer.js
