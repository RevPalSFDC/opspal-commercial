...
class OrgHierarchySeeder {
    ...
    async updateUserManagersBatch(updates) {
        ...
        try {
            ...
            const tempFile = `/tmp/user-manager-batch-update-${Date.now()}.csv`;
    ...
    async updateUserManager(userId, managerId) {
        ...
        try {
            ...
            const tempFile = `/tmp/user-manager-update-${Date.now()}.csv`;
...
if (require.main === module) {
    ...
    if (args.length === 0) {
        ...
        console.error('\nExample:');
        console.error('  node org-hierarchy-seeder.js rentable-sandbox');
        console.error('  node org-hierarchy-seeder.js rentable-sandbox --create-test-records');
        process.exit(1);
    }
