...
async function getHealthMetrics(modelId) {
  ...
  // User coverage
  const userCoverage = await query(`
    SELECT
      COUNT(DISTINCT t.Id) TerritoriesWithUsers,
  ...
  `);
  metrics.userCoverage = userCoverage[0];

  // Account coverage
  const accountCoverage = await query(`
    SELECT
      COUNT(DISTINCT t.Id) TerritoriesWithAccounts,
    ...
    WHERE t.Territory2ModelId = '${modelId}'
  `);
  metrics.accountCoverage = accountCoverage[0];
