...
#
# Version: 1.2.0 (Unified Orchestrator Integration - Phase 2.1)
# Date: 2025-11-24
#
...
OUTPUT_FORMATTER="${SCRIPT_DIR}/../../../opspal-core/cross-platform-plugin/scripts/lib/output-formatter.js"
HOOK_LOGGER="${SCRIPT_DIR}/../../../opspal-core/cross-platform-plugin/scripts/lib/hook-logger.js"
...
ENV_CONFIG_VALIDATOR="${SCRIPT_DIR}/../../../opspal-core/cross-platform-plugin/scripts/lib/env-config-validator.js"
...
PRE_OP_ORCHESTRATOR="${SCRIPT_DIR}/../../../opspal-core/cross-platform-plugin/scripts/lib/pre-operation-validation-orchestrator.js"

if [ -f "$PRE_OP_ORCHESTRATOR" ]; then
...
if [ -f "$OUTPUT_FORMATTER" ]; then
    node "$OUTPUT_FORMATTER" success \
        ...
        "Target Org:$TARGET_ORG,Total Checks:$TOTAL_CHECKS,Passed:$PASSED_CHECKS,Failed:$FAILED_CHECKS" \
        "Proceed with deployment,Review deployment plan if this is production,Consider running additional org-specific tests" \
        "Org: $TARGET_ORG • Deployment: $DEPLOY_DIR"
    echo ""
    echo "════════════════════════════════════════════════════════════"
    exit 0
else
