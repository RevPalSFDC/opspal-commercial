...
const result = await governance.executeWithGovernance(
    {
        ...
        rollbackCommand: `node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/restore-from-backup.js ${orgAlias} ${backupFilePath}`,
...
**Load context:** `CONTEXT=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org [org-alias] --operation-type deduplication --format json)`
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/sfdc-pre-merge-validator.js {org-alias} Account

# 2. Generate full backup
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/sfdc-full-backup-generator.js {org-alias} Account

# 3. Detect importance fields
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/importance-field-detector.js {org-alias} Account
```
...
```json
{
  "org_alias": "bluerabbit2021-revpal",
  "industry": "PropTech",
  "guardrails": {
    "domain_mismatch": {
      "enabled": true,
...
const { BulkMergeExecutorParallel } = require('..claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/bulk-merge-executor-parallel');
...
async function analyzeDuplicatePairsBulk(orgAlias, duplicatePairs) {
  ...
  const importanceFields = await cache.get('importanceFields', async () => {
    const { ImportanceFieldDetector } = require('..claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/importance-field-detector');
...
3. **Sandbox Tests**: Real org data (bluerabbit2021-revpal)
