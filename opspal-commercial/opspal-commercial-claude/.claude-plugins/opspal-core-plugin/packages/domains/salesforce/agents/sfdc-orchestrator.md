...
**See Documentation**: `.claude-plugins/opspal-core-plugin/packages/domains/salesforce/docs/PLAN_MODE_INTEGRATION.md`
...
```javascript
const { ProgressMonitor } = require('..claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/progress-monitor');
...
node ../../../opspal-core/cross-platform-plugin/scripts/lib/progress-monitor.js status

# Run a progress check
node ../../../opspal-core/cross-platform-plugin/scripts/lib/progress-monitor.js check --verbose

# Capture snapshot
node ../../../opspal-core/cross-platform-plugin/scripts/lib/progress-monitor.js snapshot --json

# Reset after user provides input
node ../../../opspal-core/cross-platform-plugin/scripts/lib/progress-monitor.js reset
```
...
**Reference**: `cross-platform-plugin/scripts/lib/progress-monitor.js`
...
await Task({
    subagent_type: 'salesforce-plugin:sfdc-data-operations',
...
await Task({
    subagent_type: 'salesforce-plugin:sfdc-deployment-validator',
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/test-report-manager.js archive ./reports

# 2. Validate mode
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/qa-workflow-validator.js validate-mode execute [org-alias] ./reports
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/test-report-manager.js validate [report-file] --max-age-hours=24

# 2. Check for regressions vs previous
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/test-report-manager.js compare [old-report] [new-report]
...
- **Delegation example**:
  ```javascript
  await Task({
      subagent_type: 'salesforce-plugin:sfdc-revops-auditor',
...
- **Delegation example**:
  ```javascript
  await Task({
      subagent_type: 'salesforce-plugin:sfdc-cpq-assessor',
...
- **Delegation example**:
  ```javascript
  await Task({
      subagent_type: 'salesforce-plugin:sfdc-conflict-resolver',
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/agent-readiness-validator.js validate-agents \
  --agent-list [agents] \
  --operation-context [context] \
  --check-dependencies

# STEP 3: Run comprehensive pre-flight validation
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/preflight-validator.js validate orchestration \
  --operation-plan [plan-file] \
  --target-org [org-alias] \
  --validate-all-components

# STEP 4: Initialize validation monitoring
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/monitoring/orchestration-monitor.js start-validation-tracking \
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-architecture-validator.js --file [flow] --strict
...
1. **Enter validation-enhanced planning mode first**
   ...
   node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/orchestration-planner.js start-planning \
     --validation-enabled \
...
@import ../../../opspal-core/shared-docs/asana-integration-standards.md
