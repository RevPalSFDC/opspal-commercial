...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/report-type-analyzer.js validate CampaignWithCampaignMembers --user john.doe@company.com
```
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/report-type-analyzer.js generate-permission-set CampaignWithCampaignMembers --name "Campaign_Report_Access"
```
