...
# Install: ln -sf $(pwd)/.claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/hooks/pre-commit-standards-check.sh .git/hooks/pre-commit
...
if [ -n "$STAGED_COMMANDS" ]; then
    ...
    # Run validator
    RESULT=$(node "$REPO_ROOT/.claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/scripts/lib/command-frontmatter-validator.js" --json 2>/dev/null)
    ...
    if [ "$INVALID" -gt 0 ]; then
        ...
        echo "  node .claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/scripts/lib/command-frontmatter-validator.js"
        echo ""
