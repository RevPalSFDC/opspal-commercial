...
VALIDATOR_SCRIPT="$PLUGIN_ROOT/cross-platform-plugin/scripts/lib/quality-gate-validator.js"
...
if [ ! -f "$VALIDATOR_SCRIPT" ]; then
    ...
    if [ -f "$OUTPUT_FORMATTER" ]; then
      node "$OUTPUT_FORMATTER" error \
        ...
        "Ensure cross-platform-plugin is fully installed,Verify script path is correct,Check plugin directory structure" \
