...
This directory contains hooks for the cross-platform plugin that enhance Claude Code functionality.
...
```json
{
  "hooks": {
    "UserPromptSubmit": {
      "command": "bash ${CLAUDE_PLUGIN_ROOT}/.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/master-prompt-handler.sh"
    },
    "SessionStart": {
      "command": "bash -c '... && ${CLAUDE_PLUGIN_ROOT}/.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/hooks/session-context-loader.sh'"
    }
  }
}
```
