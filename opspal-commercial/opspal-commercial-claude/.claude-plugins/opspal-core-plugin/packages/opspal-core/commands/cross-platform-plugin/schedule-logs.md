...
node .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scheduler/scripts/lib/scheduler-manager.js history daily-cpq-check-a1b2c3d4
```
...
Logs are stored in:
```
.claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scheduler/logs/
...
2. **Check full log content**:
   ```bash
   cat .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scheduler/logs/<task-id>_*.log | tail -100
   ```

3. **Look for timeout issues**:
   ```bash
   grep "TIMED OUT" .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scheduler/logs/<task-id>_*.log
   ```
...
```bash
find .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scheduler/logs -name "*.log" -mtime +30 -delete
```
