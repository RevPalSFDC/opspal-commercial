...
class WiringVerifier {
  ...
  async testAgentWiring() {
    ...
    const agentRoots = [
      path.join(process.cwd(), '.claude-plugins', 'opspal-core-plugin', 'packages', 'domains', 'salesforce', 'agents'),
      path.join(process.cwd(), '.claude-plugins', 'salesforce-plugin', 'agents')
    ];
    ...
    const resolveAgentPath = (agentName) => {
      ...
      return path.join(agentRoots[0], `${agentName}.md`);
    };
  ...
  async testServiceAccessibility() {
    ...
    const services = [
      {
        name: 'report_service',
        path: '.claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/scripts/lib/report-service.js'
      },
      {
    ...
    const configs = [
      {
        ...
        path: '.claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/config/central_services.json',
        requiredFields: ['services', 'version']
      },
      {
        name: 'Routing Policy',
        path: '.claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/config/routing_policy.json',
  ...
  generateReport() {
    ...
    if (this.errors.length === 0) {
      ...
      console.log('  2. Monitor decision log: .claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/logs/routing_decisions.jsonl');
      console.log('  3. Check telemetry: node .claude-plugins/opspal-core-plugin/packages/opspal-core/developer-tools-plugin/scripts/lib/routing-telemetry-dashboard.js');
    } else {
