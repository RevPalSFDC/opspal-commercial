...
test_reflect_command_integration() {
  ...
  local reflect_md="$REPO_ROOT/.claude-plugins/opspal-core-plugin/packages/domains/salesforce/commands/reflect.md"
  if [ ! -f "$reflect_md" ]; then
    reflect_md="$REPO_ROOT/.claude-plugins/salesforce-plugin/commands/reflect.md"
  fi
