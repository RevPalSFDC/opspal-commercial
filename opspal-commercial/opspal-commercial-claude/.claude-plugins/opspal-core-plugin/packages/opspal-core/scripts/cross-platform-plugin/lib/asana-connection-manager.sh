...
if [ -z "$ASANA_ACCESS_TOKEN" ]; then
    ...
    echo "   Run: .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scripts/lib/asana-connection-manager.sh validate"
    exit 1
fi
...
if [[ ! "$ASANA_ACCESS_TOKEN" =~ ^2/ ]]; then
    ...
    echo "   Run: .claude-plugins/opspal-core-plugin/packages/opspal-core/cross-platform-plugin/scripts/lib/asana-connection-manager.sh fix"
    exit 1
fi
...
cd "$(dirname "$0")/../.."
./claude-plugins/cross-platform-plugin/scripts/lib/asana-connection-manager.sh health
