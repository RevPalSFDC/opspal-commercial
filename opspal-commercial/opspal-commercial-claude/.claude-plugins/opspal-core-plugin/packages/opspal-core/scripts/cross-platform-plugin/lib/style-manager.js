...
`;
   ...
   */
  static getLogoPaths() {
    const assetsDir = path.join(__dirname, '../../templates/assets/logos');
    return {
      main: path.join(assetsDir, 'revpal-logo.png'),      // Full logo
      icon: path.join(assetsDir, 'revpal-icon.png'),      // "R" icon
      favicon: path.join(assetsDir, 'revpal-favicon.png') // Favicon
    };
  }
...
if (require.main === module) {
  ...
  if (args.length < 1) {
    ...
    console.log('\nExamples:');
    console.log('  style-manager.js generate --theme revpal --output styles.css');
