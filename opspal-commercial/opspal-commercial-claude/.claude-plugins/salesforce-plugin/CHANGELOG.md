...
```bash
CONTEXT=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js load {org-alias})
```

**Post-Operation Updates**:
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js update {org-alias} \
...
**Sandbox Validation**:
- ✅ Org: bluerabbit2021-revpal (Wedgewood RevPal Sandbox)
...
**New File**: `cross-platform-plugin/scripts/lib/pre-operation-validation-orchestrator.js`
...
**Enhanced File**: `cross-platform-plugin/scripts/lib/cohort-fix-planner.js`
...
**New File**: `cross-platform-plugin/scripts/lib/capability-assessment-protocol.js`
...
**New SessionStart Hook**: `cross-platform-plugin/hooks/session-start-env-config.sh`
- Auto-generates ENV_CONFIG.json on session start

---

### Testing Results (Wedgewood RevPal Sandbox)
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-validator.js MyFlow.xml --auto-fix --dry-run  # Preview only
```
...
**Test Environment**: Wedgewood RevPal Sandbox (peregrine-staging instance)
...
- **Enhanced Completion Summary**:
  ...
  📁 Output Directory: ./q2c-audit-neonone-1762976064613/
...
- Complements existing `diagram-generator` agent (cross-platform-plugin) for general diagrams
...
- **OOO Write Operations Library** (`ooo-write-operations.js`, 476 lines) - Section A & D1
  ...
  - **TESTING**: ✅ Validated with Rentable Sandbox, all 7 steps successful
...
- **Integration Test Suite** (`test/ooo-integration.test.js`, 14 test cases)
  ...
  - **LIVE TESTING**: ✅ Tested with Rentable Sandbox, all tests passed
...
1. Start with introspection (read-only, safe): `node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/ooo-write-operations.js introspect`
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/automation-inventory-orchestrator.js <org> <output>
...
**Regression Context:**
- neonone org size: 218 triggers + 1,000 Apex classes
...
timeout 1800 node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/automation-audit-v2-orchestrator.js neonone <output-dir>
...
- **Metadata API flow retrieval NOW FULLY WORKING** - Final fix for FlowMetadataRetriever
  ...
  - **IMPACT**: Metadata API fallback now successfully retrieves and parses all active flows (validated: 82 flows from neonone)
  - **FILES MODIFIED**:
...
- RESULT: 100% success rate - 82 active flows retrieved and parsed from neonone

**Validation:**
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/flow-metadata-retriever.js neonone
...
- **Metadata API flow retrieval now working** - Critical fix for FlowMetadataRetriever
  ...
  - **IMPACT**: Metadata API fallback path now successfully retrieves and parses all flows (tested with 107 flows in neonone org)
...
- ✅ API version detection tested on Neonone org
...
- **Flow Query Error Extraction** - Fixed FlowDefinition fallback not triggering due to error message extraction bug
  ...
  - **Testing**: Confirmed 107 flows successfully queried from Neonone org using FlowDefinition fallback
...
- Tested on Neonone org (FlowDefinitionView unsupported) - fallback successful
...
- **Routing Toolkit** (`scripts/lib/routing-toolkit.js`): Practical tools for managing agent routing
  ...
  - CLI: `node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/routing-toolkit.js <command>` or `routing-toolkit.js all` for complete analysis
...
- **Phase 6 Test Suite**: Automated testing framework
  ...
  - Validated: 4 personas × 3 objects in PEREGRINE_SANDBOX
...
**Phase 6 Validation** (PEREGRINE_SANDBOX):
...
- ✅ Phase 6: Testing & Validation (PEREGRINE_SANDBOX tests)
...
/audit-reports rentable-sandbox --window-months 12

# Option 2: Via agent
/agents sfdc-reports-usage-auditor
# Then: "Audit all reports and dashboards for the last 6 months in org 'rentable-sandbox'"
...
# Output
instances/rentable-sandbox/reports-usage-audit-2025-10-18/
...
### Testing
- Tested across 3 Salesforce orgs: Peregrine Sandbox, Wedgewood Revpal, Rentable Sandbox
