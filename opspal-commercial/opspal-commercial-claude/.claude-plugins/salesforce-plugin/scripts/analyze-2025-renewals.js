 ...
 * Organization: Peregrine Production (peregrine-main)
 *
...
class RenewalRevOpsAnalyzer {
    constructor(orgAlias = 'peregrine-main') {
     ...
     */
    async init() {
        console.log('🚀 Initializing RevOps Analyzer for Peregrine Production...\n');
        await this.kit.init();
    ...
    generateReport() {
        ...
        report.push(`- **Instance**: Peregrine Production (peregrine-main)`);
        ...
        report.push('| Metric | Peregrine | Industry Average | Percentile |');
        report.push('|--------|-----------|------------------|------------|');
        ...
        report.push('*Data source: Peregrine Production Salesforce Instance*');
        report.push('*All monetary values in USD*');

        const fullReport = report.join('\n');

        // Save report to file
        const reportPath = path.join(process.cwd(), `Peregrine_2025_Renewals_Analysis_${new Date().toISOString().split('T')[0]}.md`);
...
(async () => {
    try {
        const analyzer = new RenewalRevOpsAnalyzer('peregrine-main');
        await analyzer.init();
