...
validate_org_mapping() {
    ...
    if [[ "$instance_name" == "sample-org-production" ]] && [[ "$current_alias" == "neonone" ]]; then
        error "CRITICAL: 'neonone' is incorrectly set as default for sample-org-production!"
        echo ""
        echo "This is a known misconfiguration. The correct alias should be 'sample-org-production'."
        has_error=1
    fi

    if [[ "$expected_username" == *"sample-org.com"* ]] && [[ "$current_username" == *"neonone"* ]]; then
        error "CRITICAL: Username domain mismatch - expecting sample-org.com but got neonone!"
        has_error=1
    fi
