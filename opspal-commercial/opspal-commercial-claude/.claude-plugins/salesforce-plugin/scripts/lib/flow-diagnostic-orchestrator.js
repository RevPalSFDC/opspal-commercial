...
class FlowDiagnosticOrchestrator {
  ...
  _generatePreflightReport(result) {
    ...
    return `/tmp/flow-preflight-${result.flowApiName}-${Date.now()}.html`;
  }
  ...
  _generateExecutionReport(result) {
    return `/tmp/flow-execution-${result.flowApiName}-${Date.now()}.html`;
  }
  ...
  _generateCoverageReport(result) {
    return `/tmp/flow-coverage-${result.flowApiName}-${Date.now()}.html`;
  }
}
