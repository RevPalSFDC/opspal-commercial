...
class ProcedureAFieldRestoration {
    ...
    executeSoqlQuery(query, useToolingApi = false, includeDeleted = false) {
        try {
            ...
            const cmd = `sf data query --query "${query.replace(/"/g, '\\"')}" --json --target-org ${this.orgAlias} ${apiFlag} ${deletedFlag}`;
    ...
    async queryDeletedRecord() {
        ...
        const query = `SELECT FIELDS(ALL) FROM Account WHERE IsDeleted = true AND MasterRecordId = '${this.survivorId}' ALL ROWS`;
...
Examples:
  ...
  node procedure-a-field-restoration.js bluerabbit2021-revpal 001xx000ABC --dry-run
