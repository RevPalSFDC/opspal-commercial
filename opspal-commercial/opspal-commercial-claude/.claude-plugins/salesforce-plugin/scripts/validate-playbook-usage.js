...
class PlaybookValidator {
    constructor(options = {}) {
        ...
        this.resolver = new PlaybookResolver(options);
    ...
    async validateAgentConfigurations() {
        ...
        for (const file of agentFiles) {
            ...
            try {
                ...
                if (agent.playbooks) {
                    ...
                    const invalid = [];
                    for (const [name, path] of Object.entries(agent.playbooks)) {
        ...
        console.log(`  Configured: ${results.configured} (${((results.configured/results.total)*100).toFixed(0)}%)`);
        ...
        const testCases = [
            {
                ...
                expectedPlaybooks: ['pre-deployment-validation', 'deployment-rollback'],
                expectedRequired: true
            },
            {
                task: 'Bulk import 10000 contacts',
                expectedPlaybooks: ['bulk-data-operations'],
                expectedRequired: false
            },
            {
                task: 'Create a new custom field',
                expectedPlaybooks: [],
                expectedRequired: false
            },
            {
                task: 'Fix deployment error and rollback changes',
                expectedPlaybooks: ['deployment-rollback', 'error-recovery'],
                expectedRequired: true
            },
            {
                task: 'Update dashboard components',
                expectedPlaybooks: ['dashboard-report-hygiene'],
                expectedRequired: false
            }
        ];
    ...
    async generateFullReport() {
        ...
        maxScore += 30;
        const configScore = (report.details.agentConfigurations.configured /
                           report.details.agentConfigurations.total) * 30;
        score += configScore;
        ...
        const coverageScore = (report.details.playbookCoverage.wellCovered.length / totalPlaybooks) * 20;
        score += coverageScore;

        // Resolution test score (20 points)
        maxScore += 20;
        const testScore = (report.details.resolutionTests.passed /
                          (report.details.resolutionTests.passed + report.details.resolutionTests.failed)) * 20;
        score += testScore;
