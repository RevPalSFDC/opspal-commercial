...
get_job_status() {
    ...
    # Execute API call
    local cmd="sf data query --query \"SELECT Id FROM Account LIMIT 1\" --target-org $ORG_ALIAS --json 2>/dev/null"
    
    # Use sf REST API call
    cmd="sf org display --target-org $ORG_ALIAS --json 2>/dev/null | python3 -c \"
import json
import sys
...
try:
    ...
    # Disable SSL verification for sandbox orgs
...
\""
    ...
    if [ -z "$response" ] || echo "$response" | grep -q '"error"'; then
        response='{"error": "Failed to get job status via REST API"}'
    fi
