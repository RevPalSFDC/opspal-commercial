...
**Submit Feedback**:
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/validator-telemetry.js feedback {{validator_name}} {{timestamp}} \
  --accurate={{yes|no}} \
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/submit-validator-feedback.js {{validator_name}} {{timestamp}}
```
