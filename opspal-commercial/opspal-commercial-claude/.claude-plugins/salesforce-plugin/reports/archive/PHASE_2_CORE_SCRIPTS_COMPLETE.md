...
```bash
# Preflight checks
node scripts/lib/flow-preflight-checker.js neonone run-all MyFlow

# Execute Flow
node scripts/lib/flow-executor.js neonone MyFlow '{"object":"Account","operation":"insert"}'

# Parse log
node scripts/lib/flow-log-parser.js neonone parse 07Lxx000000001

# Capture snapshot
node scripts/lib/flow-state-snapshot.js neonone capture 001xx000000XXXX

# Coverage analysis
node scripts/lib/flow-branch-analyzer.js neonone analyze MyFlow '[{"executionId":"exec_001"}]'

# Orchestration
node scripts/lib/flow-diagnostic-orchestrator.js neonone full MyFlow '{"testCases":[...]}'
```
