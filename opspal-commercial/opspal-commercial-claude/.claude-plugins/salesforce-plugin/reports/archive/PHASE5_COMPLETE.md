...
node bulk-merge-executor.js --org bluerabbit --decisions dedup-decisions.json --dry-run

# Custom batch size
node bulk-merge-executor.js --org bluerabbit --decisions dedup-decisions.json --batch-size 5

# Limit pairs (useful for testing)
node bulk-merge-executor.js --org bluerabbit --decisions dedup-decisions.json --max-pairs 10

# Auto-approve (skip confirmation)
node bulk-merge-executor.js --org bluerabbit --decisions dedup-decisions.json --auto-approve
```
...
/dedupe execute bluerabbit --batch-size 5 --max-pairs 10
...
# Terminal 1:
/dedupe execute bluerabbit --batch-size 5 --max-pairs 20
...
node scripts/lib/agent-dedup-helper.js --agent sfdc-merge-orchestrator --org bluerabbit --action test
...
1. **Sandbox** (bluerabbit2021-revpal) - Initial testing
...
1. **End-to-End Testing** (v3.1.0):
   - Execute real merges on Rentable sandbox (not dry-run)
...
**Ready for Testing**: ✅ YES (dry-run validated on Rentable sandbox)
