...
sf data query --query "SELECT Id, Title, DashboardType FROM Dashboard WHERE FolderName = 'Peregrine Dashboards'" --use-tooling-api
...
curl -X GET "https://peregrinemain--staging.sandbox.my.salesforce.com/services/data/v64.0/analytics/dashboards/describe" \
...
async function testDashboardComponent() {
    const api = new ReportsRestAPI('PEREGRINE_SANDBOX');
...
# Run test
ORG=PEREGRINE_SANDBOX node test-dashboard-component.js
```
...
# Deploy
sf project deploy start --metadata-dir metadata --target-org PEREGRINE_SANDBOX
```
...
curl -X GET "https://peregrinemain--staging.sandbox.my.salesforce.com/services/data/v64.0/" \
...
ORG=PEREGRINE_SANDBOX node test-template-variables.js
```
