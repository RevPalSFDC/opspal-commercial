...
**Date**: 2025-10-16
**Based On**: Cross-sandbox testing (Bluerabbit2021 + Rentable)
...
**Problem Discovered**: Rentable sandbox hit `spawnSync /bin/sh ENOBUFS` error when running importance-field-detector.js
...
**Problem Discovered**: 15-minute backup on Rentable had no progress visibility
...
node importance-field-detector.js Account rentable-sandbox

# Force refresh
node importance-field-detector.js Account rentable-sandbox --force-refresh

# Custom cache age
node importance-field-detector.js Account rentable-sandbox --cache-age 48
```
...
```bash
# Full backup
node sfdc-full-backup-generator.js Account rentable-sandbox

# Incremental (last 7 days)
node sfdc-full-backup-generator.js Account rentable-sandbox --incremental --since 7d

# Incremental (since specific date)
node sfdc-full-backup-generator.js Account rentable-sandbox --incremental --since 2025-10-01
```
...
node sfdc-full-backup-generator.js Account rentable-sandbox --parallel --concurrency 5

# Aggressive (10 concurrent)
node sfdc-full-backup-generator.js Account rentable-sandbox --parallel --concurrency 10
```
...
- **Example**: PropTech (Rentable) may have multi-location businesses
...
# Use template
node dedup-safety-engine.js analyze rentable-sandbox pairs.csv --template proptech
