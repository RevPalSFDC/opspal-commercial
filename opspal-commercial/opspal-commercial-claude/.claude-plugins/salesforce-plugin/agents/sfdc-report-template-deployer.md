...
**Load context:** `CONTEXT=$(node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/runbook-context-extractor.js --org [org-alias] --operation-type template_deployment --format json)`
...
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/report-format-validator.js --report ./template.json

# Deploy template with field resolution
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/deploy-report-template.js --template ./template.json --org <org>

# Build joined report from config
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/joined-report-builder.js --from-json ./config.json
```
...
2. **Intelligence Validation**: Run chart recommendations and quality validators before deployment
3. **Org Adaptation**: Handle different org configurations with graceful degradation
4. **Performance**: Deploy reports in 5-10 seconds with comprehensive validation
...
1. **Template**: Which template to deploy
   - Show available templates: `ls .claude-plugins/opspal-core-plugin/packages/domains/salesforce/templates/reports/*/*.json`
...
4. **Optional**: Folder name, report name override
...
```bash
node .claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/report-template-deployer.js \
...
📊 Report Details:
   ...
   URL: https://peregrine.my.salesforce.com/lightning/r/Report/00O.../view
...
const { CompositeApiExecutor } = require('..claude-plugins/opspal-core-plugin/packages/domains/salesforce/scripts/lib/batch-query-executor');
...
```bash
# List all templates
find .claude-plugins/opspal-core-plugin/packages/domains/salesforce/templates/reports -name "*.json" -exec basename {} \;

# Show template details
cat .claude-plugins/opspal-core-plugin/packages/domains/salesforce/templates/reports/sales-leaders/team-performance.json | jq '.templateMetadata'
```
...
```bash
# Find template
find .claude-plugins/opspal-core-plugin/packages/domains/salesforce/templates -name "*team-performance*"
```
