  ...
  mockExecSync = (command) => {
    ...
    if (command.includes('sf org display')) {
      return JSON.stringify({
        status: 0,
        result: { username: 'test@example.com', id: '00Dxx0000000001' }
      });
    }
...
asyncTest('FlowPreflightChecker: checkFlowMetadata finds Flow', async () => {
  ...
  assert.strictEqual(result.success, true);
  assert.strictEqual(result.flowMetadata.DeveloperName, TEST_FLOW_NAME);
});
...
asyncTest('FlowPreflightChecker: checkValidationRules returns rules', async () => {
  ...
  assert(Array.isArray(result.allRules));
  assert(Array.isArray(result.blockingRules));
});
