...
```json
{
  "hooks": {
    "UserPromptSubmit": {
      "command": "bash ${CLAUDE_PLUGIN_ROOT}/.claude-plugins/cross-platform-plugin/hooks/master-prompt-handler.sh"
    },
    "SessionStart": {
      "command": "bash -c '... && ${CLAUDE_PLUGIN_ROOT}/.claude-plugins/cross-platform-plugin/hooks/session-context-loader.sh'"
    }
  }
}
```
