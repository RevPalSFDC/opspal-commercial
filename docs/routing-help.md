...
| `compound-salesforce-cleanup-indicators` | fix.*account.*naming.*contact, account.*naming.*reparent | `opspal-salesforce:sfdc-orchestrator` | Compound account/contact cleanup workflows require orchestrated multi-step execution. |
| `mixed-salesforce-data-cleanup` | reparent.*contacts?.*duplicate.*account, duplicate.*account.*reparent.*contacts? | `opspal-salesforce:sfdc-orchestrator` | Mixed Salesforce cleanup workflows require orchestrated verification, CSV generation, bulk reparenting, and duplicate cleanup. |
| `salesforce-duplicate-discovery-cleanup` | find.*duplicate.*account.*query, find.*duplicate.*account.*validate | `opspal-salesforce:sfdc-orchestrator` | Duplicate account discovery workflows with embedded cleanup/validation steps require orchestrated multi-step execution. |
| `automation-destructive` | delete.*flow, remove.*flow | `opspal-salesforce:sfdc-automation-builder` | Destructive automation changes require a write-capable automation specialist with deployment clearance. |
