...
cp -r .claude-plugins/opspal-salesforce/scripts/lib/backups/ "$BACKUP_DIR/sfdc-backups" 2>/dev/null || echo "   (no lib backups)"
cp -r .claude-plugins/opspal-salesforce/backups/ "$BACKUP_DIR/sfdc-backups-root" 2>/dev/null || echo "   (no root backups)"

echo "   Backing up: salesforce-plugin instances..."
cp -r .claude-plugins/opspal-salesforce/instances/ "$BACKUP_DIR/instances" 2>/dev/null || echo "   (no instances)"
...
rm -rf .claude-plugins/opspal-salesforce/scripts/lib/backups/ 2>/dev/null && echo "   ✅ Removed scripts/lib/backups/" || echo "   (already removed)"
rm -rf .claude-plugins/opspal-salesforce/backups/ 2>/dev/null && echo "   ✅ Removed backups/" || echo "   (already removed)"
...
rm -rf .claude-plugins/opspal-salesforce/merge-temp/ 2>/dev/null && echo "   ✅ Removed merge-temp/" || echo "   (already removed)"
...
rm -rf .claude-plugins/opspal-salesforce/execution-logs/ 2>/dev/null && echo "   ✅ Removed sfdc execution-logs/" || echo "   (already removed)"
rm -rf .claude-plugins/opspal-salesforce/validation-reports/ 2>/dev/null && echo "   ✅ Removed sfdc validation-reports/" || echo "   (already removed)"
rm -rf .claude-plugins/opspal-salesforce/field-importance-reports/ 2>/dev/null && echo "   ✅ Removed sfdc field-importance-reports/" || echo "   (already removed)"

# 4. Instance-specific data
echo ""
echo "4️⃣  Removing instance-specific data..."
rm -rf .claude-plugins/opspal-salesforce/instances/ 2>/dev/null && echo "   ✅ Removed instances/" || echo "   (already removed)"

# 5. Development files
echo ""
echo "5️⃣  Removing development files..."
rm -f .claude-plugins/opspal-salesforce/DEV_REFLECTION_*.json 2>/dev/null && echo "   ✅ Removed DEV_REFLECTION_*.json" || echo "   (already removed)"
rm -f .claude-plugins/opspal-salesforce/scripts/lib/test-duplicate-pairs.* 2>/dev/null && echo "   ✅ Removed test-duplicate-pairs.*" || echo "   (already removed)"
