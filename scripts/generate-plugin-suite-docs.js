...
function buildReadmePluginOverview(inventory) {
  const lines = [
  ...
  for (const plugin of inventory.plugins) {
    lines.push(
      `| [\`${plugin.name}\`](${plugin.path}) | ${plugin.version} | ${plugin.status} | ${plugin.agents.length} | ${plugin.commands.length} | ${escapeTableCell(truncate(plugin.description, 110))} |`
    );
  }
...
function updateReadmeStats(inventory) {
  ...
  content = replaceSection(content, 'Plugin Overview', buildReadmePluginOverview(inventory));
