...
const EXTERNAL_DEVTOOLS_PLUGIN_ROOT = path.join(WORKSPACE_ROOT, 'dev-tools', 'developer-tools-plugin');
const LOCAL_DEVTOOLS_PLUGIN_ROOT = path.join(REPO_ROOT, 'dev-tools', 'developer-tools-plugin');
...
const DEVTOOLS_PLUGIN_ROOT = isPluginDir(EXTERNAL_DEVTOOLS_PLUGIN_ROOT)
  ? EXTERNAL_DEVTOOLS_PLUGIN_ROOT
  : LOCAL_DEVTOOLS_PLUGIN_ROOT;
...
function collectDevtoolsInventory() {
  ...
  const isExternalWorkspaceDevtools = !devtoolsManifest.startsWith(`${REPO_ROOT}${path.sep}`);

  if (!allowUntracked && !isExternalWorkspaceDevtools && !isGitTracked(manifestRelativePath)) {
    return null;
  }
...
// ---------------------------------------------------------------------------
// Backward-compat shim for the original commercial plugin-doc-inventory API.
// The legacy collectPluginInventory(repoRoot) accepted an explicit repoRoot
// and returned a shape with plugin.dirName + plugin.counts. Callers of the
// new collectInventory() API get plugin.name, .agents[], .commands[], etc.
// ---------------------------------------------------------------------------
...
function collectPluginInventory(repoRoot = REPO_ROOT) {
  ...
  const plugins = pluginDirs.map((dirName) => {
    ...
    const lifecycleRaw = (() => {
      try {
        return JSON.parse(fs.readFileSync(path.join(pluginRoot, '.claude-plugin', 'lifecycle.json'), 'utf8'));
      } catch (_) {
        return {};
      }
    })();
