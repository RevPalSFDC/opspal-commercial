...
import { bulkTools, executeBulkTool } from './src/tools/bulk.js';
import { listTools, executeListTool } from './src/tools/lists.js';
...
function createServer() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      } else if (name.includes('__bulk_') || name.includes('__activity_types_')) {
        result = await executeBulkTool(name, args || {});
      } else if (name.includes('__list_') || name.includes('__static_list_') || name.includes('__smart_list_')) {
        result = await executeListTool(name, args || {});
      } else {
