...
1. **Gather inputs**:
```bash
node scripts/lib/gtm-capacity-inputs.js \
  --org <org-alias> \
...
```bash
node scripts/lib/gtm-capacity-model.js base \
...
```bash
node scripts/lib/gtm-sensitivity.js \
  --inputs models/inputs.json \
  --variables all \
...
```bash
node scripts/lib/gtm-scenario-packager.js \
