...
```javascript
// Get SF metadata
const sfMetadata = await Task.invoke('opspal-salesforce:sfdc-field-analyzer', {
  prompt: `Analyze fields on ${sfObject} that need to sync to HubSpot`,
  org: orgAlias
});

// Get HS properties
const hsProperties = await Task.invoke('opspal-hubspot:hubspot-property-manager', {
...
```javascript
// Field discovery
await Task.invoke('opspal-salesforce:sfdc-field-analyzer', {
  prompt: 'Discover fields on Lead for HubSpot sync'
});

// Query patterns
await Task.invoke('opspal-salesforce:sfdc-query-specialist', {
  prompt: 'Build query for recently modified Accounts'
});

// Automation check
await Task.invoke('opspal-salesforce:sfdc-automation-auditor', {
...
```javascript
// Workflow check
await Task.invoke('opspal-hubspot:hubspot-workflow-builder', {
