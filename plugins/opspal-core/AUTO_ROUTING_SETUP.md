...
{
  "hooks": {
    "user-prompt-submit": ".claude-plugins/opspal-core/hooks/user-prompt-router.sh"
  }
}
```
...
# Test hook manually
echo "Deploy to production" | .claude-plugins/opspal-core/hooks/user-prompt-router.sh
```
...
2. Check file permissions: `ls -l .claude-plugins/opspal-core/hooks/user-prompt-router.sh`
...
ls -lh .claude-plugins/opspal-core/routing-index.json
...
```bash
# Review source code
cat .claude-plugins/opspal-core/hooks/user-prompt-router.sh

# Check for network calls (should be none)
grep -i "curl\|wget\|http" .claude-plugins/opspal-core/hooks/user-prompt-router.sh
```
...
**Plugin**: opspal-core@revpal-internal-plugins
