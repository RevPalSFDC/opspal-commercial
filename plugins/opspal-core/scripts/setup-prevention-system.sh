...
#
# Usage: bash .claude-plugins/opspal-core/scripts/setup-prevention-system.sh
#
...
jq --arg root "\${CLAUDE_PLUGIN_ROOT}" '. + {
  "hooks": {
    "UserPromptSubmit": {
      "command": "bash \($root)/.claude-plugins/opspal-core/hooks/master-prompt-handler.sh",
      "timeout": 10000,
    ...
    "SessionStart": {
      "command": "bash -c '"'"'\($root)/.claude-plugins/opspal-salesforce/hooks/session-start-agent-reminder.sh && \($root)/.claude-plugins/opspal-core/hooks/session-context-loader.sh'"'"'",
      "timeout": 5000,
...
if [ -f "$PROJECT_ROOT/.claude-plugins/opspal-core/hooks/master-prompt-handler.sh" ]; then
...
if [ -f "$PROJECT_ROOT/.claude-plugins/opspal-core/hooks/prevention-system-orchestrator.sh" ]; then
...
if [ -f "$PROJECT_ROOT/.claude-plugins/opspal-core/hooks/session-context-loader.sh" ]; then
