  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        if (!params.dryRun && report.pendingWrites && report.pendingWrites.length > 0) {
          report.requiresAgentAction = true;
          report.agentInstructions =
            'Pending CRM writes are in pendingWrites[]. For Salesforce: use mcp_salesforce_data_create for each record. ' +
            'For HubSpot: use mcp__hubspot-enhanced-v3__hubspot_create. Use Fireflies_Transcript_ID__c as external ID for dedup.';
        }
      ...
      try {
        ...
        if (actionTracker) {
          ...
          if (params.transcriptId) {
            const items = await actionTracker.extractActionItems(params.transcriptId);
            result = {
            ...
            const items = await actionTracker.extractBulkActionItems({
              fromDate: params.fromDate,
              toDate: params.toDate
            });
            result = {
              transcriptsScanned: items.length,
              actionItemCount: items.reduce((sum, t) => sum + (t.actionItems ? t.actionItems.length : 0), 0),
              actionItems: items
            };
          }
          return {
