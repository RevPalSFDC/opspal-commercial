...
function extractPatterns(reflections) {
  ...
  reflections.forEach(reflection => {
    ...
    const rawIssues = data.issues_identified || data.issues || [];
    const issues = Array.isArray(rawIssues) ? rawIssues : Object.values(rawIssues);
    const rawFeedback = data.user_feedback || [];
    const feedback = Array.isArray(rawFeedback) ? rawFeedback : Object.values(rawFeedback);
