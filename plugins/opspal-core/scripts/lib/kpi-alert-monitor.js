...
class KPIAlertMonitor {
    ...
    async _notifyEmail(alert) {
        ...
        const recipients = Array.isArray(config.recipients)
            ? config.recipients
            : String(config.recipients || process.env.KPI_ALERT_EMAIL_TO || '')
                .split(/[,\s;]+/)
        ...
        const subject = `[KPI Alert][${alert.severity.toUpperCase()}] ${alert.kpiId}`;
        const textBody = [
            `KPI Alert: ${alert.kpiId}`,
            `Severity: ${alert.severity}`,
            `Timestamp: ${alert.timestamp}`,
            '',
            `Current Value: ${alert.breach?.actual ?? 'N/A'}`,
            `Threshold: ${alert.breach?.threshold ?? 'N/A'}`,
            `Breach Type: ${alert.breach?.type ?? 'N/A'}`,
            '',
            `Alert ID: ${alert.id}`
        ].join('\n');

        const webhookUrl = config.webhookUrl || process.env.KPI_ALERT_EMAIL_WEBHOOK_URL;
        if (webhookUrl) {
            try {
                ...
                return { success: true, method: 'webhook', recipients: recipients.length };
            } catch (error) {
                return { success: false, method: 'webhook', error: error.message };
            }
        }

        if (config.sendmail !== false) {
            try {
                ...
                return { success: true, method: 'sendmail', recipients: recipients.length };
            } catch (error) {
                if (config.strictSendmail) {
                    return { success: false, method: 'sendmail', error: error.message };
                }
            }
        }

        const queueDir = config.queueDir || process.env.KPI_ALERT_EMAIL_QUEUE_DIR || path.join(process.cwd(), '.temp');
        fs.mkdirSync(queueDir, { recursive: true });
        const queuePath = path.join(queueDir, `kpi-alert-email-${Date.now()}.json`);
