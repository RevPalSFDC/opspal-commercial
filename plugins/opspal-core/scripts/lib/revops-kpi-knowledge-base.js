...
class RevOpsKPIKnowledgeBase {
  ...
  getBenchmarksBySegment(kpiId, segmentation = {}) {
    ...
    const stageBenchmarks = baseBenchmarks.byStage || kpi.benchmarks?.byStage || {};
    const acvBenchmarks = baseBenchmarks.byACV || kpi.benchmarks?.byACV || {};
    const gtmBenchmarks = baseBenchmarks.byGTM || kpi.benchmarks?.byGTM || {};
    ...
    if (stage && stageBenchmarks?.[stage]) {
      result.segmentedBenchmarks.stage = stageBenchmarks[stage];
      result.segmentContext.stage = segmentationDefs.companyStage?.[stage] || {};
    }

    // Get ACV-specific benchmarks
    if (acv && acvBenchmarks?.[acv]) {
      result.segmentedBenchmarks.acv = acvBenchmarks[acv];
      result.segmentContext.acv = segmentationDefs.acvTier?.[acv] || {};
    }

    // Get GTM-specific benchmarks
    if (gtm && gtmBenchmarks?.[gtm]) {
      result.segmentedBenchmarks.gtm = gtmBenchmarks[gtm];
