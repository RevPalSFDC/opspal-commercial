...
class WebsiteEnricher extends BaseEnricher {
     ...
     * @private
     */
    async _httpGet(url, redirectCount = 0) {
        if (redirectCount > 3) {
            return null;
        }
        ...
        return new Promise(resolve => {
            ...
            const request = client.get({
                ...
                headers: {
                    'User-Agent': 'Mozilla/5.0 (compatible; OpsPal-Enricher/1.0)'
                },
                timeout: 10000
            }, response => {
                ...
                if ([301, 302, 307, 308].includes(statusCode) && location) {
                    response.resume();
                    resolve(this._httpGet(new URL(location, parsedUrl).toString(), redirectCount + 1));
                    return;
                }
