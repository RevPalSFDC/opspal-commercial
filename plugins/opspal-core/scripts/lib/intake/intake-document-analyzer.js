...
`;
  ...
  async analyzeWithClaude(textContent, documentPath) {
    ...
    const tempPromptPath = `${os.tmpdir()}/intake-analysis-prompt-${Date.now()}.txt`;
    const tempResponsePath = `${os.tmpdir()}/intake-analysis-response-${Date.now()}.json`;
  ...
  try {
    ...
    if (openForm) {
      const formPath = outputPath ?
        ...
        `${os.tmpdir()}/intake-prefilled-${Date.now()}.html`;
      ...
      try {
        ...
        platformUtils.openBrowser(formPath);
      } catch (e) {
        try {
