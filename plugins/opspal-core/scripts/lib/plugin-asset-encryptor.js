...
//
// Bridge function called by deploy-to-commercial.js via normalizeCommercialEncryptor().
// Accepts the call signature from reencryptAsset() and returns a Buffer of ciphertext.

function encryptForCommercial({ plaintext, pluginName, asset, encryptedPath, destinationPath, commercialRoot, commercialSecurityRoot }) {
  const domain = PLUGIN_DOMAIN_MAP[pluginName];
  if (!domain) {
    throw new Error(
      `encryptForCommercial: unknown plugin "${pluginName}". ` +
      `Known plugins: ${Object.keys(PLUGIN_DOMAIN_MAP).join(', ')}`
    );
  }
  ...
  if (!keyMaterial) {
    throw new Error(
      `encryptForCommercial: no key material available for plugin "${pluginName}" (domain: ${domain}). ` +
      'Set OPSPAL_KEY_DOMAIN_* env vars or place key files in ~/.claude/opspal-enc/'
    );
  }
  ...
  const encBlob = engine.encryptAsset(plaintext, pluginName, assetPath, keyMaterial, {
    requiredDomain
  });
