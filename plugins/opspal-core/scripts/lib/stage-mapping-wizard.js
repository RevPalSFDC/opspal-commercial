...
class StageMappingWizard {
  ...
  /**
   * Discover HubSpot stages using HubSpot pipelines API
   */
  async discoverHubSpotStages(portalId) {
    ...
    if (!token) {
      console.warn('HubSpot stage discovery: HUBSPOT_PRIVATE_APP_TOKEN not configured; using fallback stages');
      return fallback;
    }
    ...
    try {
      ...
      const deals = await request('/crm/v3/pipelines/deals');
      const dealStages = [];

      for (const pipeline of deals.results || []) {
      ...
      const stages = {
        Deal: dealStages.length > 0 ? dealStages : fallback.Deal
      };

      // Tickets are optional but useful for lifecycle mapping in support-heavy portals.
      try {
        const tickets = await request('/crm/v3/pipelines/tickets');
        const ticketStages = [];
        for (const pipeline of tickets.results || []) {
          for (const stage of pipeline.stages || []) {
            ticketStages.push({
              apiName: stage.id,
              label: stage.label,
        ...
        if (ticketStages.length > 0) {
          stages.Ticket = ticketStages;
        }
      } catch (_) {
        // Ticket pipeline access may be unavailable in some portals.
      }

      if (this.verbose) {
        console.log(`✓ Discovered ${stages.Deal.length} HubSpot deal stages${stages.Ticket ? ` and ${stages.Ticket.length} ticket stages` : ''}`);
      }

      return stages;
    } catch (error) {
      console.warn(`HubSpot stage discovery failed (${error.message}); using fallback stages`);
      return fallback;
    }
  }
