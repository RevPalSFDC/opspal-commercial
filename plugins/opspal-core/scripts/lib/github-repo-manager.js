...
class GitHubRepoManager {
  ...
  /**
   * List all revpal-* repos from the GitHub org.
   * Parses customerId and customer slug from repo names.
   *
   * @returns {Promise<Array<{name: string, url: string, description: string, customerId: string|null, customerSlug: string|null}>>}
   */
  async listAllRevpalRepos() {
    this.log('Listing all revpal-* repositories');

    try {
      const listCmd = this.orgName
        ? `gh repo list ${this.orgName} --json name,url,description --limit 500`
        : 'gh repo list --json name,url,description --limit 500';

      const output = execSync(listCmd, { encoding: 'utf-8' });
      const repos = JSON.parse(output);

      return repos
        .filter(repo => repo.name.startsWith('revpal-'))
        .map(repo => ({
          name: repo.name,
          url: repo.url,
          description: repo.description || '',
          customerId: this._parseCustomerIdFromRepoName(repo.name),
          customerSlug: this._parseCustomerSlugFromRepoName(repo.name)
        }));
    } catch (error) {
   ...
   * Checks for existing clone and creates parent dirs as needed.
   *
   * @param {string} repoName - Repository name (e.g., revpal-rp-acm123456-acme-robotics)
   * @param {string} targetPath - Absolute path to clone destination
   * @returns {Promise<{cloned: boolean, path: string, alreadyExists?: boolean}>}
   */
  async cloneRepo(repoName, targetPath) {
    this.log('Cloning repository', { repoName, targetPath });

    if (fs.existsSync(targetPath) && fs.existsSync(path.join(targetPath, '.git'))) {
      this.log('Clone already exists at target path');
      return { cloned: false, path: targetPath, alreadyExists: true };
    }

    if (this.dryRun) {
      this.log('[DRY RUN] Would clone repository', { repoName, targetPath });
      return { cloned: false, path: targetPath, dryRun: true };
    }
    ...
    try {
      const fullName = this.orgName ? `${this.orgName}/${repoName}` : repoName;
      const cloneCmd = `gh repo clone ${fullName} "${targetPath}"`;
      this.log('Executing clone command', cloneCmd);
      execSync(cloneCmd, { encoding: 'utf-8', stdio: this.verbose ? 'inherit' : 'pipe' });

      this.log('Repository cloned', { repoName, targetPath });
      return { cloned: true, path: targetPath };
    } catch (error) {
  ...
  /**
   * Parse customer ID from repo name.
   * Expected format: revpal-rp-xxx123456-customer-slug
   *
   * @param {string} name
   * @returns {string|null}
   */
  _parseCustomerIdFromRepoName(name) {
    // Match RP-XXX123456 pattern (case-insensitive)
    const match = name.match(/revpal-(rp-[a-z0-9]+)/i);
    if (!match) return null;

    // The customerId is the portion after 'revpal-' up to the next '-' segment
    // that looks like a slug rather than part of the ID
    const afterPrefix = name.replace(/^revpal-/, '');
    const parts = afterPrefix.split('-');

    // Walk parts to find where the ID ends and slug begins
    // ID parts: 'rp', then alphanumeric codes
    // Slug parts: lowercase words
    if (parts.length < 2) return null;

    // The ID is always 'rp-XXXNNNNNN' format — first two segments
    // But the hash part can vary, so we look for the pattern
    const idMatch = afterPrefix.match(/^(rp-[a-z]{3}\d{6})/i);
    if (idMatch) {
      return idMatch[1].toUpperCase();
    }

    // Fallback: return first two segments as ID
    if (parts[0].toLowerCase() === 'rp' && parts.length >= 2) {
  ...
  /**
   * Parse customer slug from repo name.
   * Expected format: revpal-rp-xxx123456-customer-slug
   *
   * @param {string} name
   * @returns {string|null}
   */
  _parseCustomerSlugFromRepoName(name) {
    const afterPrefix = name.replace(/^revpal-/, '');

    // Try to match the known ID format and take everything after it
    const idMatch = afterPrefix.match(/^rp-[a-z]{3}\d{6}-(.*)/i);
    if (idMatch && idMatch[1]) {
      return idMatch[1];
    }

    // Fallback: skip first two segments (rp-XXXX) and join the rest
    const parts = afterPrefix.split('-');
    if (parts[0].toLowerCase() === 'rp' && parts.length >= 3) {
