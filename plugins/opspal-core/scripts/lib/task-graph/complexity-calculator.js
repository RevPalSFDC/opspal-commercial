...
class ComplexityCalculator {
  ...
  calculate(taskDescription, context = {}) {
    ...
    return {
      ...
      shouldDecompose: score >= this._getEffectiveThreshold(factors),
      confidence,
      flagOverride: false,
      details
    };
   ...
   */
  /**
   * Get effective decomposition threshold, lowered for cross-platform operations
   * @private
   */
  _getEffectiveThreshold(factors) {
    const crossPlatformOverride = this.rubric.cross_platform_threshold_override;
    if (crossPlatformOverride && factors.includes('multi_domain')) {
      return crossPlatformOverride;
    }
    return this.rubric.decomposition_threshold;
  }
