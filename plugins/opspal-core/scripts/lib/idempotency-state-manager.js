...
class IdempotencyStateManager {
  ...
  async acquireLock(key, options = {}) {
    ...
    try {
      // Build lock payload before attempting atomic create
      const lockData = {
        key,
      ...
      // Atomic exclusive create — 'wx' fails with EEXIST if file already exists,
      // eliminating the TOCTOU race between existsSync and writeFileSync.
      try {
        const fd = fs.openSync(lockFile, 'wx'); // exclusive create — atomic
        fs.writeSync(fd, JSON.stringify(lockData, null, 2));
        fs.closeSync(fd);
      } catch (e) {
        if (e.code === 'EEXIST') {
          // Lock file exists — check whether it is stale
          let existingLockData;
          try {
            existingLockData = JSON.parse(fs.readFileSync(lockFile, 'utf8'));
          } catch (_) {
            // File unreadable/corrupt — treat as stale and fall through to retry
            existingLockData = null;
          }

          if (existingLockData) {
            const lockAge = Date.now() - new Date(existingLockData.acquiredAt).getTime();

            // Non-stale lock held by another process — deny acquisition
            ...
            // Lock is stale — overwrite it (best-effort; a concurrent writer may
            // win the next openSync, which is acceptable)
            this.log(`Stale lock found, overwriting: ${key.substring(0, 16)}...`);
          }

          // Overwrite stale/corrupt lock file
          fs.writeFileSync(lockFile, JSON.stringify(lockData, null, 2));
        } else {
          // Unexpected filesystem error — propagate to outer catch
          throw e;
        }
      }
