...
class DedupConfigLoader {
    ...
    static _validate(config) {
        ...
        // NEW: Validate criticalCustomFields configuration
        if (config.criticalCustomFields) {
            const validStrategies = [
                'highest-value', 'most-recent', 'from-decision',
                'favor-master', 'favor-duplicate', 'longest-string', 'non-null'
            ];

            Object.entries(config.criticalCustomFields).forEach(([fieldName, fieldConfig]) => {
                if (typeof fieldConfig !== 'object') {
                    errors.push(`criticalCustomFields.${fieldName} must be an object`);
                    return;
                }

                if (fieldConfig.weight !== undefined && typeof fieldConfig.weight !== 'number') {
                    errors.push(`criticalCustomFields.${fieldName}.weight must be a number`);
                }

                if (fieldConfig.weight !== undefined && (fieldConfig.weight < 0 || fieldConfig.weight > 50)) {
                    errors.push(`criticalCustomFields.${fieldName}.weight must be between 0 and 50`);
                }

                if (fieldConfig.conflictStrategy && !validStrategies.includes(fieldConfig.conflictStrategy)) {
                    errors.push(`criticalCustomFields.${fieldName}.conflictStrategy must be one of: ${validStrategies.join(', ')}`);
                }
            });
        }
    ...
    static _mergeDefaults(config) {
        const defaults = {
            ...
            canonicalWeights: {
                ...
                syncHealth: 50,
                customFieldCompleteness: 50,  // NEW: Custom field scoring weight
                numContacts: 40,
                numDeals: 25,
                ownerPresent: 10,
                createdateOldest: 5
            },
            // NEW: Critical custom fields for completeness scoring and conflict resolution
