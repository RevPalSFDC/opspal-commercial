 ...
 * Uses three clustering strategies: SF Account ID, normalized domain, and name-based matching.
 *
 * Features:
 * - Bundle A: Companies grouped by salesforceaccountid
 * - Bundle B: Companies grouped by normalized domain
 * - Bundle C: Companies grouped by similar names (domain-aware fuzzy matching)
 * - Domain normalization (lowercase, no protocol/www)
 * - Domain-aware abbreviation expansion (PD → Police Department, HOA → Homeowners Association)
...
function getNormalizationEngine() {
    if (!NormalizationEngine) {
        try {
            // From deduplication/ go up one level to scripts/lib/
            NormalizationEngine = require(path.join(__dirname, '..', 'normalization-engine')).NormalizationEngine;
        } catch (error) {
            return null;
        }
    }
...
function getFuzzyMatcher() {
    if (!FuzzyMatcher) {
        try {
            // From deduplication/ go up to salesforce-plugin/scripts/lib/
...
function getDomainDetector() {
    if (!DomainDetector) {
        try {
            // From deduplication/ go up one level to scripts/lib/
            DomainDetector = require(path.join(__dirname, '..', 'domain-detector')).DomainDetector;
        } catch (error) {
            return null;
        }
    }
