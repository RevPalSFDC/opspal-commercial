...
// Import dependent modules (optional - graceful fallback)
let FieldPolicyManager;
try { FieldPolicyManager = require('./field-policy-manager'); } catch (e) { FieldPolicyManager = null; }

let TaskVariantLoader;
try { TaskVariantLoader = require('./task-variant-loader'); } catch (e) { TaskVariantLoader = null; }
...
class RunbookPolicyRetriever {
  ...
  async retrieve(request) {
    ...
    try {
      // Initialize managers for the org (null-safe if modules unavailable)
      const policyManager = FieldPolicyManager
        ? new FieldPolicyManager(request.org, { verbose: this.verbose, pluginRoot: this.pluginRoot, instancesRoot: this.instancesRoot })
        : null;
      const variantLoader = TaskVariantLoader
        ? new TaskVariantLoader(request.org, { verbose: this.verbose, pluginRoot: this.pluginRoot, instancesRoot: this.instancesRoot })
        : null;

      // Get field policy
      const fieldPolicy = policyManager
        ? await policyManager.getPolicy(request.object)
        : { required_fields: { fields: [] }, default_fields: { fields: [] }, recommended_fields: { fields: [] }, sensitive_field_exclusions: {}, field_aliases: {}, maxFields: 100, globalDefaults: {} };

      // Get task variant
      const taskVariant = variantLoader
        ? await variantLoader.getVariant(request.task_type)
        : null;
