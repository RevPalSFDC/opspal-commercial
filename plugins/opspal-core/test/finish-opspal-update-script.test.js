...
async function runAllTests() {
  ...
  results.push(await runTest('Resumes the persisted update session and emits a JSON finish report', async () => {
    ...
    try {
      ...
      assert.strictEqual(report.summary.stepCount, 13, `Expected thirteen recorded finish steps (step0–step12 including runtime housekeeping). report=${result.stdout}`);
