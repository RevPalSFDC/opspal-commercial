...
/**
 * Unit Tests for Marketo pre-bulk-operation.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/pre-bulk-operation.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo pre-bulk-operation.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_BULK_VALIDATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__lead_create'
      }
    });
