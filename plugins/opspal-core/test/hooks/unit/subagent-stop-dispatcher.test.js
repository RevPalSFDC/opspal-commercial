...
const HOOK_PATH = 'plugins/opspal-core/hooks/subagent-stop-dispatcher.sh';
...
async function runAllTests() {
  console.log('\n[Tests] subagent-stop-dispatcher.sh\n');
  ...
  results.push(await runTest('Empty input returns {}', async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'subagent-stop-dispatcher-'));
    try {
