...
/**
 * Unit Tests for Marketo post-operation-verification.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/post-operation-verification.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo post-operation-verification.sh Tests\n');
  ...
  results.push(await runTest('Skips when verification disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_POST_VERIFICATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__lead_update'
      }
    });
