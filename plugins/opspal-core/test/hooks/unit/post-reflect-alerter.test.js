...
/**
 * Unit Tests for post-reflect-alerter.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/post-reflect-alerter.sh');
...
async function runAllTests() {
  console.log('\n[Tests] post-reflect-alerter.sh Tests\n');
  ...
  results.push(await runTest('Skips when alerting disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      encoding: 'utf8',
      env: { ...process.env, POST_REFLECT_ALERT_DISABLED: '1' }
    });
