...
/**
 * Unit Tests for session-capture-init.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/session-capture-init.sh');
...
async function runAllTests() {
  console.log('\n[Tests] session-capture-init.sh Tests\n');
  ...
  results.push(await runTest('Skips when session capture disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      encoding: 'utf8',
      env: { ...process.env, DISABLE_SESSION_CAPTURE: '1' }
    });
