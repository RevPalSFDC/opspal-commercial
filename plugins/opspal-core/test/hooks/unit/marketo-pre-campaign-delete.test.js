...
/**
 * Unit Tests for Marketo pre-campaign-delete.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/pre-campaign-delete.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo pre-campaign-delete.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_DELETE_VALIDATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__campaign_delete'
      }
    });
