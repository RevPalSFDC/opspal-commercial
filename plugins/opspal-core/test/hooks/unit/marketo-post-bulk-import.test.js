...
/**
 * Unit Tests for Marketo post-bulk-import.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/post-bulk-import.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo post-bulk-import.sh Tests\n');
  ...
  results.push(await runTest('Skips when verification disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_IMPORT_VERIFICATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__bulk_import'
      }
    });
