...
/**
 * Unit Tests for pre-operation-snapshot.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/pre-operation-snapshot.sh');
...
async function runAllTests() {
  console.log('\n[Tests] pre-operation-snapshot.sh Tests\n');
  ...
  results.push(await runTest('Skips when error recovery disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      encoding: 'utf8',
      env: { ...process.env, ERROR_RECOVERY_ENABLED: '0' }
    });
