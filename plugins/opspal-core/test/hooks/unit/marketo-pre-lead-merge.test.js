...
/**
 * Unit Tests for Marketo pre-lead-merge.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/pre-lead-merge.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo pre-lead-merge.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_MERGE_VALIDATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__lead_merge'
      }
    });
