...
/**
 * Unit Tests for pre-dependency-check.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/pre-dependency-check.sh');
...
async function runAllTests() {
  console.log('\n[Tests] pre-dependency-check.sh Tests\n');
  ...
  results.push(await runTest('Skips when dependency check disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH, 'sf_data_query', '{}'], {
      encoding: 'utf8',
