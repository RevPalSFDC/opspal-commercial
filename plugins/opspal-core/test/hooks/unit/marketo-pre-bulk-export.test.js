...
/**
 * Unit Tests for Marketo pre-bulk-export.sh
 *
...
const HOOK_PATH = path.join(
  PROJECT_ROOT,
  'plugins/opspal-marketo/hooks/pre-bulk-export.sh'
);
...
async function runAllTests() {
  console.log('\n[Tests] Marketo pre-bulk-export.sh Tests\n');
  ...
  results.push(await runTest('Skips when validation disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        MARKETO_EXPORT_VALIDATION: '0',
        CLAUDE_TOOL_NAME: 'mcp__marketo__bulk_lead_export_create'
      }
    });
