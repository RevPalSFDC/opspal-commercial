...
/**
 * Unit Tests for post-task-stall-check.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/post-task-stall-check.sh');
...
async function runAllTests() {
  console.log('\n[Tests] post-task-stall-check.sh Tests\n');
  ...
  results.push(await runTest('Skips when stall detection disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      encoding: 'utf8',
      env: { ...process.env, ENABLE_STALL_DETECTION: '0' }
    });
