...
/**
 * Unit Tests for post-todowrite-scratchpad.sh
 *
...
const HOOK_PATH = path.join(PROJECT_ROOT, 'plugins/opspal-core/hooks/post-todowrite-scratchpad.sh');
...
async function runAllTests() {
  console.log('\n[Tests] post-todowrite-scratchpad.sh Tests\n');
  ...
  results.push(await runTest('Skips when scratchpad disabled', async () => {
    const result = spawnSync('bash', [HOOK_PATH], {
      encoding: 'utf8',
      env: { ...process.env, ENABLE_SCRATCHPAD: '0' }
    });
