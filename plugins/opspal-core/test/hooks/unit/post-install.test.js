...
async function runAllTests() {
  ...
  results.push(await runTest('opspal-core post-install invokes hook reconciliation helper', async () => {
    const coreHook = PLUGIN_POST_INSTALLS.find((plugin) => plugin.name === 'opspal-core');
    assert(coreHook, 'opspal-core post-install hook should be configured');
    const content = fs.readFileSync(coreHook.hookPath, 'utf8');
    assert(
      content.includes('reconcile-hook-registration.js'),
      'opspal-core post-install hook should invoke the hook reconciliation helper'
    );
  }));
