...
# Post-Gong Sync Hook
#
# Purpose: Post-processing after Gong sync/analysis operations.
#   1. Updates daily API counter
#   2. Triggers work-index auto-capture if ORG_SLUG is set
#   3. Logs sync metadata
#
# Triggers: PostToolUse for mcp__gong* matcher (all Gong API calls)
#
# Configuration: Set GONG_POST_SYNC_ENABLED=0 to disable
###############################################################################

# Check if enabled
if [ "${GONG_POST_SYNC_ENABLED:-1}" = "0" ]; then
  exit 0
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="${HOME}/.claude/logs"
LOG_FILE="${LOG_DIR}/gong-sync.jsonl"
