...
resolve_variables() {
    local value="$1"
    # Derive REPO_ROOT from PLUGIN_ROOT (plugins/opspal-core -> repo root)
    local repo_root
...
main() {
    ...
    # Source environment files (needed when running under cron)
    local repo_root
    repo_root="$(cd "${PLUGIN_ROOT}/../.." && pwd)"
    for env_file in "${HOME}/.claude/.env" "${repo_root}/.env" "${repo_root}/.env.local"; do
        if [[ -f "$env_file" ]]; then
            set -a
            source "$env_file"
            set +a
