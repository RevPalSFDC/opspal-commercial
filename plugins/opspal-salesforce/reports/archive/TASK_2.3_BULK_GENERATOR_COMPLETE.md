...
**Required Arguments**:
- `org-alias`: Salesforce org alias (e.g., acme-production)
...
```bash
# Basic usage
node bulk-decision-generator.js process acme-production duplicate-pairs.json

# High-performance processing
node bulk-decision-generator.js process delta-sandbox pairs.csv \
    --batch-size 500 \
    --concurrency 10 \
...
```bash
node bulk-decision-generator.js process epsilon-corp2021-revpal pairs.json
```
...
```bash
node bulk-decision-generator.js process acme-production large-pairs.json \
    --batch-size 500 \
    --concurrency 10 \
...
```bash
# Start processing
node bulk-decision-generator.js process delta-sandbox massive-pairs.json \
    --batch-size 500 \
    --concurrency 10 \
    --checkpoint-interval 5000
...
node find-duplicates.js acme-production > duplicate-pairs.json
...
**Test 1: Small Batch (acme-corp - 100 pairs)**
```bash
node bulk-decision-generator.js process acme-production test-pairs-100.json
# Expected: Complete in <10 sec, 100 decisions generated
```

**Test 2: Medium Batch (acme-corp - 1,000 pairs)**
```bash
node bulk-decision-generator.js process acme-production test-pairs-1000.json \
    --batch-size 200 \
    --concurrency 5
...
```bash
# Start processing
node bulk-decision-generator.js process acme-production test-pairs-500.json \
