...
Create `.claude-plugins/opspal-salesforce/hooks/pre-agent-dedup.sh`:
```bash
#!/bin/bash
...
export DEDUP_ENGINE_PATH=".claude-plugins/opspal-salesforce/scripts/lib/dedup-safety-engine.js"
export DEDUP_CONFIG_PATH=".claude-plugins/opspal-salesforce/config/dedup-config.json"
...
Create `.claude-plugins/opspal-salesforce/scripts/lib/agent-dedup-helper.js`:
```javascript
/**
...
Modify `.claude-plugins/opspal-salesforce/commands/dedupe.md` to add `execute` action:
...
1. **prepare** - Load duplicate pairs and prepare for analysis
   Example: /dedupe prepare --org epsilon-corp --file duplicate-pairs.csv
...
Update `.claude-plugins/opspal-salesforce/agents/sfdc-dedup-safety-copilot.md` to v3.0.0:
