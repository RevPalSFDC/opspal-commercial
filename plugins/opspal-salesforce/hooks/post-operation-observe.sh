...
# ==============================================================================
# FUNCTION: Auto-detect Org from Context (Dual-Path Support)
# ==============================================================================
...
detect_org() {
  ...
  # Priority 2: From org-centric path (NEW)
  # Pattern: /orgs/{org}/platforms/{platform}/{instance}/
  if [[ "$cwd" =~ /orgs/([^/]+)/platforms/([^/]+)/([^/]+) ]]; then
    # Export additional context if available
    export DETECTED_ORG_SLUG="${BASH_REMATCH[1]}"
    export DETECTED_PLATFORM="${BASH_REMATCH[2]}"
    export DETECTED_INSTANCE="${BASH_REMATCH[3]}"
    # Return the instance name as the org alias for compatibility
    echo "${BASH_REMATCH[3]}"
    return 0
  fi

  # Priority 3: From legacy platform path
  # Pattern: /instances/{platform}/{org}/
  if [[ "$cwd" =~ /instances/(salesforce|hubspot|marketo)/([^/]+) ]]; then
    echo "${BASH_REMATCH[2]}"
    return 0
  fi

  # Priority 4: From legacy simple path
  # Pattern: /instances/{org}/
  if [[ "$cwd" =~ /instances/([^/]+) ]]; then
    echo "${BASH_REMATCH[1]}"
    return 0
  fi

  # Priority 5: From most recent sf org command (if available)
