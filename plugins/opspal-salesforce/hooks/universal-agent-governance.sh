...
#   2 - Approval required (HIGH risk - needs human approval)
#   4 - Permission error (agent tier insufficient)
#
# Version: 1.2.0 (Standardized Exit Codes)
# Created: 2025-10-25
# Updated: 2026-01-15
###############################################################################
...
if [[ -f "${PLUGIN_ROOT}/scripts/lib/sf-exit-codes.sh" ]]; then
    source "${PLUGIN_ROOT}/scripts/lib/sf-exit-codes.sh"
else
    # Fallback exit codes
    EXIT_SUCCESS=0
