...
while read field; do
  node scripts/lib/org-metadata-cache.js query myorg Account "$field"
done < field-refs.txt
```
...
node scripts/lib/flow-field-extractor.js ./flows/MyFlow.flow-meta.xml > refs.txt
...
```bash
# Verify field exists
node scripts/lib/org-metadata-cache.js query myorg Account CustomField__c
