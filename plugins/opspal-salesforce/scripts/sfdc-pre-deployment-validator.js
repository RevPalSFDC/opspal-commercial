...
class SalesforcePreDeploymentValidator {
    ...
    async validate(deploymentPath) {
        ...
        // 6.5 Validate dashboard persona contracts (warn-only)
        if (this.checks.dashboardPersonaContract) {
            await this.validateDashboardPersonaContracts(deploymentPath);
        }
    ...
    async validateReportSyntax(deploymentPath) {
        ...
        try {
            ...
            for (const file of reportFiles) {
                ...
                await this.runMetricReportChecks(file);
            }
    ...
    /**
     * Validate dashboards against persona KPI contracts (warn-only)
     */
    async validateDashboardPersonaContracts(deploymentPath) {
        console.log('🧭 Validating dashboard persona KPI contracts...');

        const dashboardsDir = path.join(deploymentPath, 'force-app', 'main', 'default', 'dashboards');
        if (!fs.existsSync(dashboardsDir)) {
            console.log('No dashboard metadata found for persona validation.');
            return;
        }

        const { loadDashboardFile, validateDashboardPersonaContract } = require('./lib/persona-kpi-validator');
        const dashboardFiles = this.findFiles(dashboardsDir, '.dashboard-meta.xml');

        if (dashboardFiles.length === 0) {
            console.log('No dashboard metadata files found.');
            return;
        }

        dashboardFiles.forEach(filePath => {
            try {
                const dashboard = loadDashboardFile(filePath);
                const result = validateDashboardPersonaContract(dashboard, {
                    org: this.orgAlias,
                    source: 'sfdc-pre-deployment-validator',
                    workspaceRoot: process.cwd()
                });

                if (result.status === 'warn') {
                    this.warnings.push({
                        type: 'DASHBOARD_PERSONA_WARNING',
                        file: filePath,
                        message: `Persona KPI warnings for ${result.dashboardName}: ${result.issues.map(i => i.message).join(' | ')}`,
                        severity: 'WARNING'
                    });
                }
            } catch (error) {
                this.warnings.push({
                    type: 'DASHBOARD_PERSONA_WARNING',
                    file: filePath,
                    message: `Persona KPI validation failed: ${error.message}`,
                    severity: 'WARNING'
                });
            }
        });
    ...
    async runMetricReportChecks(reportFile) {
        const semanticScript = path.join(__dirname, 'lib', 'report-semantic-validator.js');
        const failureScript = path.join(__dirname, 'lib', 'report-failure-mode-linter.js');
        const diagnosticsScript = path.join(__dirname, 'lib', 'report-intelligence-diagnostics.js');

        if (!fs.existsSync(semanticScript) || !fs.existsSync(failureScript) || !fs.existsSync(diagnosticsScript)) {
            this.warnings.push({
                type: 'REPORT_SEMANTIC_VALIDATION_MISSING',
                file: reportFile,
                message: 'Report validators not found. Skipping semantic and diagnostics checks.',
                severity: 'WARNING'
            });
            return;
        }
        ...
        try {
            const semanticResult = await execSafe(
                `node "${semanticScript}" --report "${reportFile}" --org ${this.orgAlias}`,
                { timeout: 20000 }
            );
            if (semanticResult.stdout && semanticResult.stdout.includes('Semantic warnings')) {
                this.warnings.push({
                    type: 'REPORT_SEMANTIC_WARNING',
                    file: reportFile,
                    message: 'Semantic warnings detected. Review report semantics output.',
                    severity: 'WARNING'
                });
            }
        } catch (error) {
            this.warnings.push({
                type: 'REPORT_SEMANTIC_VALIDATION_FAILED',
                file: reportFile,
                message: `Semantic validator failed: ${error.message}`,
                severity: 'WARNING'
            });
        }

        try {
            const failureResult = await execSafe(
                `node "${failureScript}" --report "${reportFile}" --org ${this.orgAlias}`,
                { timeout: 20000 }
            );
            if (failureResult.stdout && failureResult.stdout.includes('Failure-mode warnings')) {
                this.warnings.push({
                    type: 'REPORT_FAILURE_MODE_WARNING',
                    file: reportFile,
                    message: 'Failure-mode warnings detected. Review report construction risks.',
                    severity: 'WARNING'
                });
            }
        } catch (error) {
            this.warnings.push({
                type: 'REPORT_FAILURE_MODE_VALIDATION_FAILED',
                file: reportFile,
                message: `Failure-mode validator failed: ${error.message}`,
                severity: 'WARNING'
            });
        }

        try {
            const diagnosticsResult = await execSafe(
                `node "${diagnosticsScript}" --report "${reportFile}" --org ${this.orgAlias} --output json`,
                { timeout: 20000 }
            );
            const diagnostics = diagnosticsResult.stdout
                ? JSON.parse(diagnosticsResult.stdout)
                : {};
            const overallStatus = diagnostics.health?.overallStatus;
            if (overallStatus === 'fail') {
                this.warnings.push({
                    type: 'REPORT_HEALTH_FAIL',
                    file: reportFile,
                    message: 'Report health score flagged as FAIL. Review intent and health diagnostics.',
                    severity: 'WARNING'
                });
            } else if (overallStatus === 'warn') {
                this.warnings.push({
                    type: 'REPORT_HEALTH_WARN',
                    file: reportFile,
                    message: 'Report health score flagged as WARN. Review intent and health diagnostics.',
                    severity: 'WARNING'
                });
            }
        } catch (error) {
            this.warnings.push({
                type: 'REPORT_DIAGNOSTICS_FAILED',
                file: reportFile,
                message: `Report diagnostics failed: ${error.message}`,
                severity: 'WARNING'
            });
        }
    }
     ...
     */
    /**
     * Check for stale permission set files that would cause destructive overwrites.
     *
     * Salesforce Metadata API permission set deployments are DESTRUCTIVE OVERWRITES:
     * any fieldPermissions entries NOT in the deployed XML are REMOVED from the org.
     * This check retrieves current org state and blocks deployment if the local file
     * has fewer fieldPermissions than the org (indicating stale/incomplete local XML).
     *
     * Root cause: Reflections fa7ce0b1, 915135c9 (Feb 2026) - permission stripping
     */
    async checkPermissionSetStaleness(deploymentPath) {
        console.log('🔒 Checking permission sets for staleness (destructive overwrite prevention)...');

        const permDir = path.join(deploymentPath, 'force-app', 'main', 'default', 'permissionsets');
        if (!fs.existsSync(permDir)) {
            console.log('  ⚠️ No permission sets directory found, skipping staleness check\n');
            return;
        }
        ...
        for (const file of permFiles) {
            const content = fs.readFileSync(file, 'utf-8');
            const permSetApiName = path.basename(file, '.permissionset-meta.xml');
            const permSetLabel = this.extractXmlValue(content, 'label') || permSetApiName;

            // Count local fieldPermissions
            const localFieldBlocks = content.match(/<fieldPermissions>[\s\S]*?<\/fieldPermissions>/g) || [];
            const localFieldCount = localFieldBlocks.length;

            // Query org for current fieldPermissions count
            try {
                const query = `SELECT Id FROM FieldPermissions WHERE Parent.Name = '${permSetLabel}' OR Parent.Name = '${permSetApiName}'`;
                const result = execSafe('sf', [
                    'data', 'query',
                    '--query', query,
                ...
                if (result && result.stdout) {
                    ...
                    const orgFieldCount = (parsed.result && parsed.result.totalSize) || 0;

                    if (orgFieldCount > 0 && localFieldCount < orgFieldCount) {
                        const deficit = orgFieldCount - localFieldCount;
                        this.errors.push({
                            type: 'STALE_PERMISSION_SET',
                            message: `Permission set '${permSetLabel}' has ${localFieldCount} fieldPermissions locally but ${orgFieldCount} in org (${deficit} missing). ` +
                                `Deploying this file would REMOVE ${deficit} field permission(s) from the org. ` +
                                `Run 'sf project retrieve start -m "PermissionSet:${permSetApiName}" --target-org ${this.orgAlias}' to refresh, ` +
                                `then merge your changes into the retrieved file.`,
                            file: file,
                            severity: 'ERROR'
                        });
                    } else if (orgFieldCount > 0 && localFieldCount === orgFieldCount) {
                        console.log(`  ✅ ${permSetLabel}: ${localFieldCount} local = ${orgFieldCount} org fieldPermissions`);
                    } else if (orgFieldCount === 0) {
                        console.log(`  ℹ️  ${permSetLabel}: New permission set (${localFieldCount} fields), no org comparison needed`);
                    }
                }
            } catch (queryError) {
                // Non-blocking: if we can't query org, warn but don't block
                this.warnings.push({
                    type: 'PERMISSION_SET_STALENESS_CHECK_SKIPPED',
                    message: `Could not query org for permission set '${permSetLabel}' field count. ` +
                        `Staleness check skipped. Error: ${queryError.message}. ` +
                        `Manually verify with: sf project retrieve start -m "PermissionSet:${permSetApiName}"`,
                    file: file,
                    severity: 'WARNING'
                });
            }
        ...
        console.log('  ✅ Permission set staleness check complete\n');
    }
