...
/**
 * Detect whether a stage transition is backward (regression/reopen).
 *
 * Returns null if either stage is unknown in the hierarchy.
 * Returns an object describing the transition direction otherwise.
 *
 * @param {string} objectName - Salesforce object name
 * @param {string} fromStage - Current stage
 * @param {string} toStage - Target stage
 * @returns {Object|null} Transition analysis or null if stages unknown
 */
function detectBackwardTransition(objectName, fromStage, toStage) {
    const fromPos = getStagePosition(objectName, fromStage);
    const toPos = getStagePosition(objectName, toStage);

    if (fromPos === null || toPos === null) return null;

    const fromTerminalPos = isTerminalPositive(objectName, fromStage);
    const fromTerminalNeg = isTerminalNegative(objectName, fromStage);
    const isBackward = toPos < fromPos;

    return {
        from: { stage: fromStage, position: fromPos },
        to: { stage: toStage, position: toPos },
        direction: isBackward ? 'backward' : (toPos > fromPos ? 'forward' : 'lateral'),
        isBackward,
        isReopen: isBackward && (fromTerminalPos || fromTerminalNeg),
        fromTerminal: fromTerminalPos ? 'positive' : (fromTerminalNeg ? 'negative' : null),
        severity: fromTerminalPos && isBackward ? 'critical'
            : fromTerminalNeg && isBackward ? 'high'
            : isBackward ? 'medium'
            : 'none'
    };
}
