...
class PermissionSetCLI {
  ...
  static async executeMigration(options = {}) {
    ...
    for (const step of plan.migrationSteps) {
      ...
      if (!dryRun) {
        ...
        try {
          // Execute step by phase
          const stepOutput = this.executeStep(step, { org, verbose });
          if (verbose && stepOutput) {
  ...
  /**
   * Execute a single migration step by delegating to the appropriate SF CLI command.
   *
   * Each phase maps to a specific SF CLI or node invocation. Commands are taken from
   * the step.command field (generated by PermissionSetMigrationPlanner) so the CLI
   * acts as a thin executor that delegates to sfdc-permission-orchestrator tooling.
   *
   * @param {Object} step    - Migration step object from the plan
   * @param {Object} context - Execution context { org, verbose }
   * @returns {string} stdout from the executed command, or '' for non-command phases
   */
  static executeStep(step, context = {}) {
    const { org, verbose } = context;
    const orgFlag = org ? `--target-org ${org}` : '';

    switch (step.phase) {
      case 'BACKUP': {
        // sf data query: export current permission set assignments as backup
        const backupQuery = [
          'SELECT Id, PermissionSet.Name, Assignee.Username',
          'FROM PermissionSetAssignment',
          'WHERE PermissionSet.IsOwnedByProfile = false',
          'ORDER BY PermissionSet.Name, Assignee.Username'
        ].join(' ');
        const backupArgs = [
          'data', 'query',
          '--query', backupQuery,
        ...
        if (org) backupArgs.push('--target-org', org);
        console.log('\nBacking up current permission set assignments...');
        if (verbose) console.log(`  sf ${backupArgs.join(' ')}`);
        const output = execSync(
          ['sf'].concat(backupArgs.map(a => /\s/.test(a) ? `"${a}"` : a)).join(' '),
      ...
      case 'CREATE_CANONICAL': {
        // Delegate to the node permission-set-cli with the generated config file
        const createCmd = step.command;
        if (!createCmd) {
          throw new Error(`Step ${step.step} (CREATE_CANONICAL) has no command defined`);
        }
        console.log('\nDeploying canonical permission set...');
        if (verbose) console.log(`  ${createCmd}`);
        const output = execSync(createCmd, { encoding: 'utf-8', stdio: 'pipe' });
        console.log('✅ Canonical permission set created');
        return output;
      }

      case 'MIGRATE_ASSIGNMENTS': {
        // Delegate to the generated assignment migration script
        const migrateCmd = step.command;
        if (!migrateCmd) {
          throw new Error(`Step ${step.step} (MIGRATE_ASSIGNMENTS) has no command defined`);
        }
        const userCount = step.usersToMigrate != null ? step.usersToMigrate : 'all';
        console.log(`\nMigrating ${userCount} user assignment(s) to new canonical sets...`);
        if (verbose) console.log(`  ${migrateCmd}`);
        const output = execSync(migrateCmd, { encoding: 'utf-8', stdio: 'pipe' });
        console.log('✅ Migrated user assignments');
        return output;
      }

      case 'VALIDATION': {
        // Delegate to the generated validation script (sf data query internally)
        const validateCmd = step.command;
        if (!validateCmd) {
          throw new Error(`Step ${step.step} (VALIDATION) has no command defined`);
        }
        console.log('\nRunning validation checks...');
        if (verbose) console.log(`  ${validateCmd}`);
        const output = execSync(validateCmd, { encoding: 'utf-8', stdio: 'pipe' });
      ...
      case 'CLEANUP':
      case 'DEACTIVATE': {
        // Mark old sets for review — never delete automatically
        console.log('\nMarking old permission sets for review (no automatic deletion)...');
        const setsToReview = step.setsToDeactivate || [];
        if (setsToReview.length > 0) {
          console.log('  Permission sets to review and manually deactivate:');
          for (const setName of setsToReview) {
            console.log(`    - ${setName}`);
          }
        }
        console.log('  To deactivate: Setup > Permission Sets > [name] > Edit > uncheck Active');
        console.log('✅ Cleanup review list generated — manual deactivation required');
        return '';
      }

      case 'GRACE_PERIOD': {
        console.log('\n⏳ Grace period step recorded — no automated action taken');
        const duration = step.estimatedTime || 'see migration plan';
        console.log(`  Monitor for access issues during the grace period (${duration})`);
        console.log('✅ Grace period step recorded');
        return '';
      }

      default: {
        // For any other phase, run the step command if present and non-comment
        if (step.command && !step.command.startsWith('#')) {
          console.log('\nExecuting step command...');
          if (verbose) console.log(`  ${step.command}`);
          const output = execSync(step.command, { encoding: 'utf-8', stdio: 'pipe' });
          console.log('✅ Step completed');
          return output;
        }
        console.log(`✅ Step completed (no automated command for phase: ${step.phase})`);
        return '';
      }
    }
  }
...
DOCUMENTATION:
  User Guide:      .claude-plugins/opspal-salesforce/docs/PERMISSION_SET_USER_GUIDE.md
  Developer Guide: .claude-plugins/opspal-salesforce/docs/PERMISSION_SET_DEVELOPER_GUIDE.md
  Migration Guide: .claude-plugins/opspal-salesforce/docs/PERMISSION_SET_MIGRATION_GUIDE.md
