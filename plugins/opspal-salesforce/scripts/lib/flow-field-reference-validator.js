...
class FlowFieldReferenceValidator {
    ...
    /**
     * Filter a validation result down to existence-only blockers for deploy hooks.
    ...
    toExistenceOnlyResult(result) {
        ...
        const referencedFields = Array.from(new Set(
            filteredErrors
                .filter(error => error.object && error.field)
                .map(error => `${error.object}.${error.field}`)
        ));
        ...
        return {
            ...
            summary: {
                blockingErrors: filteredErrors.length,
...
Commands:
  validate <flowPath>          Validate field references in a Flow (requires --org)
...
Examples:
  node flow-field-reference-validator.js validate ./flows/MyFlow.xml --org myorg
  node flow-field-reference-validator.js validate ./flows/MyFlow.xml --org myorg --existence-only --json
  ...
  node flow-field-reference-validator.js population myorg Account.Industry Account.Type
    ...
    const effectiveVerbose = verbose && !jsonOutput;

    (async () => {
        try {
            switch (command) {
                case 'validate': {
                    const flowPath = positionalArgs[0];
                    const orgAlias = getOptionValue('--org');
                    if (!flowPath) {
                        console.error('Error: Flow path required');
                        process.exit(1);
                    }
                    if (!orgAlias) {
                        console.error('Error: --org <alias> is required for validate');
                        process.exit(1);
                    }
                    ...
                    const output = existenceOnly ? validator.toExistenceOnlyResult(result) : result;
                    if (jsonOutput) {
                        console.log(JSON.stringify(output, null, 2));
                    } else {
                        console.log(`\nValidation Result: ${output.valid ? '✅ PASSED' : '❌ FAILED'}`);
                        console.log(`  Flow: ${output.flowName}`);
                ...
                case 'snapshot': {
                    ...
                    const validator = new FlowFieldReferenceValidator('', { verbose: effectiveVerbose });
                ...
                case 'compare': {
                    const flowPath = positionalArgs[0];
                    const snapshotPath = positionalArgs[1];
                    ...
                    if (jsonOutput) {
                        ...
                        console.log(`  Added Fields: ${diff.fieldReferences.added.length}`);
                        console.log(`  Removed Fields: ${diff.fieldReferences.removed.length}`);
                        console.log(`  Modified Elements: ${diff.elements.modified.length}`);
                        if (diff.hasChanges) {
                            if (diff.fieldReferences.added.length > 0) {
                                console.log('\nAdded Fields:');
                                diff.fieldReferences.added.forEach(f => console.log(`  + ${f.object}.${f.field}@${f.element}`));
                            }
                            if (diff.fieldReferences.removed.length > 0) {
                                console.log('\nRemoved Fields:');
                                diff.fieldReferences.removed.forEach(f => console.log(`  - ${f.object}.${f.field}@${f.element}`));
                            }
                        }
                    }
                ...
                case 'population': {
                    const org = positionalArgs[0];
                    const fields = positionalArgs.slice(1);
                    ...
                    const validator = new FlowFieldReferenceValidator(org, { verbose: effectiveVerbose });
