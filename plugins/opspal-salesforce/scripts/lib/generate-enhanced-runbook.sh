...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 delta-sandbox"
    exit 1
fi

ORG="$1"

# Determine plugin root with robust path resolution
if [ -n "$CLAUDE_PLUGIN_ROOT" ]; then
  ...
  # Try to resolve using path-conventions module
...
if [ ! -f "$PLUGIN_ROOT/plugin.json" ] && [ ! -f "$PLUGIN_ROOT/.claude-plugin/plugin.json" ]; then
  ...
  echo "   export CLAUDE_PLUGIN_ROOT=/path/to/salesforce-plugin" >&2
  echo "" >&2
  echo "   Example:" >&2
  echo "   export CLAUDE_PLUGIN_ROOT=$(pwd)/.claude-plugins/opspal-salesforce" >&2
  exit 1
fi
