...
class ValidationRuleManager {
    ...
    async deactivate(objectName, ruleNames, options = {}) {
        ...
        const packagePath = `${os.tmpdir()}/vr-deactivate-${objectName}-${Date.now()}.xml`;
    ...
    async activate(objectName, ruleNames, options = {}) {
        ...
        const packageXml = this._createPackageXml(objectName, ruleNames);
        const packagePath = `${os.tmpdir()}/vr-activate-${objectName}-${Date.now()}.xml`;
        fs.writeFileSync(packagePath, packageXml);
...
Examples:
  node validation-rule-manager.js list-active delta-sandbox Opportunity
  node validation-rule-manager.js deactivate delta-sandbox Opportunity Amount_Required,Type_Required
  node validation-rule-manager.js activate delta-sandbox Opportunity Amount_Required
`);
        process.exit(1);
    }
