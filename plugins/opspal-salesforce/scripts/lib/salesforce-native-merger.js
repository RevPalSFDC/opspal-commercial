...
// Try to load field conflict analyzer for enhanced merge strategies
let FieldConflictAnalyzer = null;
try {
    FieldConflictAnalyzer = require('../../plugins/opspal-core/scripts/lib/deduplication/field-conflict-analyzer');
} catch (e) {
    // Also try relative path from salesforce plugin
    try {
        FieldConflictAnalyzer = require('../../../opspal-core/scripts/lib/deduplication/field-conflict-analyzer');
    } catch (e2) {
        // Field conflict analyzer not available, will use built-in strategies
    }
}
...
class SalesforceNativeMerger {
  constructor(orgAlias, options = {}) {
    ...
    this.failOnManualAcrConflicts = options.failOnManualAcrConflicts !== false;
    ...
    this.importanceKeywords = {
      ...
      revenue: ['revenue', 'amount', 'value', 'mrr', 'arr', 'acv', 'tcv', 'sga'],  // Added sga for FY26_SGA__c
      status: ['status', 'stage', 'phase', 'lifecycle', 'type'],
      contact: ['phone', 'email', 'website', 'url'],
      integration: ['integration', 'external', 'sync', 'id'],
      segment: ['segment', 'industry', 'tier', 'classification']  // NEW: Segmentation fields
    };
    ...
    // NEW: Critical custom fields configuration (mirrors canonical selector)
    this.criticalCustomFields = options.criticalCustomFields || {
    ...
    // NEW: Field conflict analyzer for enhanced merge recommendations
    this.fieldConflictAnalyzer = FieldConflictAnalyzer
      ? new FieldConflictAnalyzer({ criticalCustomFields: this.criticalCustomFields, verbose: this.verbose })
      : null;
  }
  ...
  async mergeAccounts(masterId, duplicateId, strategy = null, fieldRecommendations = null) {
    ...
    try {
      ...
      // Step 2: Resolve shared-contact ACR conflicts before merge
      const acrResolution = await this.resolveAccountContactRelationConflicts(masterId, duplicateId);

      // Step 3: Capture related records BEFORE merge
      const relatedRecordsBefore = await this.captureRelationships(duplicateId);

      // Step 4: Determine merged field values
  ...
  /**
   * Resolve AccountContactRelation conflicts before merge execution.
   * Deletes non-direct loser-account ACR links when safe and configured.
   *
   * @param {string} masterId - Survivor account ID
   * @param {string} duplicateId - Loser account ID
   * @returns {Promise<Object>} ACR resolution summary
   */
  async resolveAccountContactRelationConflicts(masterId, duplicateId) {
    if (!this.autoResolveIndirectAcrConflicts) {
      return {
        hasAcr: null,
        skipped: true,
        reason: 'autoResolveIndirectAcrConflicts disabled',
        autoDeletedCount: 0,
        manualConflicts: []
      };
    }
    ...
    const validator = new AccountMergeValidator(this.orgAlias, {
      ...
      autoResolveIndirectAcrConflicts: this.autoResolveIndirectAcrConflicts
    });

    const cleanupResult = await validator.cleanupSharedContactConflicts(
      masterId,
      duplicateId,
      {
        validation: {
      ...
      {
        dryRun: this.dryRun,
        failOnManualConflicts: this.failOnManualAcrConflicts
      }
    );

    if (cleanupResult.autoDeletedCount > 0) {
      this.log(
        `Resolved ${cleanupResult.autoDeletedCount} non-direct AccountContactRelation conflict(s) before merge`,
        'INFO'
      );
    }
   ...
   *
   * Enhanced in v3.4.0 to support field conflict analyzer recommendations
   */
  mergeFieldValues(masterRecord, duplicateRecord, strategy, fieldRecommendations = null) {
    ...
    // NEW: If we have field conflict analyzer and no explicit recommendations, generate them
    let effectiveRecommendations = fieldRecommendations;
    if (!effectiveRecommendations && this.fieldConflictAnalyzer && strategy === 'auto') {
      try {
        effectiveRecommendations = this.fieldConflictAnalyzer.getFieldRecommendationsMap(
          masterRecord,
          [duplicateRecord]
        );
        if (Object.keys(effectiveRecommendations).length > 0) {
          this.log(`Generated ${Object.keys(effectiveRecommendations).length} field recommendations from analyzer`, 'DEBUG');
        }
      } catch (e) {
        this.log(`Field conflict analyzer error: ${e.message}`, 'WARN');
      }
    }
    ...
    for (const field of this.objectMetadata.fields) {
      ...
      // NEW: Check if field conflict analyzer has a recommendation for this field
      if (effectiveRecommendations && effectiveRecommendations[field.name]) {
        const rec = effectiveRecommendations[field.name];

        // Skip from-decision fields (require manual review)
        if (rec.strategy !== 'from-decision' && rec.value !== undefined) {
          shouldUpdate = true;
          newValue = rec.value;
          reason = rec.reason || `From analyzer: ${rec.strategy}`;
        }
      }

      // If no recommendation from analyzer, apply strategy
      if (!shouldUpdate) {
        switch (strategy) {
          case 'auto':
          ...
          case 'highest-value':
            // NEW: Strategy for currency/numeric fields
            if (field.type === 'currency' || field.type === 'double' || field.type === 'percent') {
              const masterNum = parseFloat(masterValue || 0);
              const duplicateNum = parseFloat(duplicateValue || 0);
              if (duplicateNum > masterNum) {
                shouldUpdate = true;
                newValue = duplicateValue;
                reason = `Higher value selected: ${duplicateNum} > ${masterNum}`;
              }
            }
            break;

          case 'most-recent':
            // NEW: Strategy for date fields - prefer more recent
            if (field.type === 'date' || field.type === 'datetime') {
              const masterDate = masterValue ? new Date(masterValue) : new Date(0);
              const duplicateDate = duplicateValue ? new Date(duplicateValue) : new Date(0);
              if (duplicateDate > masterDate) {
                shouldUpdate = true;
                newValue = duplicateValue;
                reason = 'More recent value selected';
              }
            }
            break;
        }
