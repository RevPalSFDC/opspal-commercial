...
const CONFIG = {
    ...
    // Set ANALYTICS_LIVE_FIRST=false to use cache-first behavior (not recommended)
    LIVE_FIRST: process.env.ANALYTICS_LIVE_FIRST !== 'false',
...
class AnalyticsDiscoveryV2 {
    constructor(instanceUrl, accessToken, options = {}) {
        ...
        this.orgAlias = options.orgAlias || process.env.SF_TARGET_ORG || process.env.ORG || null;
        this.safeMode = options.safeMode ?? CONFIG.SAFE_MODE;
        // Live-first mode: query live org first, use cache only as fallback
        this.liveFirst = options.liveFirst ?? CONFIG.LIVE_FIRST;
    ...
    /**
     * Resolve folder with live-first behavior
     * Queries live org first, falls back to cache on API failure
     */
    async resolveFolderId(folderName = null, options = {}) {
        const cacheKey = `folders_report_${this.apiVersion}`;

        // In cache-first mode (liveFirst=false), check cache unless forceRefresh
        ...
        try {
            ...
            // In live-first mode, try cache as fallback on API failure
            if (this.liveFirst) {
                const fallback = await this.getCachedFallback(cacheKey);
                if (fallback) {
                    console.warn(`API failed, using cached data (${new Date(fallback.timestamp).toISOString()}): ${error.message}`);
                    const folders = fallback.data;
                    if (folderName) {
                        const folder = folders.find(f =>
                            f.label?.toLowerCase() === folderName.toLowerCase() ||
                            f.name?.toLowerCase() === folderName.toLowerCase()
    ...
    /**
     * Discover report types with live-first behavior
     * Queries live org first, falls back to cache on API failure
     */
    ...
    /**
     * Describe report type with live-first behavior
     * Queries live org first, falls back to cache on API failure
     */
    async describeReportType(reportType, options = {}) {
        ...
        try {
            ...
            // In live-first mode, try cache as fallback on API failure (except 404/400)
            if (this.liveFirst && error.statusCode !== 404 && error.statusCode !== 400) {
                const fallback = await this.getCachedFallback(cacheKey);
                if (fallback) {
                    console.warn(`API failed, using cached data (${new Date(fallback.timestamp).toISOString()}): ${error.message}`);
     ...
     * When liveFirst=true (default), returns null to force fresh API call
     * When liveFirst=false, checks cache before API call
     */
    async getCached(key, options = {}) {
        // In live-first mode, skip cache lookup unless explicitly requested
        // This ensures we always query the live org first
    ...
    /**
     * Get cached data as fallback when API call fails
     * Used in live-first mode when API is unavailable
     */
    async getCachedFallback(key) {
        // Force cache lookup regardless of liveFirst setting
        return this.getCached(key, { useCacheFirst: true });
    }
    ...
    _extractReportIdFromEndpoint(endpoint) {
        const match = String(endpoint || '').match(/\/analytics\/reports\/([a-zA-Z0-9]{15,18})(?:\/|$|\?)/);
        return match ? match[1] : null;
    }

    async _probeReportExistence(reportId) {
        if (!reportId || !this.orgAlias) {
            return { checked: false, exists: false, folderName: null };
        }

        try {
            const soql = `SELECT Id, FolderName FROM Report WHERE Id = '${reportId}' LIMIT 1`;
            const cmd = `sf data query --query "${soql}" --target-org ${this.orgAlias} --json`;
    ...
    async _buildReportNotFoundHint(endpoint) {
        const reportId = this._extractReportIdFromEndpoint(endpoint);
        if (!reportId) {
            return '';
        }

        const probe = await this._probeReportExistence(reportId);
        if (probe.checked && probe.exists) {
            const folderClause = probe.folderName ? ` (folder: "${probe.folderName}")` : '';
            return ` Report ${reportId}${folderClause} exists, but Analytics REST returned NOT_FOUND. Likely private/personal folder visibility constraints.`;
        }

        if (probe.checked && !probe.exists) {
            return ` Report ${reportId} was not found via SOQL either (likely deleted or invalid ID).`;
        }

        return ' Report may be deleted OR inaccessible due to private/personal folder visibility.';
    }
                ...
                res.on('end', async () => {
                    ...
                    try {
                        ...
                        if (res.statusCode >= 200 && res.statusCode < 300) {
                            ...
                            let message = result.message || result[0]?.message || JSON.stringify(result);
                            if (
                                res.statusCode === 404 &&
                                String(endpoint || '').includes('/analytics/reports/')
                            ) {
                                const hint = await this._buildReportNotFoundHint(endpoint);
                                if (hint) {
                                    message += hint;
                                }
                            }
