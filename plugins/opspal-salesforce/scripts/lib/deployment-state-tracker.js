...
class DeploymentStateTracker {
    ...
    /**
     * Verify post-deploy field existence against expected manifest
     * @param {string} orgAlias - Salesforce org alias
     * @param {Array} expectedFields - Array of { object, field } to verify
     * @returns {Object} Verification result
     */
    async verifyPostDeploy(orgAlias, expectedFields = []) {
        const org = orgAlias || this.orgAlias;
        if (!org) {
            throw new Error('Org alias is required');
        }
        ...
        if (!expectedFields.length) {
            return { verified: true, total: 0, missing: [], flsMissing: [] };
        }
        ...
        for (const item of expectedFields) {
            const obj = item.object || item.objectName;
            const field = item.field || item.fieldName || item.apiName;
            if (!byObject[obj]) byObject[obj] = [];
            byObject[obj].push(field);
        }
        ...
        for (const [objectName, fields] of Object.entries(byObject)) {
            try {
                const describeResult = spawnSync('sf', [
                    'sobject', 'describe', objectName,
                    '--target-org', org, '--json'
                ], {
                    encoding: 'utf8',
                    timeout: 30000,
                ...
                if (describeResult.status !== 0) {
                    throw new Error(describeResult.stderr || 'sf sobject describe failed');
                }

                const parsed = JSON.parse(describeResult.stdout);
                const orgFields = (parsed.result?.fields || []).map(f => f.name);

                for (const field of fields) {
                    if (orgFields.includes(field)) {
                        result.verified_count++;
                    } else {
                        result.missing.push({ object: objectName, field });
                        result.verified = false;
                    }
                }
            } catch (error) {
                for (const field of fields) {
                    result.missing.push({ object: objectName, field, error: error.message });
                }
                result.verified = false;
            }
        }
        ...
        if (this.verbose) {
            console.log(`[STATE] Post-deploy verification: ${result.verified_count}/${result.total} fields verified`);
            if (result.missing.length > 0) {
                console.log(`[STATE] Missing fields: ${result.missing.map(m => `${m.object}.${m.field}`).join(', ')}`);
            }
        }
    ...
    /**
     * Invalidate cached state for an org (B5c: source tracking cache invalidation)
     * Call after a failed deployment to force fresh state capture on next run
     * @param {string} orgAlias - Salesforce org alias (if null, invalidates all)
     */
    invalidateCache(orgAlias = null) {
        const snapshots = this._loadSnapshots();
        let removed = 0;

        for (const snapshot of snapshots) {
            if (orgAlias && snapshot.orgAlias !== orgAlias) continue;

            const snapshotPath = path.join(this.stateDir, `${snapshot.id}.json`);
        ...
        if (!orgAlias || this.currentState?.orgAlias === orgAlias) {
            this.currentState = null;
            this.previousState = null;
        }

        if (this.verbose) {
            console.log(`[STATE] Cache invalidated: ${removed} snapshot(s) removed${orgAlias ? ` for ${orgAlias}` : ''}`);
        }

        return { removed, orgAlias };
    }
}
