...
class OrgContextInjector {
    ...
    async enrichOrgContext(alias, source) {
        // Validate org alias to prevent command injection
        if (!validateOrgAlias(alias)) {
            if (this.options.verbose) {
                console.warn(`⚠️  Invalid org alias format: ${alias}`);
            }
            return { alias, source, detectionMethods: this.detectionMethods };
        }
    ...
    static async validateOrg(orgAlias) {
        ...
        try {
            execFileSync('sf', ['org', 'display', '--target-org', orgAlias], {
                stdio: 'ignore'
            });
            return true;
        } catch (error) {
