...
// Try to load SkillTracker from opspal-core
let SkillTracker;
try {
    const skillTrackerPath = path.join(__dirname, '../../../opspal-core/scripts/lib/skill-tracker.js');
...
class ReflectionSkillAnalyzer {
    ...
    _loadSkillRegistry(customPath) {
        const registryPath = customPath || path.join(
            __dirname,
            '../../../opspal-core/config/skill-registry.json'
        );
