...
const CANONICAL_FLOW_ELEMENT_ORDER = [
  'apiVersion', 'description', 'environments', 'interviewLabel',
  'label', 'processMetadataValues', 'processType',
  'actionCalls', 'apexPluginCalls', 'assignments',
  'collectionProcessors', 'customErrors', 'decisions',
  'dynamicChoiceSets', 'formulas', 'loops',
  'orchestratedStages', 'recordCreates', 'recordDeletes',
  'recordLookups', 'recordRollbacks', 'recordUpdates',
  'screens', 'stages', 'steps', 'subflows',
  'textTemplates', 'variables', 'waits',
  'start', 'status', 'runInMode', 'triggerOrder'
];
...
class FlowXMLValidator {
  ...
  validateFlow(flowPath) {
    ...
    try {
      ...
      this.checkElementOrdering(doc);
      this.checkRecordTriggerTypeLocation(doc);
      this.checkRestrictedValueSetLocation(doc);
      this.checkSelfReferencingConnectors(doc);
      this.checkTextTemplateVariableReferences(doc);
  ...
  /**
   * Check that top-level flow elements of the same type remain contiguous.
   */
  checkElementOrdering(doc) {
    if (!doc.documentElement) {
      return;
    }

    const topLevelElements = this.getDirectChildElements(doc.documentElement);
    const seenTags = new Set();
    const outOfOrderTags = new Set();
    const canonicalViolations = new Set();
    ...
    topLevelElements.forEach(element => {
      ...
      if (tagName === previousTag) {
        const canonicalIndex = this.getCanonicalFlowElementIndex(tagName);
        if (canonicalIndex !== null) {
          maxCanonicalIndex = Math.max(maxCanonicalIndex, canonicalIndex);
        }
        return;
      }

      if (seenTags.has(tagName)) {
        outOfOrderTags.add(tagName);
      }
      ...
      if (canonicalIndex < maxCanonicalIndex) {
        canonicalViolations.add(tagName);
        return;
      }

      maxCanonicalIndex = canonicalIndex;
    });

    outOfOrderTags.forEach(tagName => {
      this.errors.push({
        type: 'ELEMENT_ORDERING',
        severity: 'HIGH',
        message: `Top-level <${tagName}> elements are not contiguous`,
        details: `Salesforce Flow XML expects grouped top-level elements of the same type to stay together.`,
        location: `Flow root: ${tagName}`
      });
    });

    if (canonicalViolations.size > 0) {
      this.errors.push({
        type: 'ELEMENT_ORDERING',
        severity: 'MEDIUM',
        message: 'Top-level Flow elements are not in canonical Salesforce order',
        details: `Found out-of-order elements: ${Array.from(canonicalViolations).join(', ')}`,
        location: 'Flow root'
      });
    }

    if (outOfOrderTags.size > 0 || canonicalViolations.size > 0) {
      this.ensureElementOrderingFix();
    }
  }

  /**
   * Check recordTriggerType placement for record-triggered flows.
   */
  checkRecordTriggerTypeLocation(doc) {
    if (!doc.documentElement) {
      return;
    }

    const misplaced = this.getDirectChildElementsByTagName(doc.documentElement, 'recordTriggerType');
    if (misplaced.length > 0) {
      this.errors.push({
        type: 'RECORD_TRIGGER_TYPE_LOCATION',
        severity: 'HIGH',
        message: 'recordTriggerType must be nested inside <start>',
        details: 'Top-level recordTriggerType nodes are invalid in Flow XML.',
        location: 'Flow root'
      });
    }
  }
  ...
  /**
   * Check that restricted picklist metadata is nested inside valueSet.
   */
  checkRestrictedValueSetLocation(doc) {
    const restrictedNodes = doc.getElementsByTagName('restricted');

    for (let i = 0; i < restrictedNodes.length; i++) {
      const parentTag = restrictedNodes[i].parentNode?.tagName;
      if (parentTag !== 'valueSet') {
        this.errors.push({
          type: 'RESTRICTED_VALUESET_LOCATION',
          severity: 'HIGH',
          message: '<restricted> must be nested inside <valueSet>',
          details: `Found <restricted> under <${parentTag || 'unknown'}>.`,
          location: `Parent: ${parentTag || 'unknown'}`
        });
      }
    }
  }
  ...
  /**
   * Check for connectors that point back to the same flow element.
   */
  checkSelfReferencingConnectors(doc) {
    const topLevelElements = this.getDirectChildElements(doc.documentElement || {});

    topLevelElements.forEach(element => {
      const name = this.getDirectChildText(element, 'name');
      if (!name) {
        return;
      }

      const connectors = this.getDirectChildElementsByTagName(element, 'connector');
      connectors.forEach(connector => {
        const targetReference = this.getDirectChildText(connector, 'targetReference');
        if (targetReference === name) {
          this.errors.push({
            type: 'SELF_REFERENCING_CONNECTOR',
            severity: 'HIGH',
            message: `Element "${name}" has a connector targeting itself`,
            details: 'Connector targetReference cannot point back to the same flow element.',
  ...
  /**
   * Check that text template variable references resolve to known flow resources.
   */
  checkTextTemplateVariableReferences(doc) {
    ...
    topLevelElements.forEach(element => {
      const name = this.getDirectChildText(element, 'name');
      if (name) {
        validReferences.add(name);
      }
    });

    const textTemplates = doc.getElementsByTagName('textTemplates');
    const refPattern = /\{!([^}]+)\}/g;

    for (let i = 0; i < textTemplates.length; i++) {
      const template = textTemplates[i];
      const templateName = this.getDirectChildText(template, 'name') || 'Unknown';
      const bodyParts = this.getDirectChildElements(template)
      ...
      while ((match = refPattern.exec(body)) !== null) {
        ...
        const rootReference = reference.split('.')[0];
        if (!rootReference || rootReference.startsWith('$') || validReferences.has(rootReference)) {
          continue;
        }

        if (seenInvalidRefs.has(rootReference)) {
          continue;
        }
        seenInvalidRefs.add(rootReference);

        this.errors.push({
          type: 'TEXT_TEMPLATE_REFERENCE',
          severity: 'HIGH',
          message: `Text template "${templateName}" references unknown resource "${rootReference}"`,
          details: `Reference "{!${reference}}" does not match any named flow resource.`,
          location: `Text Template: ${templateName}`
        });
      }
    }
  }
  ...
  applyFixes(flowPath, doc) {
    ...
    let needsElementReorder = false;

    this.fixes.forEach(fix => {
      if (fix.reorder === true) {
        needsElementReorder = true;
        return;
      }
    ...
    if (needsElementReorder) {
      flowXML = this.reorderFlowElements(flowXML);
      this.log('  ✅ Fixed: Reordered top-level Flow elements');
    }
  ...
  /**
   * Return direct child elements with a given tag.
   */
  getDirectChildElementsByTagName(node, tagName) {
    return this.getDirectChildElements(node).filter(child => child.tagName === tagName);
  }

  /**
   * Return the text content of a direct child element.
   */
  getDirectChildText(node, tagName) {
    return this.getDirectChildElementsByTagName(node, tagName)[0]?.textContent || null;
  }

  getCanonicalFlowElementIndex(tagName) {
    const index = CANONICAL_FLOW_ELEMENT_ORDER.indexOf(tagName);
    return index === -1 ? null : index;
  }

  ensureElementOrderingFix() {
    const hasFix = this.fixes.some(fix => fix.type === 'ELEMENT_ORDERING' && fix.reorder === true);
    if (!hasFix) {
      this.fixes.push({
        type: 'ELEMENT_ORDERING',
        description: 'Reorder top-level Flow elements to canonical Salesforce order',
        automated: true,
        reorder: true
      });
    }
  ...
  reorderFlowElements(flowXML) {
    ...
    const sortedElements = originalChildNodes
      ...
      .map((node, originalIndex) => ({
        node,
        originalIndex,
        order: this.getCanonicalFlowElementIndex(node.tagName) ?? (CANONICAL_FLOW_ELEMENT_ORDER.length + originalIndex)
      }))
      .sort((a, b) => {
        if (a.order !== b.order) {
    ...
    originalChildNodes.forEach(node => {
      if (node.nodeType === 1) {
        rebuiltNodes.push(sortedElements[elementIndex++]);
      } else {
    ...
    rebuiltNodes.forEach(node => {
      root.appendChild(node);
    });

    const serialized = new XMLSerializer().serializeToString(doc);
    const xmlDeclaration = flowXML.match(/^\s*<\?xml[^>]*\?>/);
...
if (require.main === module) {
  ...
  const validator = new FlowXMLValidator({ verbose: jsonOutput ? false : true, autoFix });
