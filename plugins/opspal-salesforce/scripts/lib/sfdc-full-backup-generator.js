 ...
 *   node sfdc-full-backup-generator.js Account delta-production --include-deleted
 *   node sfdc-full-backup-generator.js Account delta-production --include-children --chunk-size 200
 *
 * Output Structure:
...
class SFDCFullBackupGenerator {
    constructor(options = {}) {
        ...
        // Support for mock bulk handler (test mode) vs CLI mode
        ...
        // Get org connection info (skip if bulkHandler provided or skipOrgCheck - test mode)
        if (this.bulkHandler || this.skipOrgCheck) {
            this.orgInfo = { instanceUrl: 'mock://test', username: 'test@test.com', orgId: '00D000000000000' };
        } else {
    ...
    async extractActiveRecords() {
        // P2 Enhancement: Route to parallel extraction if enabled and bulkHandler available
        if (this.enableParallel && this.bulkHandler) {
            return this.extractActiveRecordsParallel();
        }
    ...
    /**
     * P2 Enhancement: Build keyset queries for parallel execution
     * Divides the total record set into non-overlapping ID ranges
     */
    async buildKeysetQueries(totalRecords, batchSize) {
        // First, get sample IDs to determine ID ranges
        const sampleSize = Math.min(totalRecords, 10000);
        const sampleQuery = `SELECT Id FROM ${this.sobject} ORDER BY Id ASC LIMIT ${sampleSize}`;

        const sampleResult = await this.executeSoqlQuery(sampleQuery);
        const sampleIds = sampleResult.records.map(r => r.Id);

        if (sampleIds.length === 0) {
            return [];
        }

        // Divide into ranges based on concurrency
        const queries = [];
        const recordsPerBatch = Math.ceil(sampleIds.length / this.concurrency);

        for (let i = 0; i < this.concurrency; i++) {
            const startIdx = i * recordsPerBatch;
            const endIdx = Math.min((i + 1) * recordsPerBatch, sampleIds.length);

            if (startIdx >= sampleIds.length) break;

            let query = `SELECT FIELDS(ALL) FROM ${this.sobject}`;

            if (i === 0) {
                // First query: Id < endId
                if (endIdx < sampleIds.length) {
                    query += ` WHERE Id < '${sampleIds[endIdx]}'`;
                }
            } else if (i === this.concurrency - 1 || endIdx >= sampleIds.length) {
                // Last query: Id >= startId
                query += ` WHERE Id >= '${sampleIds[startIdx]}'`;
            } else {
                // Middle queries: Id >= startId AND Id < endId
                query += ` WHERE Id >= '${sampleIds[startIdx]}' AND Id < '${sampleIds[endIdx]}'`;
            }
    ...
    /**
     * P2 Enhancement: Extract active records using parallel processing
     * Uses keyset pagination with concurrent batch execution
     */
    async extractActiveRecordsParallel() {
        if (!this.bulkHandler) {
            throw new Error('Parallel extraction requires bulkHandler');
        }

        const countResult = await this.executeSoqlQuery(`SELECT COUNT() FROM ${this.sobject}`);
        const totalRecords = countResult.records[0]?.expr0 || 0;

        if (totalRecords === 0) {
            return { records: [], totalSize: 0, done: true };
        }

        const queries = await this.buildKeysetQueries(totalRecords, 10000);
        ...
        const { results, errors } = await this.executeBatchQueriesParallel(queries);

        if (errors.length > 0) {
            throw new Error(`Parallel extraction failed: ${errors[0].error}`);
        }

        // Combine and sort all records by Id
        const allRecords = results
            .flatMap(r => r.result.records)
    ...
    /**
     * Execute SOQL query using sf CLI or bulkHandler (test mode)
     */
    async executeSoqlQuery(query) {
        try {
            // Use bulkHandler if available (test mode)
            if (this.bulkHandler) {
                const result = await this.bulkHandler.query(query);
                // Handle both array and object responses from mock
                if (Array.isArray(result)) {
                    return { records: result, totalSize: result.length, done: true };
                }
                return {
                    records: result.records || [],
                    totalSize: result.totalSize || result.records?.length || 0,
                    done: result.done !== undefined ? result.done : true
                };
            }
...
Examples:
  ...
  # Active records only
  node sfdc-full-backup-generator.js Account delta-production --no-deleted --no-children

  # Custom chunk size
  node sfdc-full-backup-generator.js Account delta-production --chunk-size 500

  # Custom backup location
  node sfdc-full-backup-generator.js Account delta-production --backup-dir /mnt/backups
