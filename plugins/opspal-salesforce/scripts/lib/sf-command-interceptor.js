            ...
            if (NON_EXISTENT_OBJECTS[objectName]) {
                ...
                result.errors.push({
                    ...
                    guidance: [
                        ...
                        `   ${objInfo.example}`,
                        ``,
                        `📚 Documentation: .claude-plugins/opspal-salesforce/${objInfo.docs}`
                    ]
                });
            }
        }
    ...
    async validateDeployCommand(parsed, result) {
        ...
        if (sourceDir) {
            try {
                ...
                const validation = await validator.validateSourceDir(sourceDir);
                if (validation && validation.valid === false) {
                    const detail = validation.errors && validation.errors.length > 0
            ...
            switch (error.type) {
                case 'INVALID_FIELD':
                    ...
                    if (error.field === 'ApiName' && error.object === 'FlowVersionView') {
                        ...
                        guidance.push(`   📖 Docs: .claude-plugins/opspal-salesforce/docs/SALESFORCE_TOOLING_API_FLOW_OBJECTS.md`);
                ...
                case 'MALFORMED_QUERY':
                    ...
                    guidance.push(`   📖 Docs: .claude-plugins/opspal-salesforce/docs/SOQL_BEST_PRACTICES.md`);
                    break;
...
Examples:
  ...
  # Dry run JSON mode
  node sf-command-interceptor.js "sf data query ..." --dry-run --json
