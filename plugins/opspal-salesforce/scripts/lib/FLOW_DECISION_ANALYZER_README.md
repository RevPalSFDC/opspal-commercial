...
- name: Validate Flow Logic
  run: |
    for flow in force-app/main/default/flows/*.flow-meta.xml; do
      node scripts/lib/flow-decision-logic-analyzer.js $SFDC_ORG "$flow"
    done
```
...
**Main Script:** `.claude-plugins/opspal-salesforce/scripts/lib/flow-decision-logic-analyzer.js`
**Tests:** `.claude-plugins/opspal-salesforce/test/flow-decision-logic-analyzer.test.js`
