...
class AccountMergeValidator {
  ...
  constructor(orgAlias, options = {}) {
    ...
    this.autoResolveIndirectAcrConflicts = options.autoResolveIndirectAcrConflicts !== false;
  }
  ...
  async validateSharedContacts(masterId, duplicateId, mergeProfile = {}) {
    ...
    try {
      ...
      const sharedRelationships = await this.querySharedContactRelationships(masterId, duplicateId);
      const resolutionPlan = this.buildSharedContactResolutionPlan(sharedRelationships, mergeProfile);

      result.sharedContacts = sharedRelationships.map((contact) => ({
        contactId: contact.contactId,
        contactName: contact.contactName,
        masterRelationshipId: contact.masterRelationships[0]?.id || null,
        duplicateRelationshipId: contact.duplicateRelationships[0]?.id || null,
        masterRoles: contact.masterRelationships[0]?.roles || null,
        duplicateRoles: contact.duplicateRelationships[0]?.roles || null,
        masterRelationshipCount: contact.masterRelationships.length,
        duplicateRelationshipCount: contact.duplicateRelationships.length,
        duplicateHasIndirect: contact.duplicateRelationships.some((r) => r.isDirect === false)
      }));

      result.resolutionPlan = resolutionPlan;

      if (resolutionPlan.autoResolvableConflicts.length > 0) {
        result.warnings.push(
          'AUTO_RESOLVE_AVAILABLE: ' + resolutionPlan.autoResolvableConflicts.length +
          ' shared contact conflict(s) can be auto-resolved by deleting non-direct loser AccountContactRelation records.'
        );
      }

      if (resolutionPlan.manualConflicts.length > 0) {
        result.isValid = false;
        result.errors.push(
          'SHARED_CONTACT_CONFLICT: Found ' + resolutionPlan.manualConflicts.length +
          ' contact(s) with unresolved AccountContactRelation conflicts. Salesforce blocks account merges until these conflicts are cleared.'
        );
        result.errors.push('');
        result.errors.push('Manual Resolution Required:');

        for (const conflict of resolutionPlan.manualConflicts) {
          result.errors.push('  - ' + conflict.contactName + ' (' + conflict.contactId + ')');
          result.errors.push('    Master Relationships: ' + (conflict.masterRelationshipIds.join(', ') || 'None'));
          result.errors.push('    Duplicate Relationships: ' + (conflict.duplicateRelationshipIds.join(', ') || 'None'));
          result.errors.push('    Reason: ' + conflict.reason);
          result.errors.push('');
        }

        result.errors.push('REQUIRED ACTION:');
        result.errors.push('  1. Resolve listed AccountContactRelation conflicts manually');
        result.errors.push('  2. Keep only one AccountContactRelation path per shared contact');
        result.errors.push('  3. Retry merge after cleanup');
      }
  ...
  /**
   * Query shared AccountContactRelation relationships between two accounts.
   *
   * @param {string} masterId - Master account ID
   * @param {string} duplicateId - Duplicate account ID
   * @returns {Promise<Array>} Shared relationship descriptors
   */
  async querySharedContactRelationships(masterId, duplicateId) {
    const acrQuery = `
    ...
    for (const rel of relationships) {
      ...
      const mapped = {
        id: rel.Id,
        roles: rel.Roles || null,
        isDirect: rel.IsDirect === true
      };

      const container = contactMap.get(contactId);
      if (rel.AccountId === masterId) {
        container.masterRelationships.push(mapped);
      } else if (rel.AccountId === duplicateId) {
        container.duplicateRelationships.push(mapped);
      }
    }

    return Array.from(contactMap.values()).filter(
      (contact) => contact.masterRelationships.length > 0 && contact.duplicateRelationships.length > 0
    );
  }

  /**
   * Build shared-contact resolution plan.
   * Auto-resolvable conflicts are non-direct AccountContactRelation rows on the loser account.
   *
   * @param {Array} sharedContacts - Shared contact descriptors
   * @param {Object} mergeProfile - Merge profile configuration
   * @returns {Object} Resolution plan
   */
  buildSharedContactResolutionPlan(sharedContacts, mergeProfile = {}) {
    const autoResolveConfigured = mergeProfile?.validation?.autoResolveIndirectAcrConflicts;
    const autoResolveEnabled =
      (autoResolveConfigured !== false) && this.autoResolveIndirectAcrConflicts;
    ...
    for (const contact of sharedContacts) {
      const duplicateIndirect = contact.duplicateRelationships.filter(
        (relationship) => relationship.isDirect === false && this.isSalesforceId(relationship.id)
      );
      const duplicateDirect = contact.duplicateRelationships.filter(
        (relationship) => relationship.isDirect === true
      );

      if (autoResolveEnabled && duplicateIndirect.length > 0 && duplicateDirect.length === 0) {
        const deleteIds = duplicateIndirect.map((relationship) => relationship.id);
        plan.autoResolvableConflicts.push({
          contactId: contact.contactId,
          contactName: contact.contactName,
          deleteRelationshipIds: deleteIds,
          reason: 'Duplicate account has non-direct AccountContactRelation links for shared contact'
        });
        plan.deleteRelationshipIds.push(...deleteIds);
        continue;
      }

      const reason = duplicateDirect.length > 0
        ? 'Duplicate account has direct AccountContactRelation link; requires manual choice before merge'
        : 'No safe auto-delete candidate on duplicate account';

      plan.manualConflicts.push({
        contactId: contact.contactId,
        contactName: contact.contactName,
        masterRelationshipIds: contact.masterRelationships.map((relationship) => relationship.id),
        duplicateRelationshipIds: contact.duplicateRelationships.map((relationship) => relationship.id),
        reason
      });
    }
  ...
  /**
   * Cleanup auto-resolvable shared contact conflicts.
   *
   * @param {string} masterId - Master account ID
   * @param {string} duplicateId - Duplicate account ID
   * @param {Object} mergeProfile - Merge profile configuration
   * @param {Object} options - Cleanup options
   * @param {boolean} options.dryRun - When true, do not execute deletions
   * @param {boolean} options.failOnManualConflicts - Throw if unresolved conflicts remain
   * @returns {Promise<Object>} Cleanup summary
   */
  async cleanupSharedContactConflicts(masterId, duplicateId, mergeProfile = {}, options = {}) {
    const dryRun = options.dryRun === true;
    const failOnManualConflicts = options.failOnManualConflicts !== false;
    ...
    if (failOnManualConflicts && resolutionPlan.manualConflicts.length > 0) {
      throw new Error(this.buildManualSharedContactConflictMessage(resolutionPlan.manualConflicts));
    }

    const deletedRelationshipIds = [];
    if (!dryRun) {
      for (const relationshipId of resolutionPlan.deleteRelationshipIds) {
        await this.deleteAccountContactRelation(relationshipId);
        deletedRelationshipIds.push(relationshipId);
      }
    } else {
      deletedRelationshipIds.push(...resolutionPlan.deleteRelationshipIds);
    }

    return {
      hasAcr: true,
      dryRun,
      sharedContactCount: sharedRelationships.length,
      autoDeletedCount: deletedRelationshipIds.length,
      deletedRelationshipIds,
      manualConflicts: resolutionPlan.manualConflicts,
      autoResolvableConflicts: resolutionPlan.autoResolvableConflicts,
      resolutionPlan
    };
  }

  /**
   * Delete a single AccountContactRelation record.
   *
   * @param {string} relationshipId - AccountContactRelation ID
   */
  async deleteAccountContactRelation(relationshipId) {
    if (!this.isSalesforceId(relationshipId)) {
      throw new Error(`Invalid AccountContactRelation ID: ${relationshipId}`);
    }

    const deleteCmd = `sf data delete record --sobject AccountContactRelation --record-id ${relationshipId} --target-org ${this.orgAlias} --json`;
    const deleteOutput = execSync(deleteCmd, { encoding: 'utf-8' });
    const deleteData = JSON.parse(deleteOutput);

    if (deleteData.status !== 0) {
      throw new Error(`Failed to delete AccountContactRelation ${relationshipId}: ${deleteData.message || 'Unknown error'}`);
    }
  }

  /**
   * Basic Salesforce ID validation helper.
   *
   * @param {string} value - Candidate Salesforce ID
   * @returns {boolean} Whether ID shape is valid
   */
  ...
  /**
   * Build compact conflict message for callers.
   *
   * @param {Array} manualConflicts - Manual conflict entries
   * @returns {string} Error message
   */
  buildManualSharedContactConflictMessage(manualConflicts) {
    const sample = manualConflicts
      .slice(0, 3)
      .map((conflict) => conflict.contactName + ' (' + conflict.contactId + ')')
      .join(', ');

    return 'Shared contact conflicts require manual resolution for ' +
      manualConflicts.length + ' contact(s): ' + sample;
  }
