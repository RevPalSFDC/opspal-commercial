...
class DeploymentSourceValidator {
    ...
    async validateSourceDir(sourcePath) {
        ...
        try {
            ...
            // Check 1.5: Warn on object leaf directory deployments
            const objectScopeWarning = this.getObjectDirectoryScopeWarning(sourcePath, absolutePath);
            if (objectScopeWarning) {
                result.warnings.push(objectScopeWarning);
            }
    ...
    async scanMetadataTypes(metadataRoot) {
        ...
        try {
            ...
            const objectScopeType = await this.scanObjectMetadataScope(metadataRoot, contents);
            if (objectScopeType) {
                return [objectScopeType];
            }
    ...
    /**
     * Detect object-scoped metadata roots such as objects/Account or objects/Account/fields.
     * @private
     */
    isObjectMetadataDirectory(testPath, contents = []) {
        const normalizedPath = testPath.split(path.sep).join('/');
        const match = normalizedPath.match(/\/objects\/[^/]+(?:\/[^/]+)?$/);
        return Boolean(match && contents.length > 0);
    }

    /**
     * Represent object-scoped metadata roots as deployable CustomObject metadata.
     * @private
     */
    async scanObjectMetadataScope(metadataRoot, contents = null) {
        const normalizedPath = metadataRoot.split(path.sep).join('/');
        const match = normalizedPath.match(/\/objects\/([^/]+)(?:\/([^/]+))?$/);
        if (!match) {
            return null;
        }

        const resolvedContents = contents || await fs.readdir(metadataRoot);
        const fileCount = resolvedContents.length;
        if (fileCount === 0) {
            return null;
        }

        const [, objectName, leafDir] = match;
        return {
            folder: leafDir || objectName,
            type: 'CustomObject',
            count: fileCount
        };
    }
    ...
    /**
     * Warn when deployments target a leaf object metadata directory instead of the object root.
     * Deploying from objects/<Object>/<leaf>/ can omit sibling metadata such as fields or validation rules.
     * @private
     */
    getObjectDirectoryScopeWarning(sourcePath, absolutePath) {
        ...
        const [, objectName, leafDir] = match;
        return [
            `Object deployment path is scoped to a leaf directory: ${sourcePath}`,
            `Use objects/${objectName}/ instead of objects/${objectName}/${leafDir}/ to include sibling metadata folders recursively.`
        ].join(' ');
    }
    ...
    (async () => {
        try {
            switch (command) {
                ...
                case '--source-dir':
                    {
                        const result = await validator.validateSourceDir(target);
                        if (!result.valid) {
                            throw new Error(result.errors.join('; ') || 'Source validation failed');
                        }
                    }
                ...
                case '--manifest':
                    {
                        const result = await validator.validateManifest(target);
                        if (!result.valid) {
                            throw new Error(result.errors.join('; ') || 'Manifest validation failed');
                        }
                    }
                ...
                case '--metadata':
                    ...
                    {
                        const result = await validator.validateMetadata(target, orgAlias);
                        if (!result.valid) {
                            throw new Error(result.errors.join('; ') || 'Metadata validation failed');
                        }
                    }
