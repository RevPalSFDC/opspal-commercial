...
class PlaybookResolver {
    ...
    scorePlaybook(playbook, taskProfile) {
        ...
        // Check keyword overlap between task and playbook metadata
        if (taskProfile.keywords.length > 0 && playbook.keywords && playbook.keywords.length > 0) {
            const overlap = taskProfile.keywords.filter(taskKw =>
                playbook.keywords.some(playbookKw =>
                    playbookKw.includes(taskKw) || taskKw.includes(playbookKw)
                )
            );

            score.matches.keywords = overlap.length / taskProfile.keywords.length;
            if (overlap.length > 0) {
                score.reasons.push(`Keyword overlap: ${overlap.slice(0, 5).join(', ')}`);
            }
        }

        // Strong signal: direct token overlap with runbook filename/title
        const playbookNameTokens = playbook.name
            .toLowerCase()
            .replace(/^\d+[-_]?/, '')
            .split(/[-_]/)
            .map(t => t.trim())
        ...
        if (playbookNameTokens.length > 0 && taskProfile.keywords.length > 0) {
            const tokenMatches = playbookNameTokens.filter(token =>
                taskProfile.keywords.some(taskKw =>
                    taskKw.includes(token) || token.includes(taskKw)
                )
            );
            score.matches.name = tokenMatches.length / playbookNameTokens.length;

            if (tokenMatches.length > 0) {
                score.reasons.push(`Runbook name token match: ${tokenMatches.slice(0, 5).join(', ')}`);
            }
        }
