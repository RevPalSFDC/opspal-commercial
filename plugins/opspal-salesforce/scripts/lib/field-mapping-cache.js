...
// Live-first mode - skip cache by default, use only as fallback
// Set FIELD_MAPPING_LIVE_FIRST=false to use cache-first behavior (not recommended)
const LIVE_FIRST = process.env.GLOBAL_LIVE_FIRST !== 'false' &&
                   process.env.FIELD_MAPPING_LIVE_FIRST !== 'false';
...
class FieldMappingCache {
     ...
     */
    get(org, reportType, templateField, options = {}) {
    ...
    /**
     * Get cached mapping as fallback (bypasses live-first check)
     * Use this when API fails and you need cached data
     */
    getFallback(org, reportType, templateField) {
        return this.get(org, reportType, templateField, { useCacheFirst: true });
    }
    ...
    /**
     * Get all cached mappings as fallback (bypasses live-first check)
     */
    getAllFallback(org, reportType) {
        return this.getAll(org, reportType, { useCacheFirst: true });
    }
...
Examples:
  ...
  node field-mapping-cache.js clear acme-production
  node field-mapping-cache.js clear acme-production Opportunity
