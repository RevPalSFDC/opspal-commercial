 ...
 *
 * Intelligently resolves user input ("acme-corp production") to the correct
 * Salesforce org alias (acme-production vs acme-production).
 *
 * Features:
 ...
 *
 *   const result = await resolveOrgAlias('acme-corp production');
...
class InstanceAliasResolver {
     ...
     * Examples:
     *   "acme-corp production" → {org: "acme-corp", context: "production"}
     *   "delta-corp sandbox" → {org: "delta-corp", context: "sandbox"}
     *   "beta-corp" → {org: "beta-corp", context: null}
     */
...
if (require.main === module) {
    (async () => {
        ...
        if (args.length === 0) {
            ...
            console.log('  instance-alias-resolver.js "acme-corp production"');
            console.log('  instance-alias-resolver.js "delta-corp" --no-interactive');
            console.log('  instance-alias-resolver.js "beta-corp sandbox" --verbose');
            process.exit(1);
        }
