...
class PicklistDescriber {
    ...
    /**
     * Compare proposed StandardValueSet values against the current org state
     */
    async introspectBeforeDeploy(valueSetName, proposedValues = []) {
        const currentValueSet = await this.getStandardValueSet(valueSetName);
        const existing = (currentValueSet.values || []).map(value => ({
            fullName: value.fullName || value.value,
            label: value.label || value.fullName || value.value
        }));
        const existingMap = new Map(existing.map(value => [value.fullName, value]));
        ...
        for (const proposedValue of proposedValues) {
            const normalizedProposedValue = {
                fullName: proposedValue.fullName || proposedValue.value,
                label: proposedValue.label || proposedValue.fullName || proposedValue.value
            };
            const existingValue = existingMap.get(normalizedProposedValue.fullName);

            if (!existingValue) {
                toAdd.push(normalizedProposedValue);
                continue;
            }

            if (existingValue.label !== normalizedProposedValue.label) {
                conflicts.push({
            ...
            alreadyPresent.push(normalizedProposedValue);
        }
