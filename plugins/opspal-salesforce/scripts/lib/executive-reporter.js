...
class ExecutiveReporter {
     ...
     * Integrates with opspal-core PDF generator
     */
    async generatePDF(reportPaths, timestamp) {
        try {
            console.log('\n📄 Generating consolidated PDF report...');

            // Import PDF Generator from opspal-core
     ...
     * Supports webhook delivery, local sendmail, and queue-file fallback.
     */
    async emailReport(reportPaths, recipients) {
        ...
        if (recipientList.length === 0) {
            console.log('\n📧 No recipients provided - skipping email distribution');
            return { success: false, reason: 'no-recipients' };
        }

        const reportEntries = Object.entries(reportPaths)
        ...
        if (reportEntries.length === 0) {
            console.log('\n📧 No generated report files found - skipping email distribution');
            return { success: false, reason: 'no-files' };
        }

        const subject = `[Executive Compliance Report] ${new Date().toISOString().split('T')[0]}`;
        const bodyLines = [
            'Executive report package is ready.',
            '',
            `Project: ${path.basename(this.projectRoot)}`,
            `Reporting Period: ${this.getReportingPeriod()}`,
            '',
            'Generated files:',
            ...reportEntries.map((entry) => `- ${entry.type}: ${entry.path}`)
        ];
        const body = bodyLines.join('\n');

        const webhookUrl = process.env.EXEC_REPORT_EMAIL_WEBHOOK_URL ||
            process.env.EXEC_REPORT_DISTRIBUTION_WEBHOOK_URL;
        ...
        if (webhookUrl) {
            try {
                ...
                console.log(`\n📧 Report delivery requested via webhook for ${recipientList.length} recipient(s)`);
                return { success: true, method: 'webhook', recipients: recipientList.length };
            } catch (error) {
                console.log(`\n⚠️  Webhook delivery failed: ${error.message}`);
            }
        }

        try {
            execSync('command -v sendmail', { stdio: 'ignore' });
            ...
            for (const recipient of recipientList) {
                ...
                execFileSync('sendmail', ['-t', '-i'], {
                    input: message,
                    stdio: ['pipe', 'ignore', 'ignore']
                });
            }

            console.log(`\n📧 Report sent via local sendmail to ${recipientList.length} recipient(s)`);
            return { success: true, method: 'sendmail', recipients: recipientList.length };
        } catch (_) {
            // Fall through to queue-file mode
        }

        const queuePath = path.join(
            this.outputDir,
            `email-queue-${new Date().toISOString().replace(/[:.]/g, '-')}.json`
        );
        ...
        console.log('\n📧 Email delivery backend unavailable.');
        console.log(`   Queued payload at: ${queuePath}`);
        return { success: true, method: 'queue-file', queuePath };
    }
}
