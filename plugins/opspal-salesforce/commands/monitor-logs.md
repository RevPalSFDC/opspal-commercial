...
```bash
# Start monitoring
node scripts/lib/debug-log-monitor.js myorg --interval 5000

# Monitor errors only
node scripts/lib/debug-log-monitor.js myorg --errors-only

# Filter by user
node scripts/lib/debug-log-monitor.js myorg --user test@example.com
```

**JavaScript API**:
```javascript
const { DebugLogMonitor } = require('./scripts/lib/debug-log-monitor');
