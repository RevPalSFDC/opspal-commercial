...
async function runTests() {
    ...
    function applyPermissionStubs(escalator) {
        escalator.hasModifyAllDataPermission = async () => escalator.userInfo.profile === 'System Administrator';
        escalator.hasApexExecutionPermission = async () => escalator.userInfo.profile !== 'Guest User';
    }

    // Setup: Create test directory
    await fs.mkdir(testDir, { recursive: true });
    process.env.CLAUDE_LOG_DIR = testDir;

    // Stub Salesforce CLI-dependent methods to avoid external calls during tests
    FlowPermissionEscalator.prototype.getUserInfo = async function getUserInfoStub() {
        return {
    ...
    FlowPermissionEscalator.prototype.detectApexInvocation = async function detectApexStub() {
        return false;
    };
    ...
    await test('Check Modify All Data permission (System Administrator)', async () => {
        ...
        applyPermissionStubs(escalator);

        const hasPermission = await escalator.hasModifyAllDataPermission();
    ...
    await test('Check Apex execution permission (Standard User)', async () => {
        ...
        const hasPermission = await escalator.hasApexExecutionPermission();
        expect(hasPermission).toBe(true);
...
if (typeof describe !== 'undefined') {
  describe('Flow Permission Escalator', () => {
    ...
    }, 180000); // Increase timeout for complex tests
  });
}
