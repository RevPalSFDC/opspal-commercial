...
describe('FlowXMLValidator', () => {
  ...
  describe('Element Ordering', () => {
    it('should detect canonical top-level ordering issues and add one reorder fix', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow xmlns="http://soap.sforce.com/2006/04/metadata">
          <variables>
      ...
      const flowPath = path.join(tempDir, 'ordering.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);
      ...
      assert.strictEqual(result.fixes.filter(fix => fix.type === 'ELEMENT_ORDERING').length, 1);
    });

    it('should auto-fix canonical ordering while preserving XML declaration and unknown tags', () => {
      ...
      const flowPath = path.join(tempDir, 'ordering-fix.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);

      const autoFixValidator = new FlowXMLValidator({ verbose: false, autoFix: true });
      autoFixValidator.validateFlow(flowPath);

      const updatedXML = fs.readFileSync(flowPath, 'utf8');
      assert.ok(updatedXML.startsWith('<?xml version="1.0" encoding="UTF-8"?>'));
      assert.ok(updatedXML.includes('<customThing>'));
      assert.ok(updatedXML.includes('<!-- keep-comment -->'));
      assert.ok(updatedXML.indexOf('<decisions>') < updatedXML.indexOf('<variables>'));
      assert.ok(fs.existsSync(flowPath.replace('.flow-meta.xml', '.flow-meta.xml.bak')));
    });
  });
  ...
  describe('Structural Validation', () => {
    it('should detect non-contiguous top-level element ordering', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow>
          <decisions><name>Decision1</name></decisions>
          <assignments><name>Assignment1</name></assignments>
          <decisions><name>Decision2</name></decisions>
        </Flow>`;
      ...
      assert.ok(result.errors.some(e => e.type === 'ELEMENT_ORDERING'));
    });

    it('should detect top-level recordTriggerType placement', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow>
      ...
      const flowPath = path.join(tempDir, 'trigger-location.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);

      const result = validator.validateFlow(flowPath);

      assert.ok(result.errors.some(e => e.type === 'RECORD_TRIGGER_TYPE_LOCATION'));
    });

    it('should detect misplaced restricted nodes outside valueSet', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow>
          <variables>
      ...
      const flowPath = path.join(tempDir, 'restricted-location.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);

      const result = validator.validateFlow(flowPath);

      assert.ok(result.errors.some(e => e.type === 'RESTRICTED_VALUESET_LOCATION'));
    });

    it('should detect self-referencing connectors', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow>
          <assignments>
            <name>LoopBack</name>
            <connector>
              <targetReference>LoopBack</targetReference>
            </connector>
          </assignments>
        </Flow>`;

      const flowPath = path.join(tempDir, 'self-reference.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);

      const result = validator.validateFlow(flowPath);

      assert.ok(result.errors.some(e => e.type === 'SELF_REFERENCING_CONNECTOR'));
    });

    it('should detect unknown text template references', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow>
          <variables>
            <name>KnownVar</name>
          ...
          <textTemplates>
            ...
            <text>Hello {!MissingVar}</text>
          </textTemplates>
        </Flow>`;

      const flowPath = path.join(tempDir, 'text-template.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);

      const result = validator.validateFlow(flowPath);

      assert.ok(result.errors.some(e => e.type === 'TEXT_TEMPLATE_REFERENCE'));
    });

    it('should support JSON CLI output', () => {
      const flowXML = `<?xml version="1.0"?>
        <Flow>
          <recordTriggerType>Create</recordTriggerType>
        </Flow>`;

      const flowPath = path.join(tempDir, 'json-output.flow-meta.xml');
      fs.writeFileSync(flowPath, flowXML);

      try {
        execFileSync('node', [
          path.join(__dirname, '../scripts/lib/flow-xml-validator.js'),
          flowPath,
          '--json'
        ], { encoding: 'utf8' });
        assert.fail('Expected CLI to exit non-zero for invalid flow');
      } catch (error) {
