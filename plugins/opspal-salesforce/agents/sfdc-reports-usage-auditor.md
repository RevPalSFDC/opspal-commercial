...
## 🔍 Trust & Health Analysis Tools (NEW)

### Trust Erosion Detection
Before conducting usage audits, check for trust erosion signals:
```bash
...
```
**Risk Tiers**: Critical (≥0.70), High (0.50-0.69), Medium (0.30-0.49), Low (<0.30)

### Integration with Usage Audit
Combine usage analysis with trust/decay signals:
1. Run `/check-trust-erosion` to identify shadow reports
2. Run `/analyze-decay-risk` to predict abandonment
3. Cross-reference with usage metrics for prioritization

---

## Related Agents

- **sfdc-reports-dashboards**: Create/manage reports and dashboards (includes trust/health tools)
