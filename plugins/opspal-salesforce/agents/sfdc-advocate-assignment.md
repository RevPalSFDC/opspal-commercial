...
- **FuzzyMatcher**: Levenshtein distance matching with state/region validation
  ...
  - **USE FOR**: Organization/agency names (e.g., "CCC DA" → "Contra Costa County District Attorney")

- **PersonNameMatcher**: Component-level person name matching
  - Separate first name + last name matching (prevents false positives)
  - Nickname recognition (Jeff ↔ Jeffrey, Bill ↔ William)
  ...
  - **USE FOR**: Person names (e.g., matching advocate names to Salesforce users)
...
```bash
CONTEXT=$(node scripts/lib/runbook-context-extractor.js --org [org-alias] --operation-type advocate_assignment --format json)
```
...
./scripts/init-project.sh "deployment-{date}" "{org-alias}" --type deployment-directory
...
|---------------|-----|-----|
| Agency ↔ Account | **FuzzyMatcher** | Organization names need abbreviation expansion, state validation |
| Advocate ↔ User | **PersonNameMatcher** | Person names need component-level matching (first + last) |
| Region ↔ State | **FuzzyMatcher** | Geographic validation built-in |

### Person Name Matching (Advocates)
...
// Match advocate name from CSV to Salesforce user
const result = nameMatcher.match('Jeff Sudlow', 'Jeffrey Sudlow');
console.log(result);
// {
//   isMatch: true,
//   confidence: 92,
//   matchReason: 'Nickname match with high last name similarity',
//   breakdown: {
//     firstName: { similarity: 57, adjustedSimilarity: 95, nicknameMatch: true, passes: true },
//     lastName: { similarity: 100, passes: true },
//     overall: { similarity: 92, passes: true }
//   }
// }

// CORRECTLY rejects false positive
const falsePositive = nameMatcher.match('Jeffrey Sudlow', 'Jeffrey Spotts');
console.log(falsePositive);
// {
//   isMatch: false,
//   confidence: 53,
//   matchReason: 'Last name below threshold: 17% < 85%',
//   breakdown: {
//     firstName: { similarity: 100, passes: true },
//     lastName: { similarity: 17, passes: false },  // REJECTED
//     overall: { similarity: 50, passes: false }
//   }
// }

// Find best match from list of candidates
const bestMatch = nameMatcher.findBestMatch('Jeff S.', sfUsers, { nameField: 'Name' });
```

### Integration with User Analysis

When analyzing advocates from source CSV against Salesforce users:
...
const sfUsersData = fs.readFileSync(paths.data('salesforce-users.json'), 'utf-8');
const sfUsers = JSON.parse(sfUsersData).result.records;

// Match advocates from CSV to existing SF users
const advocateMatches = advocateNames.map(advocateName => {
    const match = nameMatcher.findBestMatch(advocateName, sfUsers, { nameField: 'Name' });
    return {
        csvName: advocateName,
        sfUser: match ? match.candidateName : null,
        sfUserId: match ? match.candidate.Id : null,
        confidence: match ? match.confidence : 0,
        isMatch: match !== null,
        needsCreation: match === null
    };
});

// Separate into matches vs needs creation
const existingUsers = advocateMatches.filter(m => m.isMatch);
const usersToCreate = advocateMatches.filter(m => m.needsCreation);
