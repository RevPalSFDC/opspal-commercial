...
**Load context:** `CONTEXT=$(node scripts/lib/runbook-context-extractor.js --org [org-alias] --operation-type csv_enrichment --format json)`
...
5. **Domain-Aware Matching (NEW)**
   - Industry-specific abbreviation expansion
   - Auto-detection of data domain from entity names
   - 85%+ match accuracy for domain-specific data
   - Graceful fallback when domain modules unavailable

---

## Domain-Aware Data Matching (NEW)

**Intelligent matching** with industry-specific abbreviation expansion for improved match accuracy during CSV enrichment.
...
Domain is **automatically detected** from CSV data when using FuzzyMatcher:
...
For known industry data, set domain explicitly for best results:
...
### Domain-Aware CSV Enrichment Workflow
...
// 2. Auto-detect domain from company names
const detector = new DomainDetector();
const companyNames = data.map(row => row.Name).filter(Boolean);
const detection = detector.detectDomain(companyNames);
console.log(`Detected domain: ${detection?.domain} (${(detection?.confidence * 100).toFixed(0)}%)`);

// 3. Initialize domain-aware normalizer
const normalizer = new NormalizationEngine({ domain: detection?.domain });

// 4. Normalize company names with domain context
const normalizedData = data.map(row => ({
  ...row,
  NormalizedName: normalizer.normalizeCompanyName(row.Name).normalized
}));

// 5. Use normalized names for fuzzy matching
const results = await kit.fuzzyMatch(
  normalizedData.map(r => r.NormalizedName),
  { entityType: 'Account', returnFields: ['Id', 'Name', 'OwnerId'] }
);
```
...
### Enhanced Abbreviations with Domain Context

When using domain-aware matching, abbreviations are expanded before fuzzy matching:

```javascript
// Without domain context (default abbreviations only)
const results1 = await kit.enrichCsvWithSalesforceIds(agencyNames, {
    entityType: 'Account',
    abbreviations: {
        'PD': 'Police Department',  // Manual configuration required
        'SO': 'Sheriff\'s Office'
    }
});

// With domain context (auto-loaded from domain dictionary)
const results2 = await kit.enrichCsvWithSalesforceIds(agencyNames, {
    entityType: 'Account',
    domain: 'government'  // Automatically loads 50+ abbreviations
});
```

### Benefits for CSV Enrichment
...
- **Reduces manual mapping** - domain dictionaries contain 50+ abbreviations per domain
