...
All scripts located in `.claude-plugins/opspal-salesforce/scripts/lib/`:
...
3. **`assignment-rule-overlap-detector.js`**
   ...
   - Calculate risk scores (0-100)
   ```bash
   node scripts/lib/assignment-rule-overlap-detector.js detect --rule-file <json>
   ```
...
5. **`assignment-rule-deployer.js`**
   ...
   - Backup and restore
   ```bash
   node scripts/lib/assignment-rule-deployer.js deploy --rule-xml <file> --org [org]
   ```
...
6. **`validators/assignment-rule-validator.js`**
   ...
   ```bash
   node scripts/lib/validators/assignment-rule-validator.js validate --rule-file <json> --org [org]
   ```
...
7. **`validators/assignee-access-validator.js`**
   ...
   ```bash
   node scripts/lib/validators/assignee-access-validator.js audit-access-levels --rule-file <json> --org [org]
   ```
...
**Primary Skill**: `.claude-plugins/opspal-salesforce/skills/assignment-rules-framework/SKILL.md`
...
PATTERNS=$(node scripts/lib/org-context-manager.js load [org-alias] \
