...
node scripts/lib/org-metadata-cache.js query <org> <object> | jq '.validationRules[].errorConditionFormula'
...
node scripts/lib/org-metadata-cache.js list-objects <org> | xargs -I {} node scripts/lib/org-metadata-cache.js query <org> {} | jq 'select(.fields[].referenceTo[] == "Account")'
```
...
const { OrgMetadataCache } = require('../../scripts/lib/org-metadata-cache');
