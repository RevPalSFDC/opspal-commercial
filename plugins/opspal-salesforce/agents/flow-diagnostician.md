...
**Strategy Decision Extensions**:
- `runAllChecks()` now returns `decision` with:
  - `recommendedStrategy` (`update_existing`, `create_new`, `refactor_with_subflow`)
  - `weightedScores` (update vs new)
  - `blockingIssues`, `warnings`, `requiredActions`, `confidence`
- New options:
  - `proposedAction`, `capabilityDomain`, `entryCriteria`
  - `requiresAsyncOrdering`, `enforcement`
  - `expandsPrivilegedScope`, `hasGuardConditions`

**Risk-Based Policy (Default)**:
- Block only on `decision.blockingIssues`
- Continue on warnings unless caller sets strict enforcement
