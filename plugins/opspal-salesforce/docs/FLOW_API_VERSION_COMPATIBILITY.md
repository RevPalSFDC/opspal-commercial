...
node scripts/lib/flow-api-version-validator.js validate \
...
node scripts/lib/flow-pattern-migrator.js migrate \
  force-app/main/default/flows/MyFlow.flow-meta.xml

# Batch migrate all flows
node scripts/lib/flow-pattern-migrator.js batch \
...
```javascript
const FlowAPIVersionValidator = require('./scripts/lib/flow-api-version-validator');
const FlowPatternMigrator = require('./scripts/lib/flow-pattern-migrator');
...
for flow in force-app/main/default/flows/*.flow-meta.xml; do
    ...
    node scripts/lib/flow-api-version-validator.js validate "$flow"

    if [ $? -ne 0 ]; then
        echo "❌ Validation failed for $flow"
        echo "Run migration: node scripts/lib/flow-pattern-migrator.js migrate $flow"
        exit 1
    fi
done
...
node scripts/lib/flow-api-version-validator.js validate "$FLOW_PATH"
if [ $? -ne 0 ]; then
...
node scripts/lib/flow-pattern-migrator.js migrate MyFlow.flow-meta.xml
...
node scripts/lib/flow-pattern-migrator.js batch "force-app/**/*.flow-meta.xml"
...
# Validate
node scripts/lib/flow-api-version-validator.js validate /tmp/test-flow.flow-meta.xml
...
node scripts/lib/flow-pattern-migrator.js migrate /tmp/test-flow.flow-meta.xml
