...
# 2. Validate quality
node scripts/lib/report-quality-validator.js --report-id 00O8c000000XXXX

# 3. Create a dashboard using a template
Ask agent: "Create a team pipeline dashboard using the manager template"

# 4. Optimize layout
node scripts/lib/dashboard-layout-optimizer.js --dashboard-id 01Z8c000000YYYY
```
...
node scripts/lib/chart-type-selector.js --help
```
...
```bash
# Basic usage
node scripts/lib/chart-type-selector.js --report-id 00O8c000000XXXX

# With audience context
node scripts/lib/chart-type-selector.js --report-id 00O8c000000XXXX --audience executive

# With position context (dashboard placement)
node scripts/lib/chart-type-selector.js --report-id 00O8c000000XXXX --position top-left

# JSON output
node scripts/lib/chart-type-selector.js --report-id 00O8c000000XXXX --output json
```
...
# With audience context
node scripts/lib/dashboard-layout-optimizer.js --dashboard-id 01Z8c000000YYYY --audience manager

# Dry-run mode (preview without applying)
node scripts/lib/dashboard-layout-optimizer.js --dashboard-id 01Z8c000000YYYY --dry-run

# JSON output
node scripts/lib/dashboard-layout-optimizer.js --dashboard-id 01Z8c000000YYYY --output json
```
...
```bash
# Basic usage
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000YYYY

# With threshold (fail if below score)
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000YYYY --threshold 70

# JSON output for automation
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000YYYY --output json

# Include recommendations
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000YYYY --recommendations
```
...
# With threshold
node scripts/lib/report-quality-validator.js --report-id 00O8c000000XXXX --threshold 70

# JSON output
node scripts/lib/report-quality-validator.js --report-id 00O8c000000XXXX --output json

# Include recommendations
node scripts/lib/report-quality-validator.js --report-id 00O8c000000XXXX --recommendations
```
...
8. 🆕 Run semantic parity gate (metadata + runtime evidence)
...
5. 🆕 Semantic parity gate (no critical drift)
```
...
for component in component_1 component_2 component_3 component_4 component_5 component_6; do
  node scripts/lib/chart-type-selector.js --component-id $component --audience executive
done
# Expected: Gauges for targets, Line for trends, Funnel for stages

# 3. Optimize layout for executive viewing
node scripts/lib/dashboard-layout-optimizer.js --dashboard-id 01Z8c000000ZZZZ --audience executive
# Places Quarterly Revenue vs Target (gauge) at top-left (highest importance)

# 4. Validate quality
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000ZZZZ
...
for dashboard_id in $(sf data query --query "SELECT Id FROM Dashboard" --json | jq -r '.result.records[].Id'); do
  node scripts/lib/dashboard-quality-validator.js --dashboard-id $dashboard_id --output json >> dashboard_audit.json
done
...
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000AAAA_REPLICA
...
for replica_dashboard in $(cat migration_log.json | jq -r '.replicas[].Id'); do
  node scripts/lib/dashboard-quality-validator.js --dashboard-id $replica_dashboard --output json >> replica_quality.json
done

# 4. Compare quality scores
node scripts/compare-migration-quality.js --original dashboards_to_migrate.json --replica replica_quality.json
...
ls .claude-plugins/opspal-salesforce/templates/

# Check if specific template exists
ls .claude-plugins/opspal-salesforce/templates/reports/marketing/lifecycle-funnel.json
```
...
ls -R .claude-plugins/opspal-salesforce/templates/
```
...
ls .claude-plugins/opspal-salesforce/scripts/lib/

# Check if specific script exists
ls .claude-plugins/opspal-salesforce/scripts/lib/chart-type-selector.js

# Check dependencies
cat .claude-plugins/opspal-salesforce/package.json | jq '.dependencies'
```
...
node scripts/lib/chart-type-selector.js --report-id 00O8c000000XXXX --verbose
...
```bash
# Before improvement
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000YYYY > before.json

# Make improvements
Agent: "Improve dashboard 01Z8c000000YYYY based on quality analysis"

# After improvement
node scripts/lib/dashboard-quality-validator.js --dashboard-id 01Z8c000000YYYY > after.json

# Calculate improvement
node scripts/compare-quality.js --before before.json --after after.json
# Shows: +12 points (B+ → A-)
```
...
for dashboard_id in $(sf data query --query "SELECT Id FROM Dashboard" --json | jq -r '.result.records[].Id'); do
  node scripts/lib/dashboard-quality-validator.js --dashboard-id $dashboard_id --output json >> monthly_audit_2025-10.json
done

# Identify quality trends
node scripts/analyze-quality-trends.js --input monthly_audit_2025-10.json
...
```bash
node scripts/lib/validate-template.js --template-path my-custom-template.json
```
...
**Usage:**
```bash
node scripts/lib/chart-type-selector.js --report-id 00O8c000000XXXX --industry financial-services
```
  ...
  quality-check:
    ...
    steps:
      ...
      - name: Validate Dashboard Quality
        run: |
          for dashboard_file in dashboards/*.json; do
            score=$(node scripts/lib/dashboard-quality-validator.js --dashboard-file $dashboard_file --output json | jq '.totalScore')
            if [ $score -lt 70 ]; then
      ...
      - name: Post Quality Report
        run: |
          node scripts/post-quality-report-to-pr.js
```
...
node scripts/lib/report-quality-validator.js --report-id [ID]

# Validate dashboard quality
node scripts/lib/dashboard-quality-validator.js --dashboard-id [ID]

# Get chart recommendations
node scripts/lib/chart-type-selector.js --report-id [ID]

# Optimize dashboard layout
node scripts/lib/dashboard-layout-optimizer.js --dashboard-id [ID]
