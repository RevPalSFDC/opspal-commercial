...
node scripts/auto-agent-router.js test

# Analyze a specific operation
node scripts/auto-agent-router.js analyze "update 500 opportunities"

# Route an operation (get JSON decision)
node scripts/auto-agent-router.js route "deploy to production" --json

# View analytics and statistics
node scripts/auto-agent-router.js report

# View configuration
node scripts/auto-agent-router.js config
```
...
```json
{
  "hooks": {
    "UserPromptSubmit": {
      "command": "bash ${CLAUDE_PLUGIN_ROOT}/.claude-plugins/opspal-salesforce/hooks/user-prompt-submit-enhanced.sh",
      "timeout": 5000
    }
  }
}
...
```bash
ls -lah .claude-plugins/opspal-salesforce/hooks/user-prompt-submit-enhanced.sh
```
...
```bash
echo '{"user_message":"deploy to production"}' | bash .claude-plugins/opspal-salesforce/hooks/user-prompt-submit-enhanced.sh
```
...
```bash
node scripts/auto-agent-router.js route "your operation here"
```

**Check 2**: Verify patterns match
```bash
node scripts/auto-agent-router.js config | jq '.triggers.mandatory.patterns'
```
...
```bash
node scripts/auto-agent-router.js init
```
