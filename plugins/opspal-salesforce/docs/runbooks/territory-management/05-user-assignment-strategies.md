...
> **SOQL Note**: COUNT(DISTINCT) and JOIN are not valid in SOQL.
> Use separate queries or Apex iteration for aggregation.

```bash
# Total territories
sf data query --query "SELECT COUNT(Id) FROM Territory2 WHERE Territory2ModelId = '<model_id>'"

# Territories with users (subquery)
sf data query --query "SELECT COUNT(Id) FROM Territory2 WHERE Territory2ModelId = '<model_id>' AND Id IN (SELECT Territory2Id FROM UserTerritory2Association)"

# Territories without users
sf data query --query "SELECT COUNT(Id) FROM Territory2 WHERE Territory2ModelId = '<model_id>' AND Id NOT IN (SELECT Territory2Id FROM UserTerritory2Association)"

# Total user assignments
sf data query --query "SELECT COUNT(Id) FROM UserTerritory2Association WHERE Territory2Id IN (SELECT Id FROM Territory2 WHERE Territory2ModelId = '<model_id>')"

# Unique users (GROUP BY then count results)
sf data query --query "SELECT UserId FROM UserTerritory2Association WHERE Territory2Id IN (SELECT Id FROM Territory2 WHERE Territory2ModelId = '<model_id>') GROUP BY UserId"
```
...
> **SOQL Note**: GROUP_CONCAT and JOIN are not valid in SOQL.
> Query territories and users separately, then aggregate in Apex/code.

```sql
-- Step 1: Get territories with user count
SELECT Territory2.Name, COUNT(Id)
FROM UserTerritory2Association
WHERE Territory2.Territory2ModelId = '<model_id>'
GROUP BY Territory2.Name
ORDER BY Territory2.Name

-- Step 2: For each territory, get user names
SELECT User.Name
FROM UserTerritory2Association
WHERE Territory2Id = '<territory_id>'
```
