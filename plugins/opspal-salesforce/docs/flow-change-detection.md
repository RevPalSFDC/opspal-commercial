...
```bash
node scripts/lib/flow-change-detector.js check path/to/MyFlow.flow-meta.xml
```

### Compare Flow Versions

```bash
node scripts/lib/flow-change-detector.js compare --old v1/MyFlow.flow-meta.xml --new v2/MyFlow.flow-meta.xml
```

### Check All Flows in a Directory

```bash
node scripts/lib/flow-change-detector.js directory force-app/main/default/flows/
```
