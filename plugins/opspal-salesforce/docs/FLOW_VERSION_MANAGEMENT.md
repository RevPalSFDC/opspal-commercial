...
node scripts/lib/flow-version-manager.js cleanupVersions \
  My_Flow \
  my-org \
  --keep 5 \
  --dry-run
...
```bash
# List all versions
node scripts/lib/flow-version-manager.js listVersions My_Flow my-org

# Get active version
node scripts/lib/flow-version-manager.js getActiveVersion My_Flow my-org

# Activate specific version
node scripts/lib/flow-version-manager.js activateVersion My_Flow 5 my-org

# Deactivate Flow (set to no active version)
node scripts/lib/flow-version-manager.js deactivateFlow My_Flow my-org

# Cleanup old versions
node scripts/lib/flow-version-manager.js cleanupVersions My_Flow my-org --keep 5
```
...
```bash
node scripts/lib/ooo-metadata-operations.js deployFlowSafe \
  My_Flow \
...
node scripts/lib/flow-version-manager.js activateVersion \
  Account_Record_Trigger \
  $(node scripts/lib/flow-version-manager.js getLatestVersion Account_Record_Trigger qa) \
  qa
...
node scripts/lib/flow-version-manager.js listVersions Account_Record_Trigger production
