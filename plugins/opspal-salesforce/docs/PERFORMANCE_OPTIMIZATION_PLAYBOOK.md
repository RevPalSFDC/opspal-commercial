...
1. **Run Profiler**:
   ```bash
   node scripts/lib/agent-profiler.js --agent sfdc-merge-orchestrator --profile
   ```
...
5. **Benchmark**:
   ```bash
   node scripts/lib/optimization-name.js benchmark
...
1. **Re-Profile** with Phase 1 optimization:
   ```bash
   node scripts/lib/agent-profiler.js --agent agent-name --profile
   ```
...
const OptimizationClass = require('../scripts/lib/optimization-class');
