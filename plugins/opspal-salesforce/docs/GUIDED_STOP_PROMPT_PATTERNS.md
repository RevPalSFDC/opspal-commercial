...
**Commands:**
```
node scripts/lib/picklist-dependency-manager.js
```
...
if [ ${#ERROR_DETAILS[@]} -gt 0 ]; then
    build_stop_prompt \
        ...
        --code "node scripts/validator.js --fix-all"
fi
```
...
if [ "$HAS_ERRORS" = true ]; then
    build_stop_prompt \
        ...
        --code "node scripts/lib/flow-pattern-migrator.js"
fi
```
...
--step "Re-run validation: node scripts/validator.js"
```
...
--code "node scripts/lib/validator.js --org production"
```
