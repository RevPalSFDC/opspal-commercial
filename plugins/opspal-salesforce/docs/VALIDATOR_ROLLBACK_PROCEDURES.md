...
cd .claude-plugins/opspal-salesforce/hooks
...
node scripts/analyze-validator-telemetry.js metadata-dependency-analyzer --since $(date -d '48 hours ago' +%Y-%m-%d)
```
...
3. Submit feedback: `node scripts/submit-validator-feedback.js metadata-dependency-analyzer`
...
node scripts/analyze-validator-telemetry.js metadata-dependency-analyzer
...
node scripts/analyze-validator-telemetry.js automation-feasibility-analyzer
...
while true; do
  node scripts/analyze-validator-telemetry.js [validator-name] --since $(date -d '4 hours ago' +%Y-%m-%d)
  sleep 14400  # 4 hours
done
...
FP_RATE=$(node scripts/analyze-validator-telemetry.js metadata-dependency-analyzer | grep "False Positive Rate" | awk '{print $4}' | tr -d '%')
...
AVG_TIME=$(node scripts/analyze-validator-telemetry.js flow-xml-validator | grep "Average Execution Time" | awk '{print $4}' | tr -d 'ms')
