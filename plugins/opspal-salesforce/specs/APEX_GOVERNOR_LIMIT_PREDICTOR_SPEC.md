...
```bash
# Command-line usage
node scripts/lib/apex-governor-limit-predictor.js AccountTriggerHandler.cls
...
```bash
# .claude-plugins/opspal-salesforce/hooks/pre-apex-deploy.sh
...
for file in $APEX_FILES; do
  node scripts/lib/apex-governor-limit-predictor.js "$file"
