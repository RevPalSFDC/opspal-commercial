...
- [Critical Architecture Change](#critical-architecture-change-new-merge-behavior-2025)
- [Canonical Record Selection](#canonical-record-selection-scoring-model)
- [Salesforce Integration Constraints](#salesforce-integration-constraints)
...
Duplicate CRM records degrade HubSpot performance and trust: they split activity history, corrupt attribution/analytics, confuse routing, and inflate reporting counts. This runbook provides RevPal's complete deduplication operating model: **detection → triage → safe merge → verification → prevention**.

**Key Deduplication Mechanisms:**
1. **Duplicate management tool** (native UI for contacts/companies)
2. **Duplicate alerts** (record creation warnings)
3. **Unique value properties** (up to 10 per object)
4. **Custom duplicate rules** (beta, Data Hub, 2 per object)
5. **Record merge** (UI + API, with critical 2025 architecture change)

**Critical Limitations:**
- ⚠️ **Merge creates NEW record ID** (effective Jan 2025 - MAJOR integration implication)
- ⚠️ **No unmerge capability** (irreversible operation)
- ⚠️ **Company merges blocked** when Salesforce integration enabled
- ⚠️ **250-merge participation limit** (hard constraint)
- ⚠️ **Unique-value rules NOT enforced** by forms/workflows/chatflows
...
- ✅ Detect and quantify duplicate records (contacts, companies, deals, tickets)
- ✅ Implement deterministic dedupe via unique IDs (external_id)
- ✅ Design canonical record selection algorithm (score-based)
- ✅ Perform safe merges with pre-merge normalization
- ✅ Handle Salesforce integration constraints (company merge restrictions)
- ✅ Implement fuzzy matching for no-email/no-domain populations
- ✅ Create "do not merge" guardrails (role emails, shared mailboxes)
- ✅ Respond to accidental merge incidents (recovery procedures)

**Common Scenarios:**
- "Merge duplicate contacts with same email"
- "Cannot merge companies - Salesforce integration enabled (workaround needed)"
- "Prevent API from creating duplicate companies by domain"
- "Accidental merge - need to separate records (unmerge mitigation)"
...
### Primary Agents
- **`hubspot-data-hygiene-specialist`** - Duplicate detection, cleanup strategy
- **`hubspot-workflow-builder`** - Remediation workflows, duplicate queues
- **`hubspot-custom-code-developer`** - Bulk dedupe jobs, fuzzy matching
- **`hubspot-integration-guardian`** - Integration duplication controls
- **`hubspot-security-governance-auditor`** - GDPR/legal basis handling

### Agent Usage Pattern

```javascript
// Agents should check for duplicates before operations:
const { findDuplicates } = require('./duplicate-scanner');
const dupes = await findDuplicates('contacts', 'email');
if (dupes.length > 0) {
  // Route to triage workflow or auto-merge (if safe)
}
```

---

## Critical Architecture Change: New Merge Behavior (2025)

**MOST IMPORTANT CHANGE**: HubSpot merge behavior changed effective January 14, 2025:
...
All Salesforce ↔ HubSpot sync integrations must:
1. Maintain ID mapping tables (old ID → new merged ID)
2. Re-resolve references after merge operations
3. NOT treat HubSpot object IDs as immutable

### Chain Merge Mechanics

When merging N duplicates into one:
1. Merge S1 into P0 → creates P1 (new ID)
2. Merge S2 into P1 → creates P2 (new ID)
3. Must update primaryId after each merge: `primaryId = response.id`

**See Diagram 5** in Appendix for visual representation.

---

## Canonical Record Selection (Scoring Model)

**Standardized approach to "which record should be primary"**:
...
**CRITICAL**: HubSpot blocks company merges when Salesforce integration is enabled.

### Workaround (Workflow 3)
1. **Do NOT attempt to merge companies** in HubSpot (operation will fail)
2. **Delete extra duplicate companies** in HubSpot, keeping company synced to primary Salesforce Account
3. **Resolve true duplicates in Salesforce** (merge/delete Accounts) to prevent re-creation
4. **Turn off auto-company creation** if producing duplicates
