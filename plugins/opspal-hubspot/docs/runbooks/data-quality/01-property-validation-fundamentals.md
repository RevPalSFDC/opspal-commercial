# Runbook 01: Property Validation Fundamentals (HubSpot)
...
Property validation in HubSpot ensures data quality and consistency across contacts, companies, deals, tickets, and custom objects. Unlike Salesforce validation rules (which can evaluate formulas across fields and block saves), HubSpot validation is primarily field-level enforcement with three mechanisms:

1. **Native property validation rules** (regex, unique values, required fields)
2. **Workflow-based validation** (post-save remediation)
3. **API-level validation** (programmatic enforcement)

**Critical Understanding**: HubSpot validation rules enforce at **UI and API save time** but are **NOT enforced by workflows, chatflows, or legacy forms**. This fundamental limitation requires a "defense in depth" approach with multiple validation layers.
...
- ✅ Enforce data quality standards during record creation
- ✅ Prevent invalid data entry (bad emails, malformed phone numbers)
- ✅ Implement conditional required fields ("If Country=USA, State required")
- ✅ Standardize data formats (phone normalization, email domain validation)
- ✅ Audit validation coverage across objects and properties
- ✅ Migrate Salesforce validation rules to HubSpot equivalents
- ✅ Design GDPR-compliant validation patterns

**Common Scenarios:**
- "Ensure all contacts have valid business email addresses"
- "Require phone number for MQL-stage contacts"
- "Validate US zip codes match state"
- "Prevent duplicate companies by domain"
...
### Primary Agents
- **`hubspot-data-hygiene-specialist`** - Data quality operations, validation coverage audits
- **`hubspot-property-manager`** - Property creation with validation rules
- **`hubspot-workflow-builder`** - Workflow-based validation patterns
- **`hubspot-import-manager`** - CSV validation before import
- **`hubspot-governance-enforcer`** - Validation policy compliance

### Agent Usage Pattern

```javascript
// Agents should check validation requirements before operations:
const { checkValidationCoverage } = require('./validation-audit');
const coverage = await checkValidationCoverage('contacts');
if (coverage.criticalFieldsUnvalidated > 0) {
  // Implement validations before proceeding
}
```

### Cross-Platform Integration

When validating data that syncs between Salesforce and HubSpot:

1. **Check Salesforce validation rules first**:
   ```bash
   sf data query --query "SELECT ValidationName, ErrorFormula FROM ValidationRule WHERE EntityDefinition.QualifiedApiName = 'Lead'" --use-tooling-api
   ```

2. **Mirror constraints in HubSpot** (property rules + workflows)

3. **Use unified-data-quality-validator agent** for cross-platform checks
