...
**CRITICAL**: Before ANY workflow PUT operation, you MUST follow this pattern:

### The Problem
HubSpot's workflow PUT endpoint uses **full-replace semantics**:
- The entire workflow definition is replaced with what you send
- Omitted actions are **permanently deleted**
- Concurrent updates can silently overwrite each other

### The Solution: revisionId

1. **GET** the current flow: `GET /automation/v4/flows/{flowId}`
2. **Extract** `revisionId` from the response
3. **Strip** system-managed fields: `createdAt`, `updatedAt`, `dataSources`, `portalId`, `insertedAt`
4. **Apply** your modifications to the full payload
5. **PUT** with `revisionId` and `type` included: `PUT /automation/v4/flows/{flowId}`

### Helper Script

Use `scripts/lib/workflow-safe-update-helper.js`:

```javascript
const helper = new WorkflowSafeUpdateHelper(accessToken);

// Safe update (handles GET → sanitize → PUT automatically)
const result = await helper.safeUpdate(flowId, { name: 'Updated Name' });

// Clone workflow (GET → strip all system fields → POST)
const clone = await helper.cloneFlow(sourceFlowId, 'Cloned Workflow');
```

### Fields to Strip Before PUT
- `id` (for clones only)
- `createdAt`
- `updatedAt`
- `dataSources`
...
- `updatedBy`
- `createdBy`
- `revisionId` (for clones only — KEEP for updates)

### Workflow Clone Pattern

HubSpot has no native clone endpoint. The pattern is:
1. `GET /automation/v4/flows/{sourceFlowId}`
2. Strip ALL system fields (including `id` and `revisionId`)
3. Set new `name`
4. `POST /automation/v4/flows` with sanitized payload
...
**Workaround:** Implement pre-flight validation (see [validate-workflow-config.js](../../scripts/workflows/README.md#validate-workflow-configjs-planned))
...
- Agent: `.claude-plugins/hubspot-core-plugin/agents/hubspot-workflow-auditor.md`
- Library: `.claude-plugins/hubspot-core-plugin/scripts/lib/hubspot-workflow-auditor.js`
- Tests: `.claude-plugins/hubspot-core-plugin/scripts/lib/__tests__/hubspot-workflow-auditor.test.js`
...
node scripts/workflows/validate-workflow-config.js {workflowId}
```
