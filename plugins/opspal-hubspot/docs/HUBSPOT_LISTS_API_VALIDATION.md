...
```javascript
const HubSpotFilterBuilder = require('./scripts/lib/hubspot-filter-builder');
...
```javascript
const HubSpotOperatorTranslator = require('./scripts/lib/hubspot-operator-translator');
...
```javascript
const HubSpotAssociationMapper = require('./scripts/lib/hubspot-association-mapper');
...
```javascript
const HubSpotListsAPIValidator = require('./scripts/lib/hubspot-lists-api-validator');
...
node scripts/lib/hubspot-association-mapper.js validate contacts companies 279
# Output: ✅ Correct association ID (279)

# List all associations for object
node scripts/lib/hubspot-association-mapper.js list contacts
```
...
node scripts/lib/hubspot-operator-translator.js build ">=" "100" "number"

# List all operators
node scripts/lib/hubspot-operator-translator.js list
```
...
node scripts/lib/hubspot-filter-builder.js complex '[{"property":"industry","operator":"=","values":["Tech"]}]'

# Validate filter
node scripts/lib/hubspot-filter-builder.js validate '<filter-json>'
```
...
node scripts/lib/hubspot-lists-api-validator.js validate '<request-json>' '{"objectType":"contacts"}'
```
...
**File**: `.claude-plugins/opspal-hubspot/config/hubspot-association-ids.json`
...
**File**: `.claude-plugins/opspal-hubspot/config/hubspot-operator-mappings.json`
...
# Test filter builder
node scripts/lib/hubspot-filter-builder.js simple industry "=" Tech
```
...
- [Quality Gate Framework](../../opspal-core/docs/QUALITY_GATE_FRAMEWORK.md)
