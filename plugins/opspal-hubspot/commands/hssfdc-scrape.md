...
```bash
HEAD=1 PORTAL={{portal-name}} node scripts/scrape-sfdc-sync-settings.js
```
...
1. **"No saved session found"**
   - Run authentication setup: `HEAD=1 PORTAL={portal} node scripts/scrape-sfdc-sync-settings.js`
