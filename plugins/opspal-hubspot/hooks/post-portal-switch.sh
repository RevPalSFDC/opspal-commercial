...
# Use fd 3 if you ever need to send structured JSON back to the hook system.
exec 3>&1 1>&2
...
echo -e "${COLOR_BLUE}🔄 Post-Portal Switch Hook${COLOR_NC}"
...
if [ -f .env ]; then
    ...
    echo -e "${COLOR_GREEN}✓${COLOR_NC} Switched to portal: ${PORTAL_NAME} (ID: ${PORTAL_ID})"
else
    echo -e "${COLOR_YELLOW}⚠${COLOR_NC}  Warning: .env file not found"
    exit 0
fi

# Check if MCP configuration exists
if [ ! -f .mcp.json ]; then
    echo -e "${COLOR_YELLOW}⚠${COLOR_NC}  No .mcp.json found, skipping MCP server checks"
    exit 0
fi

# Notify user about MCP environment reload
echo ""
echo -e "${COLOR_YELLOW}⚠${COLOR_NC}  Important: MCP servers may have cached old credentials"
echo -e "   ${COLOR_BLUE}Action Required:${COLOR_NC}"
echo -e "   1. Restart Claude Code to reload MCP servers, OR"
echo -e "   2. Use direct scripts instead of MCP tools for next operation"
echo ""
echo -e "   ${COLOR_GREEN}✓${COLOR_NC} Recommended: Use ./scripts/run-with-env.sh for scripts"
echo -e "   ${COLOR_GREEN}✓${COLOR_NC} Recommended: Restart Claude Code for MCP tool access"
echo ""
...
if ! echo "[$TIMESTAMP] Portal switched to: $PORTAL_NAME (ID: $PORTAL_ID)" >> "$LOG_FILE" 2>/dev/null; then
    echo -e "${COLOR_YELLOW}⚠${COLOR_NC}  Unable to write portal switch log: $LOG_FILE"
fi
...
echo -e "${COLOR_GREEN}✓${COLOR_NC} Portal switch hook completed"
echo ""
