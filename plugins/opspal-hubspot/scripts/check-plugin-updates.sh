#!/bin/bash
# SessionStart Hook: Check for HubSpot Plugin Updates
# Detects updates for the hubspot-plugin specifically from the marketplace repository
...
echo ""
echo "⚠️  HubSpot Plugin update available: v$LOCAL_VERSION → v$REMOTE_VERSION"
echo ""

# Show recent changes from git log
RECENT_CHANGES=$(git -C "$MARKETPLACE_ROOT" log HEAD..origin/main --oneline --no-decorate --grep="hubspot" 2>/dev/null | head -5)
if [ -z "$RECENT_CHANGES" ]; then
    # Fallback to all recent changes if no hubspot-specific commits
