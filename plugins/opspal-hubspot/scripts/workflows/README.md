...
**Usage:**
```bash
node scripts/workflows/validate-workflow-config.js <workflow-id> [--portal-id=12345]
```
...
**Usage:**
```bash
node scripts/workflows/cleanup-failed-modifications.js [--dry-run] [--older-than=7d]
```
 ...
 *
 * Usage: node scripts/workflows/[script-name].js <workflow-id> [options]
 *
 * Exit Codes:
 *   0: Success
...
node scripts/workflows/add-static-branch.js <workflow-id> --property=lifecyclestage

# Option 2: UI Automation (for complex LIST_BRANCH)
node scripts/workflows/add-list-branch-ui.js <workflow-id> --conditions=conditions.json
```
...
node scripts/workflows/validate-workflow-config.js <workflow-id>
```
...
```bash
# API-based, fast
node scripts/workflows/update-pipeline-stage.js <workflow-id> --stage-id=1234567
```
...
```bash
# Export structure
node scripts/workflows/export-workflow.js <source-workflow-id> > workflow-template.json

# Import to new portal
node scripts/workflows/import-workflow.js workflow-template.json --portal-id=98765
```
...
**Solution:** Run cleanup script: `node scripts/workflows/cleanup-failed-modifications.js`
