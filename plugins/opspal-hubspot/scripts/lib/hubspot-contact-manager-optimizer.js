...
class HubSpotContactManagerOptimizer {
  constructor(options = {}) {
    ...
    this.batchMetadata = BatchPropertyMetadata.withCache({
      ...
      resilientClient: this.resilientClient,
  ...
  _updateStats(duration) {
    ...
    this.stats.retriedRequests = this.stats.batchMetadataStats.retriedRequests || 0;
  }
   ...
   */
  getStats() {
    const batchMetadataStats = this.batchMetadata.getStats();
    return {
      ...this.stats,
      retriedRequests: batchMetadataStats.retriedRequests || 0,
      batchMetadataStats
    };
  }
