...
const LOCK_RETRY_DELAY = 2000; // Minimum 2s wait for 423 Locked (HubSpot-documented)
const MAX_LOCK_RETRIES = 3;    // 423 locks should be transient
...
class BatchUpdateWrapper {
   ...
   * C2: Injects objectWriteTraceId for per-record traceability
   */
  ...
  /**
   * Extract and log correlation ID from response headers (H1)
   */
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (response.status === 207) {
          ...
          const parsed = this.parseMultiStatusResponse(result, payload, batchIndex, operation, correlationId);
        ...
        if (!response.ok) {
          ...
          if (response.status === 423) {
            ...
            if (lockRetries <= MAX_LOCK_RETRIES) {
              ...
              continue; // Don't count against normal retry attempts
            }
        ...
        if (this.verbose) {
          console.log(`✓ Batch ${batchIndex} (${operation}): ${payload.inputs.length} records - Success [correlationId: ${correlationId}]`);
        }
  ...
  /**
   * C1: Parse 207 Multi-Status response to identify per-record successes and failures.
   * Maps objectWriteTraceId back to source records for correlation (C2).
   */
  parseMultiStatusResponse(result, payload, batchIndex, operation, correlationId) {
    const succeeded = [];
    const failed = [];
    ...
    // Map failed records from errors array
    ...
    if (this.verbose) {
      ...
      for (const f of failed) {
        console.log(`  ✗ traceId=${f.objectWriteTraceId}: ${f.message} (${f.category})`);
      }
    }
  ...
  /**
   * Aggregate batch results (C1: handles partial/207 results)
   */
    ...
    results.forEach(result => {
      if (result.status === 'fulfilled') {
        ...
        if (value.status === 'success') {
          ...
          // C1: 207 Multi-Status — some records succeeded, some failed
          summary.partial++;
          summary.results.push(value.data);
