...
// Live-first mode - analyze pages fresh first, use cache only as fallback
...
class BatchAnalyzer {
   ...
   */
  async analyzePage(url, checks, useCache = true, options = {}) {
    const { useCacheFirst = false } = options;
    ...
    try {
      ...
      if (this.liveFirst && useCache) {
        const fallback = this.getCachedResult(url, checks);
        if (fallback) {
          console.warn(`[BatchAnalyzer] Analysis failed for ${url}, using cached data`);
          return { ...fallback, _fromCache: true, _cacheWarning: `Live analysis failed: ${error.message}` };
        }
      }
  ...
  /**
   * Analyze page with cache-first fallback (bypasses live-first check)
   */
  async analyzePageFallback(url, checks, useCache = true) {
    return this.analyzePage(url, checks, useCache, { useCacheFirst: true });
  }
