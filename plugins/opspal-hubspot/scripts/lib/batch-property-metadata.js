...
// Live-first mode - query live API first, use cache only as fallback
// Set HS_PROPERTY_LIVE_FIRST=false to use cache-first behavior (not recommended)
const LIVE_FIRST = process.env.GLOBAL_LIVE_FIRST !== 'false' &&
                   process.env.HS_PROPERTY_LIVE_FIRST !== 'false';

class LRUCache {
  constructor(maxSize = DEFAULT_CACHE_SIZE, ttl = DEFAULT_CACHE_TTL, options = {}) {
  ...
  /**
   * Get value from cache as fallback (bypasses live-first check)
   * Use this when API fails and you need cached data
   */
  getFallback(key) {
    return this.get(key, { useCacheFirst: true });
  }
...
class BatchPropertyMetadata {
  constructor(options = {}) {
    ...
    this.liveFirst = options.liveFirst !== undefined ? options.liveFirst : LIVE_FIRST;
    this.resilientClient = options.resilientClient || null;
    this.requestFn = typeof options.requestFn === 'function' ? options.requestFn : null;
    this.cache = new LRUCache(
      options.cacheSize || DEFAULT_CACHE_SIZE,
      options.cacheTtl || DEFAULT_CACHE_TTL,
      { liveFirst: this.liveFirst }
    );
    ...
    this.transportMode = this.resilientClient
      ? 'resilient-client'
      : this.requestFn
      ? 'request-fn'
      : 'fetch';
    this.externalOnRetry = typeof options.onRetry === 'function' ? options.onRetry : null;

    if (this.resilientClient) {
      const existingOnRetry = this.resilientClient.config?.onRetry;
      this.resilientClient.config = this.resilientClient.config || {};
      this.resilientClient.config.onRetry = (meta) => {
        this._recordRetry(meta);

        if (typeof existingOnRetry === 'function' && existingOnRetry !== this.externalOnRetry) {
          existingOnRetry(meta);
        }
      };
    }
  }
      ...
      for (const [objectType, typeKeys] of Object.entries(groupedKeys)) {
        if (typeKeys[0].fetchAllProperties) {
          ...
          for (const key of typeKeys) {
            const cacheKey = this._getCacheKey(key);
            const record = this._createMetadataRecord(objectType, key, properties);
            this.cache.set(cacheKey, record);
            fetchedResults.push(record);
          }
        } else {
          ...
          for (const batch of batches) {
            ...
            for (let i = 0; i < batch.length; i++) {
              ...
              const record = this._createMetadataRecord(
                objectType,
                batch[i],
                batchResults[i]?.properties || batchResults[i],
                batch[i].id
              );
              this.cache.set(cacheKey, record);
              fetchedResults.push(record);
  ...
  /**
   * Request JSON using the configured transport.
   */
  async _requestJson(url, options, requestMeta = {}) {
    if (this.requestFn) {
      const result = await this.requestFn(url, options, requestMeta);
      return this._normalizeTransportResult(result, url);
    }

    if (this.resilientClient) {
      const result = await this.resilientClient.request(url, {
    ...
    return this._rateLimitedFetch(url, options, requestMeta);
  }

  /**
   * Rate-limited fetch with local retry logic (respects HubSpot's 100 req/10s limit)
   */
  async _rateLimitedFetch(url, options, requestMeta = {}, retries = DEFAULT_FETCH_RETRIES) {
  ...
  _createMetadataRecord(objectType, key, properties, id = key.id) {
    return {
      id,
      objectType,
  ...
  async _normalizeTransportResult(result, url) {
    if (result && typeof result.json === 'function') {
      if (result.ok === false) {
        throw new Error(`Request failed: ${result.status} ${result.statusText || ''}`.trim());
      }
      return result.json();
    }
    ...
    if (result === null || result === undefined) {
      throw new Error(`Transport returned no data for ${url}`);
    }
  ...
  _recordRetry(meta) {
    ...
    if (this.externalOnRetry) {
      this.externalOnRetry(meta);
    }
  }
