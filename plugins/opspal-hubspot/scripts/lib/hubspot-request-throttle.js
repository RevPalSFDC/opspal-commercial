...
class HubSpotRequestThrottle {
  constructor(options = {}) {
    ...
    // Concurrency limits by tier — canonical values from config/hubspot-rate-limits.json
    this.tierLimits = {
      free: { maxConcurrent: 8, requestsPerWindow: 100 },
    ...
    // Daily rate limit tracking (Item #8)
    const rateLimitsConfig = require('../../config/hubspot-rate-limits.json');
    const tierConfig = rateLimitsConfig.tiers[this.tier] || rateLimitsConfig.tiers.starter;
    this.dailyLimit = tierConfig.dailyLimit; // null for oauth_app
    this.dailyRequestCount = 0;
    this.dailyResetDate = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
    this.dailyWarningThresholds = [0.7, 0.85, 0.95]; // Warn at 70%, 85%, 95%
    this.dailyWarningsEmitted = new Set();
  ...
  async executeRequest(item) {
    try {
      ...
      // H1: Extract and log correlationId from every response
      if (response && response.headers) {
        ...
        if (this.verbose) {
          this.log(`Response: ${formatForLog(rateLimitInfo)}${correlationId ? ` [correlationId: ${correlationId}]` : ''}`);
        }
      ...
      } else if (error.status === 423 || error.message?.includes('423')) {
        this.log(`423 Locked, re-queuing with 2s delay`);
        await this.delay(2000); // HubSpot-documented minimum lock duration
  ...
  /**
   * Track daily API usage and warn when approaching limits
   * @private
   */
  _trackDailyUsage() {
    ...
    if (today !== this.dailyResetDate) {
      ...
      this.dailyWarningsEmitted.clear();
    }

    this.dailyRequestCount++;
    this.stats.dailyRequestCount = this.dailyRequestCount;

    if (this.dailyLimit) {
      const utilization = this.dailyRequestCount / this.dailyLimit;
      this.stats.dailyUtilization = (utilization * 100).toFixed(1) + '%';

      for (const threshold of this.dailyWarningThresholds) {
        if (utilization >= threshold && !this.dailyWarningsEmitted.has(threshold)) {
          this.dailyWarningsEmitted.add(threshold);
          const pct = (threshold * 100).toFixed(0);
          const remaining = this.dailyLimit - this.dailyRequestCount;
          this.log(`⚠ Daily rate limit ${pct}% reached: ${this.dailyRequestCount}/${this.dailyLimit} (${remaining} remaining)`);
        }
      }
    }
  }
  ...
  getStatus() {
    return {
      ...
      lastCorrelationId: this.stats.lastCorrelationId,
      daily: {
        requestCount: this.dailyRequestCount,
        limit: this.dailyLimit,
        utilization: this.stats.dailyUtilization,
        remaining: this.dailyLimit ? this.dailyLimit - this.dailyRequestCount : null,
  ...
  /**
   * H1: Get the last correlation ID for debugging/support escalation
   */
  getLastCorrelationId() {
    return this.stats.lastCorrelationId;
  }
