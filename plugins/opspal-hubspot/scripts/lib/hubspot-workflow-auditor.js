...
class HubSpotWorkflowAuditor {
  ...
  _validateSequence(http_log) {
    ...
    // C3: Validate revisionId safe-update pattern
    // PUT to /automation/v4/flows/{flowId} MUST include revisionId to prevent concurrent overwrites
    const putUpdates = http_log.filter(entry =>
      entry.url.includes('/automation/v4/flows/') &&
      entry.verb === 'PUT' &&
      !entry.url.endsWith('/flows') // Exclude batch/create endpoints
    );

    for (const putEntry of putUpdates) {
      const body = putEntry.request_body;
      if (body && !body.revisionId) {
        findings.push({
          priority: 'P1',
          category: 'Sequence',
          description: 'CRITICAL: Workflow PUT update missing revisionId — concurrent overwrites possible',
          evidence: `PUT ${putEntry.url} has no revisionId in request body. ` +
            'HubSpot uses full-replace semantics on PUT: omitted actions ARE REMOVED. ' +
            'Always GET the current flow first to obtain revisionId, then include it in the PUT payload.'
        });
      }

      // Also check that a GET preceded this PUT (to fetch the current revisionId)
      const putIndex = http_log.indexOf(putEntry);
      const flowId = this._extractFlowIdFromUrl(putEntry.url);
      const precedingGet = http_log.slice(0, putIndex).find(entry =>
        entry.verb === 'GET' &&
        entry.url.includes('/automation/v4/flows/') &&
        flowId && entry.url.includes(flowId)
      );
      ...
      if (!precedingGet) {
        findings.push({
          priority: 'P2',
          category: 'Sequence',
          description: 'Workflow PUT update without preceding GET — revisionId may be stale or missing',
          evidence: `PUT ${putEntry.url} was not preceded by a GET to fetch current flow state and revisionId`
        });
      }
    }
