...
// Live-first mode - fetch sitemap fresh first, use cache only as fallback
...
class SitemapCrawler {
  ...
  async parseSitemap(sitemapUrl, options = {}) {
    ...
    // Check cache first only if not in live-first mode OR useCacheFirst is true
    ...
    try {
      ...
      if (this.liveFirst && useCache) {
        const fallback = this.getCachedSitemap(sitemapUrl);
        if (fallback) {
          console.warn(`[SitemapCrawler] Fetch failed, using cached data from ${this.getCacheAge(sitemapUrl)}`);
          return { ...fallback, _fromCache: true, _cacheWarning: `Live fetch failed: ${error.message}` };
        }
      }
  ...
  /**
   * Parse sitemap with cache-first fallback (bypasses live-first check)
   * Use this when you need cached data regardless of live-first setting
   */
  async parseSitemapFallback(sitemapUrl, options = {}) {
    return this.parseSitemap(sitemapUrl, { ...options, useCacheFirst: true });
  }
