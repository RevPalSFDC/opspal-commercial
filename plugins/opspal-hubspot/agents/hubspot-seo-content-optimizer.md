...
```bash
node scripts/lib/seo-content-scorer.js <url-or-file> --keyword "target keyword"
node scripts/lib/seo-content-scorer.js ./content.html --format html --output scores.json
```

**AEO Optimizer** (`scripts/lib/seo-aeo-optimizer.js`):
```bash
node scripts/lib/seo-aeo-optimizer.js <url-or-file> --keyword "target keyword"
node scripts/lib/seo-aeo-optimizer.js ./content.md --format markdown --analyze-paa
```

**Readability Analyzer** (`scripts/lib/seo-readability-analyzer.js`):
```bash
node scripts/lib/seo-readability-analyzer.js <url-or-file>
node scripts/lib/seo-readability-analyzer.js ./content.md --format markdown
```

**Content Recommender** (`scripts/lib/seo-content-recommender.js`):
```bash
node scripts/lib/seo-content-recommender.js --gap-analysis ./gaps.json
node scripts/lib/seo-content-recommender.js --keywords ./keywords.json --crawl ./crawl.json
```

**Internal Linking Suggestor** (`scripts/lib/seo-internal-linking-suggestor.js`):
```bash
node scripts/lib/seo-internal-linking-suggestor.js ./crawl-results.json
node scripts/lib/seo-internal-linking-suggestor.js ./crawl.json --output linking.json
```
...
node scripts/lib/seo-content-scorer.js ./crawl-results.json --output ./content-optimization/content-scores.json
node scripts/lib/seo-readability-analyzer.js ./crawl-results.json --output ./content-optimization/readability-report.json
node scripts/lib/seo-aeo-optimizer.js ./crawl-results.json --output ./content-optimization/aeo-opportunities.json
node scripts/lib/seo-internal-linking-suggestor.js ./crawl-results.json --output ./content-optimization/internal-linking.json

# Step 3: Recommendations
node scripts/lib/seo-content-recommender.js --crawl ./crawl-results.json --content-scores ./content-optimization/content-scores.json --output ./content-optimization/content-recommendations.json
...
```bash
# Analyze single page
node scripts/lib/seo-content-scorer.js https://example.com/blog/post --keyword "target keyword"
node scripts/lib/seo-readability-analyzer.js https://example.com/blog/post
node scripts/lib/seo-aeo-optimizer.js https://example.com/blog/post --keyword "target keyword" --analyze-paa
...
node scripts/lib/seo-keyword-researcher.js "revenue operations" --output keywords.json

# Step 2: Analyze existing content (if site crawl available)
node scripts/lib/seo-content-recommender.js --keywords keywords.json --crawl ./crawl-results.json --output content-recommendations.json
...
(
  node scripts/lib/seo-content-scorer.js ./crawl.json --output scores.json &
  node scripts/lib/seo-readability-analyzer.js ./crawl.json --output readability.json &
  node scripts/lib/seo-aeo-optimizer.js ./crawl.json --output aeo.json &
  node scripts/lib/seo-internal-linking-suggestor.js ./crawl.json --output linking.json &
  wait
)
```
...
node scripts/lib/seo-keyword-researcher.js "keyword" --output keywords.json
node scripts/lib/seo-content-recommender.js --keywords keywords.json --crawl crawl.json --output recs.json
```
...
- **Solution**: Verify working directory is plugin root (`.claude-plugins/opspal-hubspot`)
