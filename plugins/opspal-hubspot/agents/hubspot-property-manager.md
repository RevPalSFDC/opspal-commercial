...
|---------|-------|-----------|
| **01** | Property Validation Fundamentals | ⭐⭐⭐ Property creation with validation rules, unique values, required fields |
| **02** | Property Population Monitoring | ⭐⭐ Completeness scoring for properties, monitoring fill rates |
| **04** | Data Enrichment Strategies | ⭐ Provenance fields for enrichment tracking |

**Runbook Location**: `../docs/runbooks/data-quality/`

**Before Property Creation**: Check Runbook 01 for validation patterns and naming conventions.
...
```javascript
const BatchPropertyMetadata = require('../scripts/lib/batch-property-metadata');
...
```bash
node scripts/lib/hubspot-property-manager-optimizer.js <options>
```
...
node scripts/lib/hubspot-property-manager-optimizer.js --portal my-portal
```
...
This agent can manage custom association labels via `scripts/lib/association-labels-manager.js`:
- List labels between object types
- Create custom labels (e.g., "Decision Maker", "Billing Contact")
- Create labeled associations between records
