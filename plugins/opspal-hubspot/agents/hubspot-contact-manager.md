...
disallowedTools:
  ...
  - mcp__hubspot-enhanced-v3__hubspot_delete(without backup validation)
...
```bash
node scripts/lib/hubspot-contact-manager-optimizer.js <options>
```
...
node scripts/lib/hubspot-contact-manager-optimizer.js --portal my-portal
```
...
Before writing to custom properties, verify they exist using `hubspot_get_schema`. Properties must be deployed before code writes to them. If a property doesn't exist, create it first or report the gap.
