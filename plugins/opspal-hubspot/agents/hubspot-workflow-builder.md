...
|---------|-------|-----------|
| **01** | Property Validation Fundamentals | ⭐⭐⭐ Workflow-based validation (post-save remediation) |
| **02** | Property Population Monitoring | ⭐⭐⭐ Remediation workflows, completeness-gated automation |
| **03** | Integration Health Checks | ⭐⭐ Circuit breaker workflows (protect automation during failures) |
| **04** | Data Enrichment Strategies | ⭐⭐ Workflow-based enrichment ("Enrich record" action) |
| **05** | Duplicate Detection & Deduplication | ⭐⭐ Duplicate triage queues, merge remediation workflows |

**Runbook Location**: `../docs/runbooks/data-quality/`

**Before Workflow Creation**: Check relevant runbooks for automation patterns and known limitations.

---

## 🔗 List-Workflow Pairing Validator (Reflection-Driven)

**ALWAYS validate list-workflow pairings** to prevent orphaned configurations:

```bash
# Validate a single workflow's list references
node scripts/lib/list-workflow-pairing-validator.js validate-workflow workflow.json lists.json

# Find orphaned lists (not used by any workflow)
node scripts/lib/list-workflow-pairing-validator.js find-orphans workflows.json lists.json

# Generate comprehensive pairing report
node scripts/lib/list-workflow-pairing-validator.js report workflows.json lists.json
```

**Detects**:
- Missing/deleted list references
- Archived list references
- Static list enrollment (may cause enrollment gaps)
- Circular list operations (enrollment + add to same list)
- Orphaned lists not used by workflows
- Orphaned workflows with deleted enrollment lists

**The `pre-workflow-list-validation.sh` hook automatically validates** - this section is for manual validation and troubleshooting.
...
```bash
node scripts/lib/hubspot-workflow-builder-optimizer.js <options>
```
...
node scripts/lib/hubspot-workflow-builder-optimizer.js --portal my-portal
```
...
| Create custom workflow actions | Action definition scope | Use `hubspot-custom-action-builder` |
| Complete async callbacks | Callback scope | Use `hubspot-callback-orchestrator` |
...
| Create custom workflow actions | `hubspot-custom-action-builder` | V4 Action API specialization |
| Handle async callback completion | `hubspot-callback-orchestrator` | Callback management |
...
- "Create a custom workflow action" → Route to `hubspot-custom-action-builder`
- "Complete a workflow callback" → Route to `hubspot-callback-orchestrator`
...
### revisionId Safe-Update Pattern (MANDATORY)

Before ANY PUT to `/automation/v4/flows/{flowId}`:
1. **GET** the current flow to capture `revisionId`
2. **Strip** system fields: `createdAt`, `updatedAt`, `dataSources`, `portalId`, `insertedAt`
3. **Include** `revisionId` and `type` in the PUT payload
4. **WARNING**: HubSpot uses full-replace semantics — omitted actions ARE REMOVED

Use `scripts/lib/workflow-safe-update-helper.js` for safe updates and cloning.

### Workflow Clone Pattern

HubSpot has no native clone endpoint. Use GET → sanitize → POST:
```javascript
const helper = new WorkflowSafeUpdateHelper(accessToken);
const clone = await helper.cloneFlow(sourceFlowId, 'Cloned Workflow');
```
Strip these additional fields for clones: `id`, `revisionId`
