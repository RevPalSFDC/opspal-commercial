...
tools:
  ...
  - Grep
disallowedTools:
  # Direct deletion blocked - use two-phase commit pattern
...
| Runbook | Title | Key Patterns |
|---------|-------|--------------|
| **01** | Property Validation Fundamentals | Validation rules, conditional required fields, API validation |
| **02** | Property Population Monitoring | Completeness scoring, fill rate monitoring, remediation workflows |
| **03** | Integration Health Checks | Salesforce sync health, field mapping drift detection |
| **04** | Data Enrichment Strategies | Multi-source enrichment, mastering policy, credits governance |
| **05** | Duplicate Detection & Deduplication | Canonical selection algorithm, merge operations, SF constraints |

**Runbook Location**: `../docs/runbooks/data-quality/`

**Before Operations**: Check relevant runbooks for current best practices, known issues, and proven patterns.
...
```bash
node scripts/lib/hubspot-data-hygiene-specialist-optimizer.js <options>
```
...
node scripts/lib/hubspot-data-hygiene-specialist-optimizer.js --portal my-portal
```
...
**Note**: While v4 batch associations are preferred for all objects, company-to-company associations may return 405 on v4 batch endpoints. In that case, fall back to v3 PUT. Labeled associations ALWAYS require v4.
