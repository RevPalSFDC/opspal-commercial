...
```bash
node scripts/lib/dedup-snapshot-generator.js my-config.json
node scripts/lib/dedup-clustering-engine.js my-config.json ./snapshot.json
node scripts/lib/dedup-canonical-selector.js my-config.json ./bundles.json
```
...
node scripts/lib/dedup-rollback-manager.js ./snapshot.json ./config.json \
...
node scripts/lib/dedup-ledger.js list dedupe-2025-10-14 failed

# Export audit trail
node scripts/lib/dedup-ledger.js export dedupe-2025-10-14 ./audit.csv

# Clear ledger (after successful completion)
node scripts/lib/dedup-ledger.js clear dedupe-2025-10-14
```
...
```bash
node scripts/lib/dedup-ledger.js --help
node scripts/lib/dedup-config-loader.js --help
node scripts/lib/dedup-snapshot-generator.js --help
# ... etc
```
...
```bash
# View command help
cat .claude-plugins/opspal-data-hygiene/commands/dedup-companies.md
```
