...
class DedupExecutor {
    ...
    async attachSFAccountToCanonical(bundle) {
        ...
        try {
            // Strategy:
            // 1) Query Salesforce contacts for the account
            // 2) Find first synced HubSpot contact using salesforcecontactid bridge
            // 3) Set that contact's PRIMARY company association to canonical company
            // 4) Optionally nudge sync using configured contact property

            const sfContactIds = await this.findSalesforceContactIdsForAccount(sfAccountId);
            if (sfContactIds.length === 0) {
                throw new Error(`No Salesforce contacts found for account ${sfAccountId}`);
            }

            let bridgeContact = null;
            for (const sfContactId of sfContactIds) {
                bridgeContact = await this.findHubSpotContactBySalesforceId(sfContactId);
                if (bridgeContact) break;
            }

            if (!bridgeContact) {
                throw new Error(
                    `No HubSpot contact bridge found for SF account ${sfAccountId} ` +
                    `(checked ${sfContactIds.length} Salesforce contacts)`
                );
            }

            await this.setPrimaryCompanyAssociation(bridgeContact.id, canonicalCompanyId);
            const syncNudge = await this.nudgeHubSpotContactSync(bridgeContact.id);

            console.log(
                `    ✅ Attached via contact bridge (HS Contact ${bridgeContact.id}, ` +
                `SF Contact ${bridgeContact.salesforceContactId})`
            );

            this.ledger.recordCommitted('attach_sf_account', sfAccountId, canonicalCompanyId, {
                bridgeContactId: bridgeContact.id,
                bridgeProperty: bridgeContact.bridgeProperty,
                salesforceContactId: bridgeContact.salesforceContactId,
                syncNudge
            });
    ...
    /**
     * Query Salesforce contacts for an Account
     * @private
     */
    async findSalesforceContactIdsForAccount(accountId) {
        const soql = `SELECT Id FROM Contact WHERE AccountId = '${String(accountId).replace(/'/g, "\\'")}' ORDER BY LastModifiedDate DESC LIMIT 200`;

        if (this.salesforce?.orgAlias) {
            try {
                const command = `sf data query --query "${soql.replace(/"/g, '\\"')}" --target-org ${this.salesforce.orgAlias} --json`;
                const result = JSON.parse(execSync(command, { encoding: 'utf8' }));
                const records = result?.result?.records || [];
                return records.map(record => record.Id).filter(Boolean);
            } catch (error) {
                console.warn(`    ⚠️  Salesforce CLI contact query failed: ${error.message}`);
            }
        }

        if (this.salesforce?.instanceUrl && this.salesforce?.accessToken) {
            const response = await this.salesforceApiRequest(
                'GET',
                `/services/data/v61.0/query?q=${encodeURIComponent(soql)}`
            );
            const records = response?.records || [];
            return records.map(record => record.Id).filter(Boolean);
        }

        throw new Error('Salesforce connection not configured (orgAlias or instanceUrl/accessToken required)');
    }

    /**
     * Find HubSpot contact bridged to a Salesforce Contact ID
     * @private
     */
    async findHubSpotContactBySalesforceId(sfContactId) {
        const bridgeProperties = [
            'salesforcecontactid',
            'hs_salesforce_contact_id',
            'external_salesforce_contact_id'
        ];

        for (const propertyName of bridgeProperties) {
            try {
                const response = await this.hubspotApiRequest('POST', '/crm/v3/objects/contacts/search', {
                    filterGroups: [{
                        filters: [{
                            propertyName,
                            operator: 'EQ',
                    ...
                    properties: ['email', 'firstname', 'lastname', 'salesforcecontactid', propertyName],
                    limit: 1
                });
                ...
                if (contact?.id) {
                    return {
                        id: contact.id,
                        bridgeProperty: propertyName,
                        salesforceContactId: sfContactId
                    };
                }
            } catch (_) {
                // Property may not exist in this portal - continue trying other candidates
            }
        }
    ...
    /**
     * Set HubSpot contact primary company association
     * @private
     */
    async setPrimaryCompanyAssociation(contactId, companyId) {
        await this.hubspotApiRequest(
            'PUT',
    ...
    /**
     * Optional sync nudge for HubSpot↔Salesforce sync pipelines
     * @private
     */
    async nudgeHubSpotContactSync(contactId) {
        const propertyName = this.config?.execution?.syncNudgeProperty ||
            this.config?.hubspot?.syncNudgeProperty;

        if (!propertyName) {
            return { nudged: false, reason: 'syncNudgeProperty not configured' };
        }

        const value = new Date().toISOString();
        await this.hubspotApiRequest('PATCH', `/crm/v3/objects/contacts/${contactId}`, {
            properties: {
                [propertyName]: value
            }
        }, new Set([200, 204]));

        return { nudged: true, property: propertyName, value };
    }
     ...
     * @private
     */
    async hubspotApiRequest(method, endpoint, body = null, successCodes = new Set([200, 201, 204])) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    if (!successCodes.has(res.statusCode)) {
                        return reject(new Error(`HubSpot HTTP ${res.statusCode}: ${data}`));
                    }
     ...
     * @private
     */
    async salesforceApiRequest(method, endpoint, body = null, successCodes = new Set([200, 201, 204])) {
        return new Promise((resolve, reject) => {
            const instance = new URL(this.salesforce.instanceUrl);
            const options = {
                hostname: instance.hostname,
                path: endpoint,
                method,
                headers: {
                    'Authorization': `Bearer ${this.salesforce.accessToken}`,
                    'Content-Type': 'application/json'
                }
            };
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    if (!successCodes.has(res.statusCode)) {
                        return reject(new Error(`Salesforce HTTP ${res.statusCode}: ${data}`));
                    }
    ...
    async reparentContacts(contactIds, canonicalCompanyId) {
        ...
        // Critical discovery: 96.8% of contacts needed PRIMARY after duplicate removal (delta-corp cleanup, Oct 2025)
