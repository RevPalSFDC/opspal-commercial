...
```bash
cp .claude-plugins/opspal-data-hygiene/templates/dedup-config.template.json ./test-config.json
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-snapshot-generator.js \
  ./test-config.json
...
```bash
# Test clustering
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-clustering-engine.js \
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-canonical-selector.js \
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-executor.js \
  ./dedup-reports-test/canonical-map-*.json \
  ./test-config.json
...
**Critical Metric**:
- **96.8% of contacts typically need PRIMARY repair** (production data from delta-corp cleanup)
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-guardrail-manager.js \
  ./test-config.json
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-validation-framework.js \
  pre-execution \
  ./test-config.json
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-clustering-engine.js $SNAPSHOT

BUNDLES=$(ls -t $OUTPUT_DIR/bundles-*.json | head -1)
echo "Bundles: $BUNDLES"

echo "=== Phase 2: Canonical Selection ==="
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-canonical-selector.js $BUNDLES $CONFIG
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-executor.js $CANONICAL $CONFIG
...
then
    ...
    node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-executor.js $CANONICAL $CONFIG --execute
    
    echo "=== Phase 2.5: Association Repair ==="
    node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-association-repair.js $CANONICAL $CONFIG --execute
    
    echo "=== Phase 4: Guardrails ==="
    node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-guardrail-manager.js $CONFIG --execute
    
    echo "=== Validation ==="
    node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-validation-framework.js post-execution $CONFIG
fi
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-snapshot-generator.js ./test-config.json
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-association-verifier.js \
  ./test-config.json \
  verify-batch \
...
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-rollback-manager.js list

# Dry run rollback
node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-rollback-manager.js \
  rollback \
  ./dedup-reports-test/snapshot-*.json \
  ./test-config.json
...
time node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-snapshot-generator.js ./test-config.json
# Expected: 5-10 minutes for 10,000 companies

# Measure Phase 1 (Clustering)
time node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-clustering-engine.js ./snapshot.json
# Expected: ~1 minute

# Measure Phase 2 (Canonical Selection)
time node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-canonical-selector.js ./bundles.json ./test-config.json
# Expected: ~1 minute

# Measure Phase 3 (Execution)
time node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-executor.js ./canonical-map.json ./test-config.json --execute
...
/usr/bin/time -v node .claude-plugins/opspal-data-hygiene/scripts/lib/dedup-snapshot-generator.js ./test-config.json
