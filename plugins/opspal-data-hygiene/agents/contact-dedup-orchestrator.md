...
- **PersonNameMatcher** (**PREFERRED for person names**): Component-level matching (first+last separately)
  ...
  - Location: `opspal-core/scripts/lib/person-name-matcher.js`
...
**CRITICAL**: Use `PersonNameMatcher` for Contact/Lead name matching to prevent false positive duplicates:
...
// result.isMatch: true (nickname match)

const falsePositive = nameMatcher.match('Jeffrey Sudlow', 'Jeffrey Spotts');
// result.isMatch: false (last names don't match - correctly rejected)
```
